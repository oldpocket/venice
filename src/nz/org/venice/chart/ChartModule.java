...
/**
 * The charting module for venice. This class provides the user interface used
 * to draw any of the required charts. Example:
 * 
 * <pre>
 ...
 * <h2>Structure</h2>
 *
 * The chart module is made up of three core classes. These core classes are:
 * <code>ChartModule</chart>, <code>Chart</code> & <code>BasicChartUI</code>.
 *
 * <p>
 * <ul>
 *
 * <li><code>BasicChartUI</chart>
 * <p>
 ...
 * <code>JComponent</code> class. It provides the code to allow the user to
 * select and unselect a portion of the chart and also manages graph annotations
 * via tooltips.</li>
 *
 * <li><code>ChartModule</chart>
 * <p>
 * This class handles the integration of the chart module with <i>Venice</i>. It
 * is the container class of the actual chart, and is responsible for laying out
 * the chart widget and the toolbar in a frame. It also provides the menu.</li>
 *
 * </ul>
 *
 * <p>
 ...
 *
 * <dt><i>Annotations</i>
 * <dd>These appear on the graph as little yellow notes indicating <i>Graph</i>
 * specific information to the user. This information may include buy/sell
 * recommendations or any other data.
 *
 * <dt><i>Chart</i>
 * <dd>A chart represents the entire graphable area. The chart can consist of
 * several <i>Graph Levels</i>, each graph level may contain several
 * <i>Graphs</i>.
 *
 * <dt><i>Graph</i>
 * <dd>A graph represents a specific type of graph to display, for example a
 * <i>Line Graph</i>, a <i>Bar Graph</i>, a <i>Moving Average Graph</i> etc.
 * These graphs can then be used to display different things to the user, for
 * example the <i>Bar Graph</i> can be used to graph a stock's volume. A <i>Line
 * Graph</i> can be used to graph a stock's day close.
 *
 * <dt><i>Graph Level</i>
 * <dd>For each chart there can be several <i>levels</i> of graph, these levels
 * are displayed vertical one on top of the other. The top level may contain
 * several stock's day close graphs. The bottom levels typically contain
 * indicators such as volume or RSI.
 *
 * <dt><i>Graph Source</i>
 * <dd>Contains useful information that <i>Graphs</i> need to know so they can
 * graph particular data (such as quote data). This information includes the
 * values to be graphed, a title, the axis types to use and any
...
public class ChartModule extends JPanel implements IModule, MouseListener, MouseMotionListener, ActionListener {
	...
	public ChartModule(JDesktopPane desktop, ChartModuleSettings settings) {
		...
		while (levelIterator.hasNext()) {
			GraphSettingsGroup settingsGroup = (GraphSettingsGroup) levelIterator.next();
			...
			while (graphIterator.hasNext()) {
				GraphSettingsGroup subSettingsGroup = (GraphSettingsGroup) graphIterator.next();
			...
			if (primaryGraph.getSourceType() == IGraphSource.SYMBOL) {
				try {
					...
					add(primaryGraph, s, primaryQuoteBundle, levelIndex, menuSettings);
		...
		chart.resetBuffer();
		chart.setXRange(settings.getStartX(), settings.getEndX());
	...
	private void initChart(boolean indexChart) {
		...
		addMenuItem.setAccelerator(KeyStroke.getKeyStroke('A', java.awt.Event.CTRL_MASK, false));
		addMenuItem.addActionListener(this);
		menu.add(addMenuItem);
		menu.addSeparator();

		closeMenuItem = new JMenuItem(Locale.getString("CLOSE"));
		closeMenuItem.setAccelerator(KeyStroke.getKeyStroke('C', java.awt.Event.CTRL_MASK, false));
	...
	public void add(IGraph graph, Symbol symbol, EODQuoteBundle quoteBundle, int level) {
		...
		EODQuoteChartMenu menu = new EODQuoteChartMenu(this, quoteBundle, symbol, graph, indexChart);
	...
	public void add(IGraph graph, Symbol symbol, EODQuoteBundle quoteBundle, int level, MenuSettings menuSettings) {
		...
		// Add menu for this quote
		EODQuoteChartMenu menu = new EODQuoteChartMenu(this, quoteBundle, symbol, graph, indexChart, menuSettings);
	...
	public void add(IGraph graph, Portfolio portfolio, EODQuoteBundle quoteBundle, int level,
		...
		// Add menu for this portfolio
		PortfolioChartMenu menu = new PortfolioChartMenu(this, quoteBundle, portfolio, graph, menuSettings);
	...
	/**
	 * Add the graph to the specified level. This is identical to the add method
	 * except that it does not add a new menu for the graph.
	 *
	 * @param graph The new graph to add
	 * @param level graph level to add the new graph
	 * @see #add
	...
	/**
	 * Create a new level and add the graph. This is identical to the add method
	 * except that it does not add a new menu for the graph.
	 *
	 * @param graph the new graph to add.
	 * @see #add
	...
	/**
	 * Remove the graph from the chart. Currently does not remove the menu for the
	 * appropriate symbol. Probably should.
	 *
	 ...
	 * Annotations are little popup text notes that contain information about the
	 * graph, such as buy/sell suggestions.
	 *
	...
	public void mousePressed(MouseEvent e) {
		...
		if (x != null && viewMode == MOVING) {
			...
			if ((start = chart.move(mouseX, mouseY)) != null) {
				...
				chart.setDrawnLineStart(start.getXData(), start.getYData(), start.getYCoord());
			}
		}
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (x != null && viewMode == EDITING) {
			String val = JOptionPane.showInputDialog(this, Locale.getString("ENTER_TEXT"), "Text",
	...
	public void actionPerformed(ActionEvent e) {
		...
		/*
		 * paintOnChart and eraseOnChart are "toggle switches" - they toggle the editing
		 * and zoom functions. Ie While in editing mode, a user won't be able to select
		 * portions of the chart.
		 */

		/*
		 * FIXME: Change interface to reflect this. Either identify the mode or make the
		 * buttons depressed or something.
		 * 
		 ...
		 */
			...
			Thread menuAction = new Thread() {
				public void run() {
					SortedSet symbols = SymbolListDialog.getSymbols(desktop, Locale.getString("ADD_GRAPH"));
			...
			if (symbol.compareTo(prev) != 0) {
				...
				while (levelIterator.hasNext()) {
					...
					while (graphIterator.hasNext()) {
						...
						if (graphSymbol.compareTo(symbol) == 0 && graphAdded.get(graphKey) == null) {
							GraphSettings graphSettings = new GraphSettings(String.valueOf(graph.hashCode()),
									String.valueOf(chart.hashCode()), graph.getName());

							graphAdded.put(graphKey, graphSettings);
							/*
							 * Exclude AdvanceDecline graphs because of the length of time required to
							 * construct them.
							 */
							...
							primaryGroup = (GraphSettingsGroup) symbolAdded.get(graphSymbol);
							// Primary Graph
							if (levelIndex == 0 && primaryGroup == null) {
								...
								GraphSettingsGroup subGraphSettingsGroup = new GraphSettingsGroup();

								java.util.List subGraphSettingsList = primaryGroup.getSubGraphSettingsList();
	...
	private EODQuoteBundle getQuoteBundle(String symbol, GraphSettings graphSettings) {
		...
		case IGraphSource.SYMBOL:
			try {
				...
				EODQuoteBundle bundle = new EODQuoteBundle(new EODQuoteRange(s));
		...
		case IGraphSource.PORTFOLIO:
			try {
				...
				EODQuoteBundle bundle = new EODQuoteBundle(new EODQuoteRange(symbolsTraded, startDate, endDate));
	...
	/**
	 * Functions that will run after the chart has been fully loaded TODO: Check if
	 * we should implement listeners/observers
	 */

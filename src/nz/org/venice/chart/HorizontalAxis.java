...
public class HorizontalAxis {
	...
	public void drawLabels(Graphics g, double scale, int x, int y) {
		...
		while (pointIterator.hasNext()) {
			...
			if (availableWidth > 1.5 * g.getFontMetrics().stringWidth(string)) {
				midX = (int) (((value.intValue() + lastValue) / 2) * scale) + x;
				startX = midX - g.getFontMetrics().stringWidth(string) / 2;
	...
	public void drawGrid(Graphics g, int y, double horizontalScale, int heightOfGraph) {
		...
		lineSize = (int) (STATIC_GRID_HEIGHT + heightOfGraph / VARIABLE_GRID_HEIGHT_SCALE);
		...
		while (iterator.hasNext()) {
			...
			if (i++ > 0 && iterator.hasNext())
				g.drawLine((int) (axisPoint.intValue() * horizontalScale), y,
						(int) (axisPoint.intValue() * horizontalScale), y - lineSize);
		}
	}
	...
	private String stringValue(TradingDate date) {
		...
		case (QUARTERS):
			return "Q" + Integer.toString(value(date));
		case (MONTHS):
	...
	public int getWidth() {
		...
		if (points.size() > 2)
			return ((Integer) points.elementAt(2)).intValue() - ((Integer) points.elementAt(1)).intValue();

		// unless weve only two points
		else if (points.size() > 1)
			return ((Integer) points.elementAt(1)).intValue() - ((Integer) points.elementAt(0)).intValue();

...
/**
 * Moving Average Convergence Divergence graph. This graph draws two simple
 * moving averages, the fast one in green, the slow one in red. When the two
 * lines cross they indicate a <b>Buy</b> or <b>Sell</b> recommendation.
 */
public class MACDGraph extends AbstractGraph {
	 ...
	 */
	private void createMACDGraph(Graphable source, int periodOne, int periodTwo, double smoothingConstantOne,
			double smoothingConstantTwo) {
		...
		if (periodOne > periodTwo) {
			slowMovingAverage = ExpMovingAverageGraph.createMovingAverage(source, periodOne, smoothingConstantOne);
			fastMovingAverage = ExpMovingAverageGraph.createMovingAverage(source, periodTwo, smoothingConstantTwo);
		} else {
			slowMovingAverage = ExpMovingAverageGraph.createMovingAverage(source, periodTwo, smoothingConstantTwo);
			fastMovingAverage = ExpMovingAverageGraph.createMovingAverage(source, periodOne, smoothingConstantOne);
		}
	}
	...
	private void createMACDGraph(Graphable source, int periodOne, int periodTwo) {
		...
		slowMovingAverage = MovingAverageGraph.createMovingAverage(source, slowPeriod);
		fastMovingAverage = MovingAverageGraph.createMovingAverage(source, fastPeriod);
	}
	...
	public void render(Graphics g, Color colour, int xoffset, int yoffset, double horizontalScale, double verticalScale,
		...
		GraphTools.renderLine(g, fastMovingAverage, xoffset, yoffset, horizontalScale, verticalScale, topLineValue,
				bottomLineValue, dates, vertOrientation);

		// Slow moving line
		g.setColor(Color.red.darker());
		GraphTools.renderLine(g, slowMovingAverage, xoffset, yoffset, horizontalScale, verticalScale, topLineValue,
				bottomLineValue, dates, vertOrientation);
	}
	...
	public double getHighestY(List x) {
		...
		return fastHighestY > slowHighestY ? fastHighestY : slowHighestY;
	}
	...
	public double getLowestY(List x) {
		...
		return fastLowestY < slowLowestY ? fastLowestY : slowLowestY;
	}
	...
	public void setSettings(HashMap settings) {
		...
		// create MACD according to the type of average that user have been defined (EMA
		// or SMA)
		if (averageType.compareTo(MACDGraphUI.SMA) == 0)
			createMACDGraph(getSource().getGraphable(), periodFirstAverage, periodSecondAverage);
		else
			createMACDGraph(getSource().getGraphable(), periodFirstAverage, periodSecondAverage,
					smoothingConstantFirstAverage, smoothingConstantSecondAverage);

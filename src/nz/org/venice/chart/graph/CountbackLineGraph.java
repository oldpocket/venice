...
/**
 * 3 Bar Nett Countback lines. This graph draws a single across a price value
 * which indicates a buy signal if used with highs in a down trend and a sell
 * signal with lows in a new uptrend.
 *
 * @author Mark Hummel
...
public class CountbackLineGraph extends AbstractGraph {
	...
	public void render(Graphics g, Color colour, int xoffset, int yoffset, double horizontalScale, double verticalScale,
		...
		GraphTools.renderLine(g, countback, xoffset, yoffset, horizontalScale, verticalScale, topLineValue,
	 ...
	 */
	public Graphable createCountbackLine(Graphable dayHigh, Graphable dayLow, int type) {
		...
		case BREAKOUT:
			...
			countbackValue = getBreakoutCBLValue(dataHigh, localMin, dataLow[localMin]);
			index = localMin;
			break;

		case STOPLOSS:
			localMax = getHighestHigh(dataHigh);
			countbackValue = getStopLossCBLValue(dataLow, localMax, dataLow[localMax]);
	...
	public void setSettings(HashMap settings) {
		...
		countback = createCountbackLine(dayHigh, dayLow, type);
	 ...
	 */
	private double getBreakoutCBLValue(double[] data, int startIndex, double high) {
		int count = 0;
		int i;
	 ...
	 */
	private double getStopLossCBLValue(double[] data, int startIndex, double low) {
	...
	/**
	 *
	 * Return the index to the first local minimum from the end of the data that may
	 * also be a pivot point low, indicative of a breakout.
	 *
	 * @param data An array of price data

...
public class KDGraphUI implements GraphUI {
	...
	private void buildPanel() {
		...
		kSmoothTextField = GridBagHelper.addTextRow(panel, Locale.getString("KSMOOTH"), "", layout, c, 8);
		dSmoothTextField = GridBagHelper.addTextRow(panel, Locale.getString("DSMOOTH"), "", layout, c, 8);
	}
	...
	public String checkSettings(HashMap settings) {
		...
		if (kSmooth < MINIMUM_K_SMOOTH || kSmooth > period)
			return Locale.getString("ERROR_KSMOOTH_LIMITS", MINIMUM_K_SMOOTH);
		...
		if (dSmooth < MINIMUM_D_SMOOTH || dSmooth > period)
			return Locale.getString("ERROR_DSMOOTH_LIMITS", MINIMUM_D_SMOOTH);
	...
	/**
	 * Retrieve the k smooth value from the settings hashmap. If the hashmap is
	...
	/**
	 * Retrieve the d smooth value from the settings hashmap. If the hashmap is
	 * empty, then return the default.
	 *
	 * @param settings the settings

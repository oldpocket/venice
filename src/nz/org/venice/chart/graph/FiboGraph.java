...
public class FiboGraph extends AbstractGraph {
	 ...
	 */
	public FiboGraph(IGraphSource source) {
		super(source);
	 ...
	 */
	public FiboGraph(IGraphSource source, HashMap settings) {
		super(source);
	...
	public void render(Graphics g, Color colour, int xoffset, int yoffset, double horizontalScale, double verticalScale,
		...
		for (int i = 0; i < fibopct.length; i++) {
			...
			GraphTools.renderHorizontalLine(g, yValue, xoffset, yoffset, horizontalScale, verticalScale,
					bottomLineValue, topLineValue, xRange, vertOrientation);

			label = fmt.format(fibopct[i] * 100) + "% " + fmt1.format(yValue);
			renderLabel(g, yValue, label, xoffset, yoffset, horizontalScale, verticalScale, topLineValue,
					bottomLineValue, xRange);

		}
	}

	public static void renderLabel(Graphics g, double yValue, String text, int xoffset, int yoffset,
			double horizontalScale, double verticalScale, double topLineValue, double bottomLineValue, List xRange) {

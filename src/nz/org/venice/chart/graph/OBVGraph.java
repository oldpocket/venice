...
/**
 * On Balance Volume (OBV) graph. This graph is used as a precursor for stock
 * trends. If it goes down its a possible indicator that the stock will too and
 * vice-versa. OBV is calculated by starting with an arbitary constant. To
 * calculate a day's OBV, get the previous day's OBV. If the stock went up that
 * day, add the volume. If the stock went down, minus the volume.
 *
...
public class OBVGraph extends AbstractGraph {
	...
	public OBVGraph(IGraphSource open, IGraphSource close, IGraphSource volume) {
		...
		// create OBV
		obv = createOBV(open.getGraphable(), close.getGraphable(), volume.getGraphable());
	}
	...
	public void render(Graphics g, Color colour, int xoffset, int yoffset, double horizontalScale, double verticalScale,
		...
		g.setColor(colour);
		GraphTools.renderLine(g, obv, xoffset, yoffset, horizontalScale, verticalScale, topLineValue, bottomLineValue,

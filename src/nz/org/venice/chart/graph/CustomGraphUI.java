...
public class CustomGraphUI implements GraphUI {
	 ...
	 */
	public CustomGraphUI(HashMap settings, Graphable source, EODQuoteBundle quoteBundle, Symbol symbol) {
		this.source = source;
	...
	private void buildPanel() {
		...
		indicatorComboBox = GridBagHelper.addExpressionRow(panel, Locale.getString("CUSTOM"), "", layout, c);
		isPrimaryCheckBox = GridBagHelper.addCheckBoxRow(panel, Locale.getString("IS_PRIMARY"), true, layout, c);
	}
	 ...
	 * section, but rather with the core graph code. This case is an exception since
	 * the user interface needs to generate the graph to verify that the custom
	 * indicator expression is valid.
	 *
	 ...
	 * @exception EvaluationException if there was an error evaluating the
	 *                                expression
	 ...
	 * chart has two kinds of graphs - a primary and a secondary. All primary graphs
	 * are drawed together (in the top graph). All secondary graphs are drawn
	 * individually (below the top graph).
	 *
	 * @param settings the settings
	 * @return <code>true</code> if the graph is primary; <code>false</code>
	 *         otherwise
	 */

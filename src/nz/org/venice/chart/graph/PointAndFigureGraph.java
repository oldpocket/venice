...
/**
 * Point and Figure graph. This graph draws a series of characters (X/O) mapping
 * the general movement. A change in column shows a reversal such that price
 * difference met the price scale.
 *
 * @author Mark Hummel
...
public class PointAndFigureGraph extends AbstractGraph {
	...
	public PointAndFigureGraph(GraphSource source, HashMap settings) {
		...
		if (priceReversalString != null && boxString != null) {
			try {
				...
				pointAndFigure = createPointAndFigureGraph(source.getGraphable(), priceReversalScale, boxPriceScale);
	...
	public void render(Graphics g, Color colour, int xoffset, int yoffset, double horizontalScale, double verticalScale,
		...
		GraphTools.renderMarker(g, pointAndFigure, xoffset, yoffset, horizontalScale, verticalScale, topLineValue,
	...
	/*
	 * We currently return the price value of the top of an upmove and the bottom of
	 * a downmove rather than returning all points and dates in between.
	 * 
	 */
	public String getToolTipText(Comparable x, int y, int yoffset, double verticalScale, double bottomLineValue) {
		...
		index = (upmove) ? getLargestElement(list) : getSmallestElement(list);
		...
		if (index < list.size()) {
			...
			text = "<html>" + getSource().getName() + "," + date.toLongString() + "<p>";
	 ...
	 * <li>Stay in the same column until price changes direction, and then move one
	 * column to the right.</li>
	 ...
	 */
	public static PFGraphable createPointAndFigureGraph(Graphable source, double priceReversalScale,
		...
		while (iterator.hasNext()) {
			...
			if (plot) {
				...
				/*
				 * Rules for generating PF state that when price moves by greater than twice the
				 * box value the inteverning boxes are marked.
				 */

				/*
				 * When prev = 0.0, no plot has yet been made. We avoid putting in intervening
				 * marks in this case to avoid a large column of 'X's arising out of 0.
				 */
				...
				if (prev != 0.0) {
					while (diff >= 0.0) {
						double tmp = (upmove) ? values[i] - (boxPriceScale * counter)
								: values[i] + (boxPriceScale * counter);

						double tmp2 = 100 * boxPriceScale;
						double tmp3 = tmp * 100;

						/*
						 * Round values to fit in box value operands multiplied by by 100 and then
						 * result divided by 100 to avoid round off error.
						 */
				...
				/*
				 * Needs more data than just price and column for tooltips, because in vanilla
				 * P&F, no passage of time is indicated.
				 */
		...
		/*
		 * Column span is reduced to create a gap, which makes graph look less
		 * cluttered.
		 * 
		 * The point of spanning data across date "columns" is so that in the Graphable
		 * each date associates with a price point.
		 */
		...
		for (i = 0; i <= columnNumber; i++) {
			...
			pointAndFigure.putData(date, data.getDate(), data.getList(), data.getMarker());
	...
	private double calculateDefaultBoxPriceScale() {
		...
		while (numColumns < minColumns) {
			temp = createPointAndFigureGraph(getSource().getGraphable(), defaultBoxPriceScale * 3.0,

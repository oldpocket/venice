...
/**
 * Support and Resistence "graph". This graph draws single lines across common
 * price occurrances.
 *
 * The support and resistence levels are determined by the most numerous troughs
 * and peaks within the last year of data that have the greatest vertical
 * separation and do not overlap.
 *
 *
...
public class SupportAndResistenceGraph extends AbstractGraph {
	...
	/*
	 * Fudge factors: These values seemed to work well; I didn't do any sort of
	 * exhaustive experiment or optimisation to determine them.
	 * 
	 * Length of trend: directionCount > 4 Overlap tolerance: 20
	 * 
	 */
	...
	// See Graph.java
	public void render(Graphics g, Color c, int xoffset, int yoffset, double horizontalScale, double verticalScale,
			double topLineValue, double bottomLineValue, List xRange, boolean vertOrientation) {

		g.setColor(Color.green.darker());
		GraphTools.renderLine(g, support, xoffset, yoffset, horizontalScale, verticalScale, topLineValue,
				bottomLineValue, xRange, vertOrientation);

		g.setColor(Color.red.darker());
		GraphTools.renderLine(g, resistence, xoffset, yoffset, horizontalScale, verticalScale, topLineValue,
				bottomLineValue, xRange, vertOrientation);
	 ...
	 * @return tool tip text containing the day low, day high and day close quotes
	 */
	 ...
	 * 
	 *                   Currently only the points of the most numerous peaks and
	 *                   troughs are used as a heuristic to determine the points of
	 *                   support and resistence. It's a heuristic only, and still
	 *                   needs work.
	 */
	public Graphable[] createSupportAndResistence(Graphable source, Graphable support, Graphable resistence, int lag,
			int type) {
		...
		// level
		for (i = values.length - 1; i > stop || peaksIndex < 5 || troughsIndex < 5; i--) {
			diff = values[i] - prev;
		...
		int scoreList[] = new int[(Math.max(peaksCount.size(), troughsCount.size()))];

		/*
		 * Mark levels high which are the most numerous, have the greatest vertical
		 * separation and whose separation is the least.
		 */

		while (i < (Math.min(peaksCount.size(), troughsCount.size()))) {
			...
			if (start1 - overlapTolerance > end2 || end1 < start2 - overlapTolerance) {
				score += overlapBonus;
		...
		case HEURISTIC:
			...
			while (iterator.hasNext()) {
				...
				if (values[i] == resistPrice || values[i] == supportPrice) {
	...
	private double binOccur(HashMap bins, double values[], int length) {
		...
		while (iterator.hasNext()) {
			...
			for (i = 0; i < values.length; i++) {
				...
				if (values[i] >= binValue && values[i] < binValue + binInc) {

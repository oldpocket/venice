...
/**
 * Barchart graph. This graph draws a single vertical line between the day low
 * and the day high values and draws horizontal bars at the day open and the day
 * close.
 */
public class BarChartGraph extends AbstractGraph {
	 ...
	 */
	public BarChartGraph(IGraphSource dayOpen, IGraphSource dayLow, IGraphSource dayHigh, IGraphSource dayClose) {
		super(dayClose);
	...
	public void render(Graphics g, Color c, int xoffset, int yoffset, double horizontalScale, double verticalScale,
		...
		render(g, c, xoffset, yoffset, horizontalScale, verticalScale, topLineValue, bottomLineValue, xRange, true);
	}
	...
	public void render(Graphics g, Color c, int xoffset, int yoffset, double horizontalScale, double verticalScale,
		...
		while (iterator.hasNext()) {
			...
			if (dayLowY != null && dayHighY != null && dayCloseY != null) {
				...
				openY = GraphTools.calcYCoord(yoffset, dayOpenY.doubleValue(), topLineValue, bottomLineValue,
						verticalScale, vertDirection);

				lowY = GraphTools.calcYCoord(yoffset, dayLowY.doubleValue(), topLineValue, bottomLineValue,
						verticalScale, vertDirection);

				highY = GraphTools.calcYCoord(yoffset, dayHighY.doubleValue(), topLineValue, bottomLineValue,
						verticalScale, vertDirection);

				closeY = GraphTools.calcYCoord(yoffset, dayCloseY.doubleValue(), topLineValue, bottomLineValue,
						verticalScale, vertDirection);

				// Draw bar
//		g.drawLine(xCoordinate, lowY, xCoordinate, highY);
				g.drawLine(xCoordinate + halfbarWidth + 1 + halfBlankWidth, lowY,
						xCoordinate + halfbarWidth + 1 + halfBlankWidth, highY);
...
//			   closeY);
				g.drawLine(xCoordinate + halfbarWidth + 1 + halfBlankWidth, closeY,
						...
						/* DAY_CLOSE_BAR_WIDTH * horizontalScale */ , closeY);
				// Draw perpendicular line indicating day open
//		g.drawLine(xCoordinate, openY,
//			   (int)(xCoordinate - 0.2 * horizontalScale
//				 /*DAY_CLOSE_BAR_WIDTH * horizontalScale*/),
//			   openY);
				g.drawLine(xCoordinate + halfBlankWidth, openY, xCoordinate + halfbarWidth + 1 + halfBlankWidth
				/* DAY_CLOSE_BAR_WIDTH * horizontalScale */, openY);
	...
	public String getToolTipText(Comparable x, int y, int yoffset, double verticalScale, double bottomLineValue) {
		...
		if (dayLowY != null && dayHighY != null) {
			...
			int dayLowYCoordinate = yoffset
					- GraphTools.scaleAndFitPoint(dayLowY.doubleValue(), bottomLineValue, verticalScale);

			int dayHighYCoordinate = yoffset
					- GraphTools.scaleAndFitPoint(dayHighY.doubleValue(), bottomLineValue, verticalScale);

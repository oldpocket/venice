 ...
 * of graphs. Classes that implement this interface will be able to render the
 * graph onto the chart and provide axes information. Each graph can have its
 * own user interface to allow the user to set user-definable parameters.
 *
...
public interface Graph {
	 ...
	 *                        this if the graph should be drawn in a certain colour
	 *                        - such as a Moving Average graph
	 ...
	 *                        starts
	 * @param horizontalScale horizontal scale factor; use this to convert between X
	 *                        value in the <code>xRange</code> to a cartesian
	 *                        coordinate x
	 * @param verticalScale   vertical scale factor; use this to convert between Y
	 *                        value to a cartesian coordinate y
	 ...
	 * @param xRange          a <code>List</code> of <code>Comparable</code> that
	 *                        contain the X values to plot
	...
	/**
	 * Return an array of acceptable major deltas for the vertical axis. The
	 * vertical axis is created in such a way that each horizontal line is placed in
	 * the graph at a sensible value, e.g. a graph may have four horizontal lines,
	 * each separated by $1.20:
	 *
	 * <pre>
	 * --------------- $4.80
	 *
	 ...
	 * </pre>
	 *
	 * Here the values of the horizontal lines are $1.20, $2.40, $3.60 & $4.80.
	 * These are <i>sensible</i> values, values such as $1.36, $2.72, $4.08 & $5.44
	 * are not as sensible. All the allowable values are calculated by the cross
	 * product of Major Deltas x Minor Deltas.
	 *
	...
	/**
	 * Return the graph's current settings. Each graph must contain its user
	 * definable settings in a hashmap. If a graph does not have any user definable
	 * settings, then it can just return an empty hashmap here.
	 *
	 * @return settings
	 */
	 ...
	 * Pass the initial settings or an empty hashmap to use the graph's default
	 * settings. If the graph does not have any user-definable settings, then it
	 * should not have a user interface and should return <code>null</code> here.
	 *
	 ...
	 * appear together in the top chart. <i>Secondary</i> graphs appear in their own
	 * charts which are added below. The day close graph is primary; while the day
	 * close graph is secondary.
	 *
	...
	/**
	 * Return whether the graph has a price for range xRange Is trivially true for
	 * all graphs apart from the PointAndFigure graph as P&F is the only graph type
	 * where price data does not map one to one to date.
	 *
	 * @param xRange Range of X values

...
public class BasicChartUI extends ComponentUI implements ImageObserver {
	...
	public synchronized Double getYAtPoint(Chart chart, int yCoordinate) {
		...
		VerticalAxis verticalAxis = (VerticalAxis) verticalAxes.elementAt(level);
		...
		double dataValue = GraphTools.getPointFromScale(yCoordinate, bottomLineValue, yoffset, verticalScale);
	 ...
	 * @return the level which contains the y coordinate or <code>0</code> if none
	 *         do
	 */
	...
	public String getToolTipText(Chart chart, int xCoordinate, int yCoordinate) {
		...
		if (yCoordinate < (height - X_LABEL_HEIGHT) && verticalAxis != null && x != null) {
	...
	public void paint(Graphics g, JComponent c) {
		...
		if (image == null || image.getWidth() < width || image.getHeight() < height
			...
			image = new BufferedImage(width + BUFFER_BUFFER_SIZE, height + BUFFER_BUFFER_SIZE,
		...
		if (width != bufferWidth || height != bufferHeight || cdm.dataExists()) {
	...
	private synchronized void bufferedPaint(Graphics g, Chart chart, int width, int height) {
	...
	private void calculateHorizontalAxes(Graphics g, Chart chart, int width, int height) {
		...
		horizontalScale = HorizontalAxis.calculateScale(width - Y_LABEL_WIDTH, chart.getXRange().size());

		if (horizontalScale * MINIMUM_TRADING_DAYS_IN_MONTH < 2 * g.getFontMetrics().stringWidth(new String("Feb"))) {

			if (quartersHorizontalAxis == null)
				quartersHorizontalAxis = new HorizontalAxis(chart.getXRange(), HorizontalAxis.QUARTERS,
						HorizontalAxis.MINOR);

			minorHorizontalAxis = quartersHorizontalAxis;
		} else {
			if (monthsHorizontalAxis == null)
				monthsHorizontalAxis = new HorizontalAxis(chart.getXRange(), HorizontalAxis.MONTHS,
						HorizontalAxis.MINOR);
		...
		if (majorHorizontalAxis == null)
			majorHorizontalAxis = new HorizontalAxis(chart.getXRange(), HorizontalAxis.YEARS, HorizontalAxis.MAJOR);
	}
	...
	private void drawText(Graphics g, Chart chart) {
		...
		g.setColor(BasicChartUI.getComplementaryColour(PreferencesManager.getDefaultChartBackgroundColour()));
		...
		while (it.hasNext()) {
			...
			if (it2.hasNext()) {
				...
				fontSize = 12;
				/*
				 * Is verticalScale monitor/resolution indedendant? In any case, these fudge
				 * factors should be replaced/ calculated
				 */
	...
	private int getYCoordinate(Chart chart, Coordinate point) {
		return getYCoordinate(chart, point.getYData(), point.getYCoord().intValue(), point.getLevel());
	}
	...
	private int getYCoordinate(Chart chart, Double y, int yIndex, int level) {
		...
		int scaledPoint = GraphTools.scaleAndFitPoint(y.doubleValue(), (double) bottomLineValue, verticalScale);
	...
	// For the given Y value, return the Y coordinate
	private int getYCoordinate(Chart chart, Double y, int yIndex, int height2, int lvl) {
		...
		int yOffset = verticalAxis.getHeightOfGraph() + (height2 - verticalAxis.getHeightOfGraph()) / 2;
	...
	public double getDifference(Chart chart, Integer x, Integer y, Coordinate start, Coordinate end) {
	...
	public Coordinate getMoved(Chart chart, Integer x, Integer y, Coordinate start, Coordinate end) {
		Comparable dataX;
		Double dataY;
		int absY;
		int x1, y1, x2, y2;
		...
		/*
		 * Determine which end of the line the user has chosen to move.
		 */
		dist1 = Math.sqrt(((x1 - x.intValue()) * (x1 - x.intValue())) + ((y1 - y.intValue()) * (y1 - y.intValue())));

		dist2 = Math.sqrt(((x2 - x.intValue()) * (x2 - x.intValue())) + ((y2 - y.intValue()) * (y2 - y.intValue())));
	...
	/*
	 * Return true if coordinates c1 and c2 are "within" delta of each other. True
	 * distance would be pythagorean.
	 */
	...
	private void drawLevels(Graphics g, Chart chart, int width, int height) {
		...
		while (iterator.hasNext()) {
			...
			drawLevel(g, graphs, chart, yoffset, width,
					((Integer) levelHeights.elementAt(level)).intValue() - firstHorizontalLine, level);
	...
	private String getLevelTitle(Vector graphs) {
		...
		/*
		 * // Now convert list of symbols to string of comma separated // company names
		 * String title = new String(""); String companyName = null;
		 * 
		 * symbolsIterator = symbols.iterator();
		 * 
		 * while(symbolsIterator.hasNext()) { symbol = (String)symbolsIterator.next();
		 * 
		 * if(title.length() != 0) title = title.concat(", ");
		 * 
		 * companyName = QuoteSourceManager.getSource().getSymbolName(symbol);
		 * 
		 * if(companyName != null) title = title.concat(companyName); }
		 */
	...
	private VerticalAxis calculateVerticalAxis(Chart chart, Vector graphs, int height, int level) {
		...
		if (verticalAxis == null) {
			...
			verticalAxis = new VerticalAxis(getLowestY(chart.getXRange(), graphs),
					getHighestY(chart.getXRange(), graphs), firstGraph.getAcceptableMinorDeltas(),
	...
	private void drawGraphs(Graphics g, Vector graphs, Chart chart, VerticalAxis verticalAxis, int yoffset, int width,
		...
		while (iterator.hasNext()) {
			...
			graph.render(g, getGraphColour(graph, chart), 0,
					...
					horizontalScale, verticalAxis.getScale(), verticalAxis.getTopLineValue(),
					verticalAxis.getBottomLineValue(), chart.getXRange(), chart.getOrientation());
		}
	}
	...
	private void drawHorizontalGrid(Graphics g, Chart chart, VerticalAxis verticalAxis, int yoffset) {
		...
		minorHorizontalAxis.drawGrid(g, yoffset + verticalAxis.getHeightOfGraph(), horizontalScale,
				verticalAxis.getHeightOfGraph());
		majorHorizontalAxis.drawGrid(g, yoffset + verticalAxis.getHeightOfGraph(), horizontalScale,
				verticalAxis.getHeightOfGraph());
	}
	...
	private void drawHorizontalLabels(Graphics g, int height) {
		...
		minorHorizontalAxis.drawLabels(g, horizontalScale, 0, height - X_LABEL_HEIGHT + g.getFontMetrics().getHeight());
		majorHorizontalAxis.drawLabels(g, horizontalScale, 0,
				height - X_LABEL_HEIGHT + g.getFontMetrics().getHeight() * 2);
		...
		while (iterator.hasNext()) {
			...
			while (innerIterator.hasNext()) {
				...
				if (annotations != null && chart.isAnnotated(graph))
					drawGraphAnnotations(g, chart, graph, verticalAxis, yoffset, annotations);
	...
	private void drawGraphAnnotations(Graphics g, Chart chart, Graph graph, VerticalAxis verticalAxis, int yoffset,
		...
		while (iterator.hasNext()) {
			...
			// chart
			if (x.compareTo(chart.getStartX()) >= 0 && x.compareTo(chart.getEndX()) <= 0) {
				...
				if (y != null) {
					...
					int yCoordinate = yoffset + verticalAxis.getHeightOfGraph() - GraphTools.scaleAndFitPoint(
							y.doubleValue(), verticalAxis.getBottomLineValue(), verticalAxis.getScale());

					drawAnnotation(g, text, getXCoordinate(chart, x), yCoordinate);
				}
			}
		}
	}
	...
	private void drawAnnotation(Graphics g, String text, int x, int y) {
		...
		int width = g.getFontMetrics().stringWidth(text) + 2 * ANNOTATED_TEXT_MARGIN;
		...
		g.setColor(Color.black);
		g.drawString(text, x + ANNOTATED_TEXT_MARGIN, y + height - g.getFontMetrics().getDescent());
	}
	...
	private int getFirstHorizontalLine(Graphics g) {
		return (firstHorizontalLine = (int) (TOP_GRAPH_BUFFER_MULTIPLIER * g.getFontMetrics().getHeight()));
	}
	...
	private void calculateColourMap(Chart chart) {
		...
		if (colourMap == null) {
			...
			Color[] colours = { Color.blue.darker(), Color.cyan.darker(), Color.magenta.darker(), Color.orange.darker(),
					Color.pink.darker(),

					Color.blue, Color.magenta, Color.orange, Color.pink,

					Color.blue.darker().darker(), Color.cyan.darker().darker(), Color.magenta.darker().darker(),
					Color.orange.darker().darker(), Color.pink.darker().darker(),

					Color.blue.brighter(), Color.cyan.brighter(), Color.magenta.brighter(), Color.orange.brighter(),
					Color.pink.brighter() };
	...
	/**
	 * Return the colour we will draw the given graph. Note that the graph may
	 * choose to override this colour.
	 *
	...
	/**
	 * 
	 * Given a colour, return it's colour wheel complement so that it generates a
	 * useful contrast. (e.g Given white return black, blue return orange etc)
	 * 
	...
	/*
	 * This code treats RGB space as a vector in R3 (4 for alpha channel) To find
	 * the colour complement, this method: 1. Finds the closest main colour defined
	 * in bright/dark array 2. Calculates the distance between the given colour and
	 * the closest colour. 3. Apply that delta to the main complement colour (ie if
	 * the closest colour is white, the closest opposite is black) and return that
	 * colour.
	 * 
	 * Further refinement: Calculate angles from the origin, and rotate the delta
	 * using those angles.
	 */
	public static Color getComplementaryColour(Color colour) {
		...
		oppositeColour = (closestIsBright) ? darkColours[minColourIndex] : brightColours[minColourIndex];

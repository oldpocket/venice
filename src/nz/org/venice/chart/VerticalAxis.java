...
public class VerticalAxis {
	...
	private void calculateOptimumValueDelta() {
		...
		for (int i = 0; i < major.length; i++) {
			for (int j = 0; j < minor.length; j++) {
				...
				bottomLineValue = (double) Math.floor(lowest / valueDelta) * valueDelta;
				topLineValue = bottomLineValue + (horizontalLines - 1) * valueDelta;
	...
	public void drawGridAndLabels(Graphics g, IGraph graph, String title, int xoffset, int yoffset, int width) {
		...
		// of space
		if ((yLabelVerticalOffset + g.getFontMetrics().getHeight()) > horizontalLineSpacing)
		...
		for (int i = 0; i < horizontalLines; i++) {
			...
			yLabel = graph.getYLabel(topLineValue - valueDelta * i);
			...
			g.drawString(yLabel, xoffset + width + Y_LABEL_WIDTH - yLabelWidth - Y_LABEL_HORIZONTAL_OFFSET,
					y - yLabelVerticalOffset);

			// draw title
			if (i == 0)
				g.drawString(title, xoffset + Y_LABEL_HORIZONTAL_OFFSET, y - yLabelVerticalOffset);
		}

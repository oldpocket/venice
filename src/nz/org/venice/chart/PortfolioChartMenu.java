...
/**
 * Provides a menu which is associated with a stock symbol being graphed. The
	...
	public void actionPerformed(ActionEvent e) {
		...
		// Otherwise check dynamic menus
			...
			// Ok looks like its adding a graph
				...
				addGraph(new LineGraph(
						new PortfolioGraphSource(portfolio, quoteBundle, PortfolioGraphSource.RETURN_VALUE),
						RETURN_VALUE, true), RETURN_VALUE, 0);
			else if (text == CASH_VALUE)
				addGraph(
						new LineGraph(new PortfolioGraphSource(portfolio, quoteBundle, PortfolioGraphSource.CASH_VALUE),
								CASH_VALUE, true),
						CASH_VALUE, 0);
			else if (text == SHARE_VALUE)
				addGraph(new LineGraph(
						new PortfolioGraphSource(portfolio, quoteBundle, PortfolioGraphSource.SHARE_VALUE), SHARE_VALUE,
						true), SHARE_VALUE, 0);

			else if (text == STOCKS_HELD)
				addGraph(
						new BarGraph(new PortfolioGraphSource(portfolio, quoteBundle, PortfolioGraphSource.STOCKS_HELD),
								STOCKS_HELD, false),
						STOCKS_HELD);
			...
			// Otherwise it's an account in the portfolio
				...
				addGraph(
						new LineGraph(new PortfolioGraphSource(portfolio, quoteBundle, accountName), accountName, true),
						accountName, 0);
			}
		}
	}

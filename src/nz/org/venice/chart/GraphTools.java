...
public class GraphTools {
	...
	public static void renderHorizontalLine(Graphics g, double yValue, int xoffset, int yoffset, double horizontalScale,
			double verticalScale, double topLineValue, double bottomLineValue, List xRange, boolean vertOrientation) {

		int vertDirection = (vertOrientation) ? 1 : -1;
		int startX = xoffset;
		int endX = (int) (xoffset + (xRange.size() - 1) * horizontalScale);

		int y = calcYCoord(yoffset, yValue, topLineValue, bottomLineValue, verticalScale, vertDirection);

		g.drawLine(startX, y, endX, y);
	}

	public static void renderLine(Graphics g, Graphable source, int xoffset, int yoffset, double horizontalScale,
			double verticalScale, double topLineValue, double bottomLineValue, List xRange) {
		renderLine(g, source, xoffset, yoffset, horizontalScale, verticalScale, topLineValue, bottomLineValue, xRange,
				true);
	}
	...
	public static void renderLine(Graphics g, Graphable source, int xoffset, int yoffset, double horizontalScale,
		...
		while (iterator.hasNext()) {
			...
			if (y != null) {
				...
				yCoordinate = calcYCoord(yoffset, y.doubleValue(), topLineValue, bottomLineValue, verticalScale,
						vertDirection);
...
//			       lastXCoordinate, lastYCoordinate);
					g.drawLine(xCoordinate + halfbarWidth + 1 + halfBlankWidth, yCoordinate, lastXCoordinate,
							lastYCoordinate);
				else
//		    g.drawLine(xCoordinate, yCoordinate,
//
...
//		lastXCoordinate = xCoordinate;
	...
	public static void renderBar(Graphics g, Graphable source, int xoffset, int yoffset, double horizontalScale,
			double verticalScale, double topLineValue, double bottomLineValue, List xRange) {
		renderBar(g, source, xoffset, yoffset, horizontalScale, verticalScale, topLineValue, bottomLineValue, xRange,
				true);
	}
...
//	int x1 = -1;
		...
		y2 = calcYCoord(yoffset, 0, topLineValue, bottomLineValue, verticalScale, vertDirection);
		...
		while (iterator.hasNext()) {
			...
			y1 = calcYCoord(yoffset, doubleValue, topLineValue, bottomLineValue, verticalScale, vertDirection);
...
//	    x1 = x2 + 1;
	...
	public static double getPointFromScale(double point, double offset, double height, double scale) {
	...
	public static void renderChar(Graphics g, PFGraphable source, int xoffset, int yoffset, double horizontalScale,
			double verticalScale, double topLineValue, double bottomLineValue, List xRange) {

		renderChar(g, source, xoffset, yoffset, horizontalScale, verticalScale, topLineValue, bottomLineValue, xRange,
				true);
	...
	// chars is character to draw - in synch with list xRange
	public static void renderMarker(Graphics g, PFGraphable source, int xoffset, int yoffset, double horizontalScale,
			double verticalScale, double topLineValue, double bottomLineValue, List xRange) {

		renderMarker(g, source, xoffset, yoffset, horizontalScale, verticalScale, topLineValue, bottomLineValue, xRange,
				true);
		...
		while (iterator.hasNext()) {
			...
			for (int j = 0; yList != null && j < yList.size(); j++) {
				...
				if (y != null) {
					...
					yCoordinate2 = calcYCoord(yoffset, y.doubleValue() - boxPrice, topLineValue, bottomLineValue,
					...
					if (marker.compareTo("O") == 0) {
						...
						g.drawOval(xCoordinate, yCoordinate, deltaX, deltaY / 2);
					} else {
						g.drawLine(xCoordinate, yCoordinate, deltaX + xCoordinate, yCoordinate2);

						g.drawLine(xCoordinate, yCoordinate2, deltaX + xCoordinate, yCoordinate);
	...
	/**
	 * 
	 * Utility method for calculating screen points given a data value, scale and
	 * direction.
	 *
	 ...
	 * @param verticalScale   The verticalScale used to convert between Y value and
	 *                        cartesian coordinate y.
	 * @param vertDirection   A flag indicating the vertical direction of the graph:
	 *                        1 = Graph is drawn from top to bottom -1 = Graph is
	 *                        drawn from bottom to top
	 * 
	 */

	public static int calcYCoord(int yoffset, double yValue, double topLineValue, double bottomLineValue,
			double verticalScale, int vertDirection) {
		...
		return vertOffset - scaleAndFitPoint(vertDirection * yValue, vertDirection * bottomLineValue, verticalScale);
	}
}

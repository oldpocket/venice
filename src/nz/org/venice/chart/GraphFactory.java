...
public class GraphFactory {
	...
	/**
	 * Create a new graph according to the name and using the supplied symbol and
	 * quote bundle.
	 * 
	 ...
	 * @param quoteBundle the end of day bundle containg quote data
	 * @param symbol      The symbol of the graph
	 * @param adjust      An optional adjustment (for splits etc) applied to the
	...
	public static Graph newGraph(String graphName, boolean index, EODQuoteBundle quoteBundle, Symbol symbol,
		...
		if (graphName == Locale.getString("BAR_CHART")) {
			...
			graph = new CountbackLineGraph(sources[DAY_LOW], sources[DAY_HIGH], sources[DAY_CLOSE]);
	...
	private static GraphSource[] getSources(String graphName, EODQuoteBundle quoteBundle, boolean index,
		...
		if (adjust != null) {
			for (int i = 0; i < SOURCE_LEN; i++) {
				...
				if (sources[i] != null) {
					sources[i].adjust(adjust.getType(), adjust.getValue(), adjust.getStartPoint(),
							adjust.getDirection());
	...
	/*
	 * These could be replaced with a single getQuoteType up in getSources but was
	 * done like this for historical reasons and readability. Also, I expect the

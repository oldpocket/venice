...
/**
 * Parse a string into an executable expression. This class acts as a gatekeeper
 * to the <i>Gondala</i> language which is used to perform manipulations on
 * stock market quotes.
 * <p>
 * <h2>Langauge EBNF</h2>
 * 
...
public class Parser {
	...
	/**
	 * Parse the given string into an executable expression. The first argument
	 * allows you to parse any predefined variables which are available to the
	 * expression. Any variables defined by the expression will be added to this.
	 *
		...
		while (head.getType() == Token.INCLUDE_TOKEN) {
			...
			if (includedStoredExpression == null) {
				...
				try {
					...
					IncludeExpression incExpression = new IncludeExpression(includedExpressions);
	...
	private static IExpression parseFactor(Variables variables, TokenStack tokens) throws ParserException {
		...
		// NUMBER
		if (tokens.match(Token.NUMBER_TOKEN) || tokens.match(Token.TRUE_TOKEN) || tokens.match(Token.FALSE_TOKEN)
		...
		// FUNCTION
		else if (tokens.match(Token.LAG_TOKEN) || tokens.match(Token.MIN_TOKEN) || tokens.match(Token.MAX_TOKEN)
				|| tokens.match(Token.AVG_TOKEN) || tokens.match(Token.SD_TOKEN) || tokens.match(Token.SUM_TOKEN)
				|| tokens.match(Token.RSI_TOKEN) || tokens.match(Token.NOT_TOKEN) || tokens.match(Token.PERCENT_TOKEN)
				|| tokens.match(Token.DAY_OF_WEEK_TOKEN) || tokens.match(Token.DAY_OF_YEAR_TOKEN)
				|| tokens.match(Token.DAY_TOKEN) || tokens.match(Token.MONTH_TOKEN) || tokens.match(Token.YEAR_TOKEN)
				|| tokens.match(Token.SQRT_TOKEN) || tokens.match(Token.ABS_TOKEN) || tokens.match(Token.CORR_TOKEN)
				|| tokens.match(Token.EMA_TOKEN) || tokens.match(Token.BBL_TOKEN) || tokens.match(Token.BBU_TOKEN)
				|| tokens.match(Token.MACD_TOKEN) || tokens.match(Token.MOMENTUM_TOKEN) || tokens.match(Token.OBV_TOKEN)
				|| tokens.match(Token.SD_TOKEN) || tokens.match(Token.SIN_TOKEN) || tokens.match(Token.COS_TOKEN)
				|| tokens.match(Token.LOG_TOKEN) || tokens.match(Token.EXP_TOKEN) || tokens.match(Token.TREND_TOKEN)
				|| tokens.match(Token.RANDOM_TOKEN) || tokens.match(Token.ALERT_TOKEN) || tokens.match(Token.HALT_TOKEN)
				|| tokens.match(Token.FLOOR_TOKEN) || tokens.match(Token.CEIL_TOKEN)
				|| tokens.match(Token.LOGGING_TOKEN) || tokens.match(Token.DATACHECK_TOKEN))
			expression = parseFunction(variables, tokens);

		// ABBREVIATION QUOTE FUNCTIONS
		else if (tokens.match(Token.DAY_OPEN_TOKEN) || tokens.match(Token.DAY_CLOSE_TOKEN)
				|| tokens.match(Token.DAY_HIGH_TOKEN) || tokens.match(Token.DAY_LOW_TOKEN)
				|| tokens.match(Token.DAY_VOLUME_TOKEN))
			expression = parseDayQuoteFunction(variables, tokens);

		// FLOW_CONTROL
		else if (tokens.match(Token.IF_TOKEN) || tokens.match(Token.FOR_TOKEN) || tokens.match(Token.WHILE_TOKEN))
		...
		// DECLARE VARIABLE
		else if (tokens.match(Token.CONSTANT_TOKEN) || tokens.match(Token.BOOLEAN_TOKEN)
				|| tokens.match(Token.INTEGER_TOKEN) || tokens.match(Token.FLOAT_TOKEN))
	...
	private static IExpression parseVariable(Variables variables, TokenStack tokens) throws ParserException {
		...
		if (variable == null) {
			...
			if (variable.isFunction()) {
				...
				/*
				 * If no parameters were supplied, add a terminal expression which will be
				 * ignored. (ClauseExpression requires at least one child)
				 */
	...
	private static IExpression parseFunction(Variables variables, TokenStack tokens) throws ParserException {
		...
		// Create epxression
		expression = ExpressionFactory.newExpression(function, arg1, arg2, arg3, arg4);
	...
	private static IExpression parseUserFunction(Variables variables, TokenStack tokens, int type)
		...
		if (variables.contains(functionNameStr) && parameterMap.get(functionNameStr) == null) {
			throw new ParserException(Locale.getString("VARIABLE_DEFINED_ERROR", functionName.getVariableName()));
		}

		variables.add(functionNameStr, type, false, true, 0.0);
		...
		if (!tokens.match(Token.RIGHT_PARENTHESIS_TOKEN)) {
			...
			parameterExpression = parseParameters(variables, tokens, functionName.getVariableName());
		} else {
			parseRightParenthesis(variables, tokens);
		}
	...
	private static IExpression parseParameters(Variables variables, TokenStack tokens, String functionName)
		...
		while (token.getType() != Token.RIGHT_PARENTHESIS_TOKEN) {
			...
			if (parameterMap.get(name) != null && parameterMap.get(name).equals(functionName)) {
				throw new ParserException(Locale.getString("VARIABLE_DEFINED_ERROR", name));
			}
			...
			if (!tokens.match(Token.BOOLEAN_TOKEN) && !tokens.match(Token.FLOAT_TOKEN)
					&& !tokens.match(Token.INTEGER_TOKEN) && !tokens.match(Token.RIGHT_PARENTHESIS_TOKEN)) {
				throw new ParserException(Locale.getString("UNEXPECTED_SYMBOL_ERROR"));
			}
			token = tokens.pop();
		}

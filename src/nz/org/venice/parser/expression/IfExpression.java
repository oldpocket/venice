...
public class IfExpression extends TernaryExpression {
	...
	/**
	 * Check the input arguments to the expression. The first argument must be
	 * {@link #BOOLEAN_TYPE}, the remaining arguments can be {@link #BOOLEAN_TYPE},
	 * {@link #FLOAT_TYPE} or {@link #INTEGER_TYPE} and must be the same.
	 *
	...
	public int checkType() throws TypeMismatchException {
		if (getChild(0).checkType() == BOOLEAN_TYPE && (getChild(1).checkType() == getChild(2).checkType()
				|| ((getChild(1).getType() == BOOLEAN_TYPE && getChild(2).getType() == BOOLEAN_TYPE)))) {
			return getType();
		} else {
			String types = getChild(0).getType() + " , " + getChild(1).getType() + " , " + getChild(2).getType();

			String expectedTypes = BOOLEAN_TYPE + " , " + BOOLEAN_TYPE + " , " + BOOLEAN_TYPE;
	...
	public int getType() {
		...
		if (getChild(1).getType() == BOOLEAN_TYPE && getChild(2).getType() == BOOLEAN_TYPE) {
			return BOOLEAN_TYPE;
		} else if (getChild(1).getType() == FLOAT_TYPE || getChild(2).getType() == FLOAT_TYPE) {
			return FLOAT_TYPE;
		} else if (getChild(1).getType() == INTEGER_TYPE && getChild(2).getType() == INTEGER_TYPE) {
			return INTEGER_TYPE;
		} else {

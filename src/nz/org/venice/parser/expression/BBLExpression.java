...
/**
 * An expression which finds the BBL (Bollinger Band Lower) quote over a given
 * trading period.
 *
...
public class BBLExpression extends TernaryExpression {
	...
	/**
	 * Create a new Bollinger Band Lower expression for the given <code>quote</code>
	 * kind, for the given number of <code>days</code>, starting with
	 ...
	 */
	public BBLExpression(IExpression quote, IExpression days, IExpression lag) {
	...
	public double evaluate(Variables variables, IQuoteBundle quoteBundle, Symbol symbol, int day)
		...
		if (period <= 0) {
			EvaluationException e = EvaluationException.BBL_RANGE_EXCEPTION;
		...
		if (offset > 0) {
			EvaluationException e = EvaluationException.BBL_OFFSET_EXCEPTION;
	...
	public String toString() {
		...
		return new String("bol_lower(" + c1 + ", " + c2 + ", " + c3 + ")");
	}
	...
	public Object clone() {
		return new BBLExpression((IExpression) getChild(0).clone(), (IExpression) getChild(1).clone(),

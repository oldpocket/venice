...
/**
 * An expression which finds the OBV (On Balance Volume) over a given trading
 * period.
 *
...
public class OBVExpression extends TernaryExpression {
	...
	/**
	 * Create a new On Balance Volume (OBV) expression for the given
	 * <code>quote</code> kind, for the given number of <code>days</code>, starting
	 * with <code>lag</code> days away.
	 *
	...
	public String toString() {
		/*
		 * assert getChild(0) != null; assert getChild(1) != null; assert getChild(2) !=
		 * null;
		 */
		...
		/*
		 * return new String("obv(" + getChild(0).toString() + ", " +
		 * getChild(1).toString() + ", " + getChild(2).toString() + ")");
		 */
	}

	/*
	 * public boolean testSimplify() { for (int i = 0; i < getChildCount(); i++) {
	 * if (getChild(i) == null) { return false; } } return true; }
	 */
	...
	public int checkType() throws TypeMismatchException {
		...
		if (getChild(0).checkType() == INTEGER_TYPE && getChild(1).checkType() == INTEGER_TYPE
			...
			String expectedTypes = INTEGER_TYPE + " , " + INTEGER_TYPE + " , " + INTEGER_TYPE;

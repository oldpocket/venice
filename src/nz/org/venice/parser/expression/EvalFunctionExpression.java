...
public class EvalFunctionExpression extends UnaryExpression {
	...
	public double evaluate(Variables variables, QuoteBundle quoteBundle, Symbol symbol, int day)
		...
		/*
		 * We use parameters (as a copy of variables) so that if a function sets the
		 * value of a parameter it doesn't overwrite the value of a function.
		 * 
		 * But we also need to transfer the value in the case where the value of a
		 * variable is set, but it's not a parameter.
		 * 
		 * e.g. Case 1: int myvar = 0 int function procMyVar(int myvar) { myvar = 3 }
		 * 
		 * e.g. Case2: int myvar = 0 int function setMyVar(int parm) { myvar = parm }
		 * 
		 * In Case1, when the function exits, we want myvar to be 0. In Case2, when the
		 * function exits, we want myvar to be parm.
		 * 
		 */
		...
		while (variableIterator.hasNext()) {
			...
			if (v1.getValue() != v2.getValue() && setParameters.get(v1.getName()) == null) {
	...
	private boolean checkParameters() {
		...
		if (parameterValuesList.getChildCount() != parameterNamesList.getChildCount()) {
			...
			// to have parameters
			if (parameterValuesList.getChildCount() == 1 && parameterNamesList.getChildCount() == 0) {
				return true;
			}
			return false;
		}
	...
	private void setupParameters(Variables parameters, QuoteBundle quoteBundle, Symbol symbol, int day)
		...
		for (int i = 0; i < parameterValuesList.getChildCount(); i++) {
			DefineParameterExpression nameExpression = (DefineParameterExpression) parameterNamesList.getChild(i);

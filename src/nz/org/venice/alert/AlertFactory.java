...
public class AlertFactory {
	...
	/**
	 * Create a writer which creates file alerts directly using the user
	 * preferences.
	 *
	...
	/**
	 * Create a writers which creates database alerts directly using the user
	 * preferences.
	 *
	...
	public static Alert newAlert(Symbol symbol, TradingDate startDate, TradingDate endDate, String target,
			String boundType, String fieldType, boolean enabled, TradingDate dateSet) throws AlertException {
		if (boundType.equals("no bound") || fieldType.equals("no field")) {

			Alert alert = new GondolaAlert(symbol, startDate, endDate, target, enabled);
			...
			try {
				...
				Alert alert = new OHLCVAlert(symbol, startDate, endDate, targetValue.doubleValue(),
						Alert.stringToBoundType(boundType), fieldType, enabled);

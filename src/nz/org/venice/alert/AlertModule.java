 ...
 * user to display, modify and delete alerts which will trigger when a condition
 * is met. (e.g. close above a certain value)
 *
 * @author Mark Hummel
 * @see Alert
 */
public class AlertModule extends AbstractTable implements IModule, ActionListener {
	...
	private List createColumns() {
		...
		columns.add(new Column(SYMBOL_COLUMN, Locale.getString("SYMBOL"), Locale.getString("SYMBOLS_COLUMN_HEADER"),
		...
		columns.add(new Column(TARGET_COLUMN, Locale.getString("ALERT_TARGET"),
				Locale.getString("ALERT_TARGET_COLUMN_HEADER"), String.class, Column.VISIBLE));

		columns.add(new Column(BOUND_TYPE_COLUMN, Locale.getString("ALERT_BOUND_TYPE"),
				Locale.getString("ALERT_BOUND_TYPE"), String.class, Column.VISIBLE));

		columns.add(new Column(ALERT_TYPE_COLUMN, Locale.getString("ALERT_TYPE"), Locale.getString("ALERT_TYPE"),
				String.class, Column.VISIBLE));

		columns.add(new Column(FIELD_COLUMN, Locale.getString("ALERT_FIELD_TYPE"), Locale.getString("ALERT_FIELD_TYPE"),
				String.class, Column.VISIBLE));

		columns.add(new Column(DATE_SET_COLUMN, Locale.getString("DATE_COLUMN_HEADER"),
				Locale.getString("DATE_COLUMN_HEADER"), TradingDate.class, Column.VISIBLE));

		columns.add(new Column(STATUS_COLUMN, Locale.getString("ALERT_ENABLED"), Locale.getString("ALERT_ENABLED"),
				Boolean.class, Column.VISIBLE));
	...
	private void addMenu() {
		...
		JMenu alertMenu = MenuHelper.addMenu(menuBar, Locale.getString("ALERT_TITLE"));

		alertMenu.add(createShowColumnMenu(model));

		addAlert = MenuHelper.addMenuItem(this, alertMenu, Locale.getString("ALERT_ADD"));

		removeAlert = MenuHelper.addMenuItem(this, alertMenu, Locale.getString("ALERT_DEL"));

		editAlert = MenuHelper.addMenuItem(this, alertMenu, Locale.getString("ALERT_EDIT"));

		enableAlert = MenuHelper.addMenuItem(this, alertMenu, Locale.getString("ALERT_ENABLE"));

		disableAlert = MenuHelper.addMenuItem(this, alertMenu, Locale.getString("ALERT_DISABLE"));
	...
	public void actionPerformed(final ActionEvent e) {
		...
		if (e.getSource() == addAlert) {
			if (selectedRows.length > 0) {
				Symbol symbol = (Symbol) model.getValueAt(selectedRows[0], SYMBOL_COLUMN);
				addAlert(symbol);
			} else {
				addAlert(null);
			}
		...
		if (e.getSource() == removeAlert) {
			if (JOptionPane.showConfirmDialog(desktop, Locale.getString("ALERT_CONFIRM_DEL"),
					Locale.getString("ALERT_DEL"), JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
	...
	private void addAlert(final Symbol s) {
		...
		Thread showAddDialog = new Thread() {
			...
			public void run() {
				AlertDialog dialog = new AlertDialog(desktop, s, alertWriter);
	...
	private void editAlert(final Alert alert) {
		...
		Thread showEditDialog = new Thread() {
			...
			public void run() {
				AlertDialog dialog = new AlertDialog(desktop, alert, alertWriter);

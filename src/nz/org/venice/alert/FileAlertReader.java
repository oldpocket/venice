...
public class FileAlertReader implements AlertReader {
	...
	private Alert strToAlert(String line, String enabledString) throws AlertException {
		...
		try {
			...
			TradingDate endDate = (!endDateString.equals("NoEndDate"))
					? new TradingDate(endDateString, TradingDate.BRITISH)
					: null;
			...
			if (boundTypeString != null) {
				...
				newAlert = new OHLCVAlert(symbol, startDate, endDate, targetValue.doubleValue(), boundType,
						fieldTypeString, enabled);
			} else {
				newAlert = new GondolaAlert(symbol, startDate, endDate, target, true);
			}

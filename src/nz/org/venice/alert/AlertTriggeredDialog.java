...
public class AlertTriggeredDialog {
	...
	public static int show(JDesktopPane desktop, Alert alert, String triggerValue) {
		...
		Object[] buttons = { okButton, openTableButton, openChartButton, openAlertsButton };
		...
		int optionSelected = JOptionPane.showOptionDialog(desktop, message, "Alert", JOptionPane.INFORMATION_MESSAGE,
				JOptionPane.YES_NO_CANCEL_OPTION, aboutIcon, buttons, buttons[0]);
	...
	private static String generateMessage(Alert alert, String triggerValue) {
		...
		if (alert instanceof OHLCVAlert) {
			message += Alert.boundTypeToString(alert.getBoundType()) + " bound  on " + alert.getField()
					+ Locale.getString("ALERT_TRIGGER_MESG_OHLCV", alert.getSymbol().toString(), alert.getField(),
							triggerValue);
		} else {
			message += Locale.getString("ALERT_TRIGGER_MESG_EXP", alert.getSymbol().toString());
		}

		message += Locale.getString("ALERT_TRIGGER_MESG_DATE", alert.getDateSet().toString());
		return message;
	}
}
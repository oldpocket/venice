...
/**
 * Table model to display a portfolio to the user. The model tells a table how
 * to display the portfolio by describing the columns and how to populate the
 * table.
 *
...
public class PortfolioModel extends AbstractTableModel {
	 ...
	 * @param quoteBundle Quote Bundle. The quote bundle may be null, but only if
	 *                    the portfolio has never performed any trades.
	 */
	public PortfolioModel(Portfolio portfolio, EODQuoteBundle quoteBundle) {
		super();
		...
		columns.add(new Column(DATE_COLUMN, Locale.getString("DATE"), Locale.getString("DATE_COLUMN_HEADER"),
				TradingDate.class, Column.VISIBLE));
		columns.add(new Column(STOCKS_HELD_COLUMN, Locale.getString("STOCKS_HELD"),
				Locale.getString("STOCKS_HELD_COLUMN_HEADER"), String.class, Column.VISIBLE));
		columns.add(new Column(CASH_VALUE_COLUMN, Locale.getString("CASH_VALUE"),
				Locale.getString("CASH_VALUE_COLUMN_HEADER"), Money.class, Column.HIDDEN));
		columns.add(new Column(SHARE_VALUE_COLUMN, Locale.getString("SHARE_VALUE"),
				Locale.getString("SHARE_VALUE_COLUMN_HEADER"), Money.class, Column.HIDDEN));
		columns.add(new Column(MARKET_VALUE_COLUMN, Locale.getString("MARKET_VALUE"),
				Locale.getString("MARKET_VALUE_COLUMN_HEADER"), Money.class, Column.VISIBLE));
		columns.add(new Column(RETURN_VALUE_COLUMN, Locale.getString("RETURN_VALUE"),
				Locale.getString("RETURN_VALUE_COLUMN_HEADER"), Money.class, Column.HIDDEN));
		columns.add(new Column(MARKET_VALUE_CHANGE_COLUMN, Locale.getString("MARKET_VALUE_CHANGE"),
				Locale.getString("MARKET_VALUE_CHANGE_COLUMN_HEADER"), Money.class, Column.VISIBLE));
		columns.add(new Column(PERCENT_CHANGE_COLUMN, Locale.getString("PERCENT_CHANGE"),
				Locale.getString("PERCENT_CHANGE_COLUMN_HEADER"), ChangeFormat.class, Column.VISIBLE));
		setColumns(columns);
	}
	...
	/**
	 * This function examines the dates of the transactions in the portfolio, and
	 * the date range of the quote bundle, and returns a list of dates which we
	 * should dispaly in the table.
	 *
	...
	private List generateDateRange() {
		...
		// If there are no transactions in the portfolio, then there is nothing to
		// table.
		if (portfolio.countTransactions() == 0)
			...
			if (quoteBundle != null && quoteBundle.getLastDate() != null && quoteBundle.getLastDate().after(lastDate))

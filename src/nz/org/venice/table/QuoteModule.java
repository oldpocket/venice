 ...
 * user to display a table of specific stock quotes on specific days.
 *
...
public class QuoteModule extends AbstractTable implements Module, ActionListener {
	 ...
	 */
	public QuoteModule(EODQuoteBundle quoteBundle, boolean singleDate) {
	 ...
	 */
	public QuoteModule(EODQuoteBundle quoteBundle, String filterExpressionString, boolean singleDate) {
		...
		model = new EODQuoteModel(quoteBundle, quotes, singleDate ? Column.HIDDEN : Column.VISIBLE,
				quoteBundle.getAllSymbols().size() == 1 ? Column.HIDDEN : Column.VISIBLE);
		setModel(model,
				quoteBundle.getAllSymbols().size() == 1 ? EODQuoteModel.DATE_COLUMN : EODQuoteModel.ACTIVITY_COLUMN,
				SORT_UP);
	...
	private List extractQuotesUsingRule(String filterExpression, EODQuoteBundle quoteBundle) {
		...
		try {
			...
			JOptionPane.showInternalMessageDialog(DesktopManager.getDesktop(),
					e.getReason() + ": " + expression.toString(), Locale.getString("ERROR_EVALUATION_EQUATION"),
	...
	private void addMenu() {
		...
		// Table Menu
		{
			...
			applyExpressions = MenuHelper.addMenuItem(this, tableMenu, Locale.getString("APPLY_EQUATIONS"));

			applyFilter = MenuHelper.addMenuItem(this, tableMenu, Locale.getString("APPLY_FILTER"));

			sortByMostActive = MenuHelper.addMenuItem(this, tableMenu, Locale.getString("SORT_BY_MOST_ACTIVE"));
		...
		// Symbols Menu
		{
			...
			findSymbol = MenuHelper.addMenuItem(this, symbolsMenu, Locale.getString("FIND"));
			...
			alertSymbols = MenuHelper.addMenuItem(this, symbolsMenu, Locale.getString("ALERT_TITLE"));
		}
	...
	private void applyFilter() {
		...
		Thread thread = new Thread() {
			...
			public void run() {
				...
				String expressionString = ExpressionQuery.getExpression(desktop, Locale.getString("FILTER_BY_RULE"),
						Locale.getString("BY_RULE"), filterExpressionString);

				if (expressionString != null) {
					filterExpressionString = expressionString;

					// Get new list of symbols to display
					final List quotes = extractQuotesUsingRule(filterExpressionString, quoteBundle);
		...
		Thread thread = new Thread() {
			...
			public void run() {
				...
				if (symbol != null) {
					...
					JOptionPane.showInternalMessageDialog(DesktopManager.getDesktop(),
							Locale.getString("SYMBOL_X_NOT_FOUND", symbol.toString()),
							Locale.getString("SYMBOL_NOT_FOUND"), JOptionPane.INFORMATION_MESSAGE);
				}
			}
		};

...
public class TransactionDialog extends JInternalFrame implements ActionListener {
	...
	// Results of dialog. Has it finished. What button was pressed. What is the
	// transaction
	// on the screen.
	...
	public TransactionDialog(JDesktopPane desktop, Portfolio portfolio) {
		...
		dateTextField = GridBagHelper.addTextRow(mainPanel, Locale.getString("DATE"), today.toString("dd/mm/yyyy"),
				gridbag, c, 10);
	...
	private JPanel getDividendPanel() {
		...
		// Amount Line
		amountTextField = GridBagHelper.addTextRow(panel, Locale.getString("TOTAL_DIVIDEND_VALUE"), "", gridbag, c, 15);
	...
	private JPanel getDividendDRPPanel() {
		...
		sharesTextField = GridBagHelper.addTextRow(panel, Locale.getString("SHARES"), "", gridbag, c, 15);
	...
	private JPanel getAccumulatePanel() {
		...
		// Symbol Line
		symbolTextField = GridBagHelper.addTextRow(panel, Locale.getString("SYMBOL"), "", gridbag, c, 5);
		...
		// Share Value Line
		amountTextField = GridBagHelper.addTextRow(panel, Locale.getString("TOTAL_SHARE_VALUE"), "", gridbag, c, 15);

		amountTextField.setToolTipText(Locale.getString("TRANSACTION_TOTAL_AMOUNT_FIELD_TOOLTIP"));

		tradeCostTextField = GridBagHelper.addTextRow(panel, Locale.getString("TRADE_COST"), "", gridbag, c, 15);
	...
	private JPanel getCashPanel() {
		...
		amountTextField = GridBagHelper.addTextRow(panel, Locale.getString("AMOUNT"), "", gridbag, c, 15);
	...
	/**
	 * Display a dialog letting the user enter a new transaction. Add the
	...
	public boolean editTransaction(Transaction oldTransaction) {
		...
		// To the user pressed the OK button then add the transaction. To do this remove
		// all
	...
	private void displayTransaction(Transaction transaction) {
		...
		if (type == Transaction.ACCUMULATE) {
			...
			amountTextField.setText(String.valueOf(transaction.getAmount()));
			tradeCostTextField.setText(String.valueOf(transaction.getTradeCost()));
		} else if (type == Transaction.DEPOSIT || type == Transaction.FEE || type == Transaction.INTEREST
	...
	private Transaction buildTransaction() {
		...
		int type = Transaction.stringToType((String) typeComboBox.getSelectedItem());

		try {
			date = new TradingDate(dateTextField.getText(), TradingDate.BRITISH);
		} catch (TradingDateFormatException e) {
			String message = new String(Locale.getString("ERROR_PARSING_DATE", dateTextField.getText()));
			JOptionPane.showInternalMessageDialog(desktop, message, Locale.getString("ERROR_BUILDING_TRANSACTION"),
		...
		if (cashAccountComboBox != null) {
			String accountName = (String) cashAccountComboBox.getSelectedItem();
			cashAccount = (CashAccount) portfolio.findAccountByName(accountName);
		}

		CashAccount cashAccount2 = null;
		if (cashAccountComboBox2 != null) {
			String accountName = (String) cashAccountComboBox2.getSelectedItem();
			cashAccount2 = (CashAccount) portfolio.findAccountByName(accountName);
		}

		ShareAccount shareAccount = null;
		if (shareAccountComboBox != null) {
			String accountName = (String) shareAccountComboBox.getSelectedItem();
			shareAccount = (ShareAccount) portfolio.findAccountByName(accountName);
		}
		...
		try {
			if (amountTextField != null)
				if (!amountTextField.getText().equals(""))
					amount = new Money(cashAccount.getCurrency(), amountTextField.getText());
			...
			if (tradeCostTextField != null)
				if (!tradeCostTextField.getText().equals(""))
					tradeCost = new Money(cashAccount.getCurrency(), tradeCostTextField.getText());
		}
		...
		// Can't parse money field?
			...
			String message = new String(Locale.getString("ERROR_PARSING_MONEY", e.getReason()));
			JOptionPane.showInternalMessageDialog(desktop, message, Locale.getString("ERROR_BUILDING_TRANSACTION"),
					JOptionPane.ERROR_MESSAGE);
			return null;
		}
		...
		// Can't parse numeric field?
			...
			String message = new String(Locale.getString("ERROR_PARSING_NUMBER", e.getMessage()));
		...
		if (type == Transaction.TRANSFER && cashAccount == cashAccount2) {
			JOptionPane.showInternalMessageDialog(desktop, Locale.getString("SOURCE_AND_DESTINATION_SAME"),
					Locale.getString("ERROR_BUILDING_TRANSACTION"), JOptionPane.ERROR_MESSAGE);
			return null;
		}

		// If we are using the stock symbol check that its valid.
		// user hasn't tried to accumulate/reduce or dividend DRP 0 shares.
		if ((type == Transaction.ACCUMULATE || type == Transaction.REDUCE || type == Transaction.DIVIDEND_DRP)
				&& symbolParseError != null) {
			JOptionPane.showInternalMessageDialog(desktop, symbolParseError,
					Locale.getString("ERROR_BUILDING_TRANSACTION"), JOptionPane.ERROR_MESSAGE);
			return null;
		}
		...
		if ((type == Transaction.ACCUMULATE || type == Transaction.REDUCE || type == Transaction.DIVIDEND_DRP)
				&& shares <= 0) {
			JOptionPane.showInternalMessageDialog(desktop, Locale.getString("SHARES_GREATER_THAN_ZERO"),
					Locale.getString("ERROR_BUILDING_TRANSACTION"), JOptionPane.ERROR_MESSAGE);
			return null;
		}

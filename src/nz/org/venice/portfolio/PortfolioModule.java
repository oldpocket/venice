...
/**
 * Venice module for displaying a portfolio to the user. This module allows a
 * user to view a portfolio and, manage the accounts and transactions in that
 * portfolio.
 *
...
public class PortfolioModule extends JPanel implements Module, ActionListener {
	 ...
	 */
	public PortfolioModule(JDesktopPane desktop, Portfolio portfolio, EODQuoteBundle quoteBundle) {
	...
	private void createMenu() {
		...
		{
			portfolioGraph = MenuHelper.addMenuItem(this, portfolioMenu, Locale.getString("GRAPH"));
			portfolioTable = MenuHelper.addMenuItem(this, portfolioMenu, Locale.getString("TABLE"));
			...
			if (!portfolio.isTransient()) {
				...
				portfolioDelete = MenuHelper.addMenuItem(this, portfolioMenu, Locale.getString("DELETE"));

				portfolioRename = MenuHelper.addMenuItem(this, portfolioMenu, Locale.getString("RENAME"));
			}

			portfolioMenu.addSeparator();

			portfolioClose = MenuHelper.addMenuItem(this, portfolioMenu, Locale.getString("CLOSE"));
		}

		JMenu accountMenu = MenuHelper.addMenu(menuBar, Locale.getString("ACCOUNT"), 'A');
		{
			accountNewCashAccount = MenuHelper.addMenuItem(this, accountMenu, Locale.getString("NEW_CASH_ACCOUNT"));
			accountNewShareAccount = MenuHelper.addMenuItem(this, accountMenu, Locale.getString("NEW_SHARE_ACCOUNT"));
		}
		...
		JMenu transactionMenu = MenuHelper.addMenu(menuBar, Locale.getString("TRANSACTION"), 'T');
		{
			...
			transactionShowHistory = MenuHelper.addMenuItem(this, transactionMenu, Locale.getString("SHOW_HISTORY"));
		}
	...
	public void redraw() {
		...
		if (accounts.size() == 0) {
			...
			accountTable = new AccountTable(portfolio, quoteBundle);
	...
	private void restrictTableHeight(JScrollPane scrollPane, JTable table) {
		...
		double maximumHeight = table.getTableHeader().getPreferredSize().getHeight() + table.getRowHeight() * rows + 4;
	...
	public void tablePortfolio() {
		...
		// If the quote bundle has quotes before or at the first trade in the
		// portfolio then we will use the quote bundle given.
		if (portfolio.getStartDate() == null || quoteBundle.getFirstDate().compareTo(portfolio.getStartDate()) <= 0)
	...
	private void deletePortfolio() {
		...
		ConfirmDialog dialog = new ConfirmDialog(desktop, Locale.getString("SURE_DELETE_PORTFOLIO"),
	...
	private void renamePortfolio() {
		...
		TextDialog dialog = new TextDialog(desktop, Locale.getString("ENTER_NEW_PORTFOLIO_NAME"),
				Locale.getString("RENAME_PORTFOLIO"), oldPortfolioName);
	...
	private void newCashAccount() {
		AccountDialog dialog = new AccountDialog(desktop, Locale.getString("ENTER_ACCOUNT_NAME"),
				Locale.getString("NEW_CASH_ACCOUNT"), portfolio.getCurrency());
		...
		if (dialog.showDialog()) {
			...
			ExchangeRateCache.getInstance().getRate(portfolio.getLastDate(), accountCurrency, portfolio.getCurrency());
	...
	private void newShareAccount() {
		AccountDialog dialog = new AccountDialog(desktop, Locale.getString("ENTER_ACCOUNT_NAME"),
				Locale.getString("NEW_SHARE_ACCOUNT"), portfolio.getCurrency());
	 ...
	 * When the dialog is closed it will call redraw() on both the portfolio module
	 * window and the transaction module window (if its open) to ensure everything
	 * is kept in-sync.
	 */

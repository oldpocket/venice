...
/**
 * Representation of a portfolio. A portfolio object contains several accounts,
 * accounts can be either {@link CashAccount} or {@link ShareAccount}.
 *
...
public class Portfolio implements Cloneable {
	 ...
	 *
	 * @param name        The name of the portfolio
	 * @param isTransient Set to <code>true</code> if the portfolio displays working
	 *                    information and shouldn't be saved.
	 */
	 ...
	 *
	 * @return <code>true</code> if the portfolio is transient and shouldn't be
	 *         saved
	 */
	...
	public void addTransaction(Transaction transaction) {
		...
		// add all of the transactions. I.e. we must add the transactions in
		// chronological
		...
		// Otherwise we can just append
			...
			for (Iterator iterator = accounts.iterator(); iterator.hasNext();) {
				...
				if (account == transaction.getCashAccount() || account == transaction.getCashAccount2()
			...
			if (transaction.getType() == Transaction.WITHDRAWAL)
				deposits = exchangeRateCache.subtract(transaction.getDate(), deposits, transaction.getAmount());
			else if (transaction.getType() == Transaction.DEPOSIT)
				deposits = exchangeRateCache.add(transaction.getDate(), deposits, transaction.getAmount());
		}
	}
	...
	public Object clone() {
		...
		for (Iterator transactionIterator = transactions.iterator(); transactionIterator.hasNext();) {
			...
			if (clonedTransaction.getShareAccount() != null) {
				...
				String accountName = clonedTransaction.getShareAccount().getName();
			...
			if (clonedTransaction.getCashAccount() != null) {
				...
				String accountName = clonedTransaction.getCashAccount().getName();
			...
			if (clonedTransaction.getCashAccount2() != null) {
				...
				String accountName = clonedTransaction.getCashAccount2().getName();
	...
	/**
	 * Return the start date of this portfolio. The start date is defined as the
	...
	/**
	 * Get the value of the portfolio on the given day. Currently this function
	 * should only be called for dates after the last transaction. When it
	 * calculates the value it will assume all transactions have taken place.
	 *
	...
	/**
	 * Return an iterator that iterates over every day from the date of the first
	 * transaction to whenever the user chooses to stop iterating. Each iteration
	 * will return the state of the Portfolio on that date. This allows the user to
	 * efficiently query a range of historical states of the Portfolio.
	 *
	...
	/**
	 * Returns the state of the Portfolio on the given date. If the date is before
	 * the last transaction in the Portfolio, then this function will-reconstruct
	 * the Portfolio to how it would have been on that date. This can be slow, so if
	 * you want to query a range of dates, the {@link #iterator} method is more
	 * efficient.
	 *
	 ...
	 * @return <code>true</code> if the portfolios are equal; <code>false</code>
	 *         otherwise
	 */
	public boolean equals(Object object) {
		...
		return (portfolio.getName().equals(getName()) && portfolioAccounts.equals(accounts)
				&& portfolio.getTransactions().equals(getTransactions()) && portfolio.isTransient() == isTransient());
	}

	/**
	 * An iterator which iterates through the states of the Portfolio from the date
	 * of the first transaction to whenever the caller stops iterating.
	 */
	private class PortfolioIterator implements Iterator {
		...
		/**
		 * Create a new Portfolio Iterator that iterates over the given portfolio.
		 *
		...
		public PortfolioIterator(Portfolio referencePortfolio) {
			...
			// transactions.
			iteratorPortfolio = (Portfolio) referencePortfolio.clone();
			...
			List transactions = new ArrayList(iteratorPortfolio.getTransactions());
		...
		/**
		 * Traverse to the state of the Portfolio on the next trading date. The state of
		 * the Portfolio may be identical to the previous state.
		 */
		public Object next() {

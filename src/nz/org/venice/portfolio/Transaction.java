...
/**
 * Representation of a single transaction on the portfolio. A transaction is ANY
 * kind of financial action from trading a stock to receiving credit interest.
 * Transactions are used to build up a portfolio, the user does not enter how
 * many shares they own but rather their share transactions and their current
 * total is calculated from that.
 *
...
public class Transaction implements Cloneable, Comparable {
	 ...
	 * @param shares       for share trades and dividend DRPs, the number of symbols
	 *                     bought/solded/re-invested
	 * @param tradeCost    for shares trades, the cost of the trade including any
	 *                     GST, Stamp Duty, Taxes etc
	 ...
	 */
	public Transaction(int type, TradingDate date, Money amount, Symbol symbol, int shares, Money tradeCost,
			CashAccount cashAccount, CashAccount cashAccount2, ShareAccount shareAccount) {
		this.type = type;
		this.date = date;
		this.amount = amount;
		this.symbol = symbol;
	...
	/**
	 * Create a new withdrawal transaction. Money has been withdrawn from a
	 * {@link CashAccount}.
	 *
	 * @param date    date the transaction took place
	...
	public static Transaction newWithdrawal(TradingDate date, Money amount, CashAccount account) {
		return new Transaction(WITHDRAWAL, date, amount, null, 0, null, account, null, null);
	}

	/**
	 * Create a new deposit transaction. Money has been deposited into a
	 * {@link CashAccount}.
	 *
	 * @param date    date the transaction took place
	...
	public static Transaction newDeposit(TradingDate date, Money amount, CashAccount account) {
		return new Transaction(DEPOSIT, date, amount, null, 0, null, account, null, null);
	}

	/**
	 * Create a new interest transaction. {@link CashAccount} has received credit
	 * interest.
	 *
	 * @param date    date the transaction took place
	 * @param amount  the amount deposited
	...
	public static Transaction newInterest(TradingDate date, Money amount, CashAccount account) {
		return new Transaction(INTEREST, date, amount, null, 0, null, account, null, null);
	}

	/**
	 * Create a new fee transaction. {@link CashAccount} has been debited with some
	 * sort of fee.
	 *
	 * @param date    date the transaction took place
	 * @param amount  the amount withdrawn
	...
	public static Transaction newFee(TradingDate date, Money amount, CashAccount account) {
		return new Transaction(FEE, date, amount, null, 0, null, account, null, null);
	}
	 ...
	 */
	public static Transaction newAccumulate(TradingDate date, Money amount, Symbol symbol, int shares, Money tradeCost,
			CashAccount cashAccount, ShareAccount shareAccount) {
		return new Transaction(ACCUMULATE, date, amount, symbol, shares, tradeCost, cashAccount, null, shareAccount);
	}
	 ...
	 * @param cashAccount  cash account
	 */
	public static Transaction newReduce(TradingDate date, Money amount, Symbol symbol, int shares, Money tradeCost,
			CashAccount cashAccount, ShareAccount shareAccount) {
		return new Transaction(REDUCE, date, amount, symbol, shares, tradeCost, cashAccount, null, shareAccount);
	}
	 ...
	 * @param cashAccount  cash account
	 */
	public static Transaction newDividend(TradingDate date, Money amount, Symbol symbol, CashAccount cashAccount,
			ShareAccount shareAccount) {
		return new Transaction(DIVIDEND, date, amount, symbol, 0, null, cashAccount, null, shareAccount);
	}

	/**
	 * Create a new dividend transaction. A share has paid a dividend and the
	 * dividend has been re-invested back into the holding, increasing the holding
	 * by several shares.
	 *
	 * @param date         date the transaction took place
	 * @param symbol       the stock paying the dividend
	 ...
	 * @param shareAccount share account
	 */
	public static Transaction newDividendDRP(TradingDate date, Symbol symbol, int shares, ShareAccount shareAccount) {
		return new Transaction(DIVIDEND_DRP, date, null, symbol, shares, null, null, null, shareAccount);
	}

	/**
	 * Create a new transfer transaction. A transfer transaction is when cash has
	 * been transferred between two cash accounts.
	 *
	 * @param date         date the transaction took place
	 ...
	 */
	public static Transaction newTransfer(TradingDate date, Money amount, CashAccount cashAccount,
			CashAccount cashAccount2) {
		return new Transaction(TRANSFER, date, amount, null, 0, null, cashAccount, cashAccount2, null);
	}
	...
	public static String typeToString(int type) {
		String[] typeNames = { Locale.getString("WITHDRAWAL_TRANSACTION"), Locale.getString("DEPOSIT_TRANSACTION"),
				Locale.getString("INTEREST_TRANSACTION"), Locale.getString("FEE_TRANSACTION"),
				Locale.getString("ACCUMULATE_TRANSACTION"), Locale.getString("REDUCE_TRANSACTION"),
				Locale.getString("DIVIDEND_TRANSACTION"), Locale.getString("DIVIDEND_DRP_TRANSACTION"),
	...
	/**
	 * Compare this transaction to another one. Transactions are compared by date
	 * only.
	 *
	 * @param object other transaction object
	 * @return <code>-1</code> if this transaction is before; <code>0</code> if they
	 *         fall on the same date; <code>1</code> if this transaction is after
	 */
	 ...
	 * @return <code>true</code> if the transactions are equal; <code>false</code>
	 *         otherwise
	 */
	public boolean equals(Object object) {
		...
		if (!transaction.getDate().equals(getDate())
				|| transaction.getType() != getType() && transaction.getShares() != getShares())
			return false;
	...
	public Object clone() {
		Transaction clonedTransaction = new Transaction(getType(), getDate(), getAmount(), getSymbol(), getShares(),
				getTradeCost(), getCashAccount(), getCashAccount2(), getShareAccount());
	...
	public String toString() {
		...
		return new String(getDate().toString("dd/mm/yyyy") + "\t" + Transaction.typeToString(getType()) + "\t"
				+ getAmount() + "\t" + symbol + "\t" + getShares() + "\t" + getTradeCost() + "\t" + cashAccountName
				+ "\t" + cashAccountName2 + "\t" + shareAccountName);
	}
	 ...
	 *
	 * @return symbol or <code>null</code> if this transaction was not a share
	 *         transaction
	 */
	 ...
	 *
	 * @return number of shares or <code>0</code> if this transaction was not a
	 *         share transaction
	 */
	 ...
	 *
	 * @return cost of trade or <code>0</code> if this transaction was not a share
	 *         transaction
	 */
	 ...
	 *
	 * @return cash account or <code>null</code> if no cash account associated with
	 *         this transaction
	 * @see CashAccount
	 */
	 ...
	 *
	 * @return second cash account or <code>null</code> if only zero or one cash
	 *         accounts are associated with this transaction
	 * @see CashAccount
	 */
	 ...
	 *
	 * @return share account or <code>null</code> if no cash account associated with
	 *         this transaction
	 * @see ShareAccount
	 */

...
public class ShareAccount extends AbstractAccount implements Cloneable {
	 ...
	 * @return <code>true</code> if the share accounts are equal; <code>false</code>
	 *         otherwise
	 */
	...
	public void transaction(Transaction transaction) {
		...
		if (type == Transaction.ACCUMULATE) {
			...
			double averageCost = ((transaction.getAmount().doubleValue() + transaction.getTradeCost().doubleValue())
					/ transaction.getShares());
			...
			if (holding != null)
				...
				stockHoldings.put(symbol, new StockHolding(symbol, shares, averageCost, transaction.getDate()));
			...
			if (holding != null)
				...
				stockHoldings.put(symbol, new StockHolding(symbol, shares, 0.0D, transaction.getDate()));
	 ...
	 * @return stock holding for the symbol or <code>null</code> if we do not own
	 *         any
	 */
	 ...
	 *
	 * @param symbol the stock symbol
	 * @return <code>TRUE</code> if we are holding the symbol; <code>FALSE</code>
	 *         otherwise
	 */

...
public class ProxyPage extends JPanel implements IPreferencesPage {
	...
	private JPanel createProxyPanel() {
		...
		useProxyCheckBox = GridBagHelper.addCheckBoxRow(borderPanel, Locale.getString("USE_PROXY"),
		...
		hostTextField = GridBagHelper.addTextRow(borderPanel, Locale.getString("PROXY_HOST"), proxyPreferences.host,
				gridbag, c, 20);

		hostTextField.setToolTipText(Locale.getString("PROXY_HOST_FIELD_TOOLTIP"));

		portTextField = GridBagHelper.addTextRow(borderPanel, Locale.getString("PROXY_PORT"), proxyPreferences.port,
				gridbag, c, 5);

		portTextField.setToolTipText(Locale.getString("PROXY_PORT_FIELD_TOOLTIP"));

		useAuthCheckBox = GridBagHelper.addCheckBoxRow(borderPanel, Locale.getString("PROXY_AUTH"),
		...
		userTextField = GridBagHelper.addTextRow(borderPanel, Locale.getString("PROXY_USER"), proxyPreferences.user,
				gridbag, c, 10);

		userTextField.setToolTipText(Locale.getString("PROXY_AUTH_USER_FIELD_TOOLTIP"));

		passwordTextField = GridBagHelper.addPasswordRow(borderPanel, Locale.getString("PROXY_PASSWORD"),
				proxyPreferences.password, gridbag, c, 10);
	...
	/**
	 * Setup the networking to handle authentication requests and work http proxies
	 * correctly
	 */
		...
		if (proxyPreferences.isEnabled) {
			...
			if (proxyPreferences.authEnabled) {
				...
				java.net.Authenticator
						.setDefault(new ProxyAuthenticator(proxyPreferences.user, proxyPreferences.password));
			}
		}
	}
}

...
public class MacrosPage extends JPanel implements IPreferencesPage {
	...
	/**
	 * Used for building the table combo box for determining how many entries to add
	 * to the combo box
	 */
	...
	final String[] names = { Locale.getString("FILENAME_COLUMN_HEADER"), Locale.getString("MACRO_NAME_COLUMN_HEADER"),
			Locale.getString("STARTUP_COLUMN_HEADER"), Locale.getString("INMENU_COLUMN_HEADER") };
	...
	private JTable getMacros_table() {
		if (macros_table == null) {
					...
					case STARTUP_COLUMN:
						...
						if (s.equals(Locale.getString("NO_START"))) {
							for (int i = 0; i < stored_macros.size(); i++) {
								if (i != row) {
									StoredMacro t = (StoredMacro) stored_macros.get(i);
									int other_old_seq = t.getStart_sequence();
									if (other_old_seq > 1 && other_old_seq > old_seq)
							...
							for (int i = 0; i < stored_macros.size(); i++) {
								if (i != row) {
									...
									if (t.isOn_startup() && other_old_seq >= new_seq
											&& (other_old_seq < old_seq || !m.isOn_startup())) {
										t.setStart_sequence(other_old_seq + 1);

										// Case 2: Moving from low to high
									} else if (t.isOn_startup() && other_old_seq > 1 && other_old_seq > old_seq
			...
			TableColumn mc = macros_table.getColumn(Locale.getString("INMENU_COLUMN_HEADER"));
			int width = (int) new javax.swing.JCheckBox().getPreferredSize().getWidth();
			mc.setWidth(width);
	...
	private void setComboEditor() {
		...
		TableColumn col = macros_table.getColumn(Locale.getString("STARTUP_COLUMN_HEADER"));
		...
		up_button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (row > 0) {
					...
					macros_table.changeSelection(row - 1, macros_table.getSelectedColumn(), false, false);
				}
			}
		});
		...
		if (delete_button == null) {
			...
			delete_button.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					StoredMacro macro = (StoredMacro) stored_macros.get(selected_row);
					String filepath = PreferencesManager.getMacroHome() + File.separator + macro.getFilename();
					int response = JOptionPane.showInternalConfirmDialog(delete_button,
							Locale.getString("SURE_DELETE_MACRO", macro.getName(), filepath),
							Locale.getString("DELETE_MACRO_TITLE"), JOptionPane.YES_NO_OPTION);
					if (response == JOptionPane.YES_OPTION) {
						if (macro.delete()) {
							...
							JOptionPane.showInternalMessageDialog(delete_button,
									Locale.getString("UNABLE_TO_DELETE_NAME_ERROR", filepath),
									Locale.getString("WARNINGTITLE"), JOptionPane.ERROR_MESSAGE);
						}
					}
				}
			});

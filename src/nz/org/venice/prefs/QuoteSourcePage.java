...
/**
 * Provides a preferences page to let the user modify the quote source. The
 * quote source can be from a database (internal or external) or sample quotes.
 *
...
public class QuoteSourcePage extends JPanel implements PreferencesPage {
	...
	public QuoteSourcePage(JDesktopPane desktop) {
		...
		pane.addTab(Locale.getString("SAMPLES"), createSamplesPanel(quoteSource, buttonGroup));
		pane.addTab(Locale.getString("INTERNAL"), createInternalPanel(quoteSource, buttonGroup));
		pane.addTab(Locale.getString("DATABASE"), createDatabasePanel(quoteSource, buttonGroup));
	...
	private JPanel createDatabasePanel(int quoteSource, ButtonGroup buttonGroup) {
		...
		// Host
		databaseHost = GridBagHelper.addTextRow(borderPanel, Locale.getString("HOST"), databasePreferences.host,
				gridbag, c, 15);
		...
		// Port
		databasePort = GridBagHelper.addTextRow(borderPanel, Locale.getString("PORT"), databasePreferences.port,
				gridbag, c, 15);
		...
		// Username
		databaseUsername = GridBagHelper.addTextRow(borderPanel, Locale.getString("USERNAME"),
		...
		databasePassword = GridBagHelper.addPasswordRow(borderPanel, Locale.getString("PASSWORD"), passwdString,
				gridbag, c, 15);
		...
		// Database Name
		databaseName = GridBagHelper.addTextRow(borderPanel, Locale.getString("DATABASE_NAME"),
		...
		databasePasswordPrompt = GridBagHelper.addCheckBoxRow(borderPanel, Locale.getString("PROMPT_FOR_PASSWORD"),
				databasePreferences.passwordPrompt, gridbag, c);
	...
	/**
	 * Sets the databaseDriver dropdown to contain the list of valid drivers for the
	 * currently selected software
	 *
	 */
	...
	public void save() {
		...
		if (useDatabase.isSelected()) {
			...
			databasePreferences.password = (databasePassword.getPassword().length > 0)
			...
			if (sourceChanged) {
				int flushCache = JOptionPane.showConfirmDialog(this, Locale.getString("QUOTE_SOURCE_CHANGED_QUESTION"),

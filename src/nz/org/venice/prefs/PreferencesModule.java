...
/**
 * The preferences module for venice. This class provides the user interface to
 * change any of the preferences settings. Preferences are organised as a set of
 * pages, each page is responsible for one group of settings. Example:
 * 
 * <pre>
...
public class PreferencesModule extends JPanel implements IModule, ActionListener {
	...
	public PreferencesModule(JDesktopPane desktop, int page) {
		...
		add(split = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, pageList, activePage.getComponent()),
				BorderLayout.CENTER);
	...
	/**
	 * Overrides the default method. Determines the preferred size of all page
	 * components, rather than just the one to be displayed first
	 */

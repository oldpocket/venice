...
public class XMLHelper {
	 ...
	 * @param xpath A string representing a simplified xpath expression It handles
	 *              neither axes nor predicates. If the last element in the path is
	 *              not a leaf node, it returns the first element in the list.
	 *
	 * 
	 */
	 ...
	 * 
	 *                   Note that it is not a precondition of this method that e be
	 *                   the head of the list.
	 */

	public static Vector readList(Element e, String listName, String typeString, String entryName) {
		...
		if (e.getTagName().equals(listName) && getAttribute(e, "type").equals(typeString)) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (getAttribute((Element) list.get(i), "type").equals(typeString)) {
					...
					Vector tmp = processList((Element) list.get(i), listName, entryName);
					if (tmp != null) {
	...
	/**
	 *
	 * Return a List of Nodes which are direct descendents of the node n. This
	 * method exists because the standard Java method getElementsByTagName does a
	 * n/descendant::tagName rather than than n/tagName.
	 */
	...
	/**
	 *
	 * Convert a list whose elements consist of strings representing symbols to
	 * elements representing Venice Symbols
	 * 

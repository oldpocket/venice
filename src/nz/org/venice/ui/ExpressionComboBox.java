...
/**
 * Extension of JComboBox used for displaying an editable expression field in
 * the venice application. This ComboBox allows the user to select expressions
 * entered in the Functions preferences page. This way the user does not have to
 * keep typing in the same expressions multiple times.
 *
 * <pre>
...
public class ExpressionComboBox extends JComboBox implements PopupMenuListener {
	...
	/**
	 * Return the expression string in the ComboBox. If a name of an expression is
	 * in the box then its expression will be returned.
	 *
	...
	/**
	 * Return whether the current displayed expression is a stored expression. A
	 * stored expression is one the user has entered and can refer to by using a
	 * keyword.
	 *
	...
	/**
	 * Set the expression string in the ComboBox. If the given expression has a
	 * name, then the name will be displayed in the comboBox instead of the
	 * expression.
	 *
	 * @param expressionText expression text to display
	 */
	...
	/**
	 * Tell all expression ComboBoxes that the stored expressions have been modified
	 * by the user and that their popup menus need to be changed.
	 */
		...
		Thread thread = new Thread(new Runnable() {
			public void run() {
				if (!isDialogUp) {
					...
					StoredExpression storedExpression = ExpressionEditorDialog.showAddDialog(storedExpressions,
							Locale.getString("ADD_EQUATION"), getText());
		...
		if (!isDialogUp) {
			...
			if (storedExpression != null) {
				int option = JOptionPane.showInternalConfirmDialog(DesktopManager.getDesktop(),
						Locale.getString("SURE_DELETE_EQUATION", getText()), Locale.getString("DELETE_EQUATION"),
			...
			public void run() {
				if (!isDialogUp) {
					...
					if (isStoredExpression()) {
						...
						StoredExpression storedExpression = findStoredExpressionByName(getText());
						if (storedExpression != null) {
							storedExpression = ExpressionEditorDialog.showEditDialog(storedExpressions,
									Locale.getString("EDIT_EQUATION"), storedExpression);
						...
						String newExpressionText = ExpressionEditorDialog
								.showEditDialog(Locale.getString("EDIT_EQUATION"), expressionText);

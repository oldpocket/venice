...
/**
 * Helper for constructing tables. The abstract table sets up the look & feel
 * for tables in Venice. It also provides column sorting and helper methods for
 * expression columns.
 *
...
public class AbstractTable extends SortedTable {
	...
	// Images used for arrows representing when stock has gone up, down or is
	// unchanged
	...
	private class PatternRenderer extends JPanel implements TableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
			...
			if (isSelected)
				...
				component.setBackground(row % 2 != 0 ? backgroundColor : alternativeBackgroundColor);
			return component;
		}
	}
	...
	private class ChangeFormatRenderer extends JPanel implements TableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
			...
			if (isSelected) {
				...
				setBackground(row % 2 != 0 ? backgroundColor : alternativeBackgroundColor);
			}
	...
	public AbstractTable() {
		...
		unchangedImageIcon = (unchangedImageResource != null ? new ImageIcon(unchangedImageResource) : null);

		// Set up a cell renderer to render each alternative row a different colur.
		// JTable sets up specific renderers for these objects. So this is the minimum
		// number
		// of renderers we need to define to override the defaults.
		setDefaultRenderer(Object.class, new PatternRenderer(getDefaultRenderer(Object.class)));
		setDefaultRenderer(Number.class, new PatternRenderer(getDefaultRenderer(Number.class)));
		setDefaultRenderer(Boolean.class, new PatternRenderer(getDefaultRenderer(Boolean.class)));
	...
	/**
	 * Helper method to create and return the "show columns" menu which allows the
	 * user to select which columns are visible.
	 *
	 * @param model Table model.
	 */
	...
	/**
	 * Helper method to display a dialog to let the user set expressions. The user
	 * can input expressions which are compiled and run against all the quotes in
	 * the table. The results are shown in columns which are made visible.
	 *
	...
	protected void applyExpressions(final AbstractQuoteModel model) {
		...
		Thread thread = new Thread() {
			...
			public void run() {
				final ExpressionColumnDialog dialog = new ExpressionColumnDialog(model.getExpressionColumns().length);
					...
					SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							...
							for (int i = 0; i < expressionColumns.length; i++) {
								boolean containsExpression = expressionColumns[i].getExpressionText().length() > 0;
								JCheckBoxMenuItem menuItem = (JCheckBoxMenuItem) showExpressionColumnMenuItems.get(i);

								showColumn(expressionColumns[i].getNumber(), containsExpression);

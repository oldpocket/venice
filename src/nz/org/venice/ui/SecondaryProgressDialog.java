...
public class SecondaryProgressDialog implements ProgressDialog {
	...
	/**
	 * Create a new Secondary Progress Dialog. This will actually create a proxy to
	 * the {@link PrimaryProgressDialog}. The purpose of this proxy is that it
	 * contains the logic to test whether or not it has the authority to change the
	 * real progress dialog. The Secondary Progress Dialog is returned when the
	 * programme calls {@link ProgressDialogManager#getProgressDialog} and there is
	 * already a progress dialog up.
	 * <p>
	...
	/**
	 * Set the note or rather the action we are curently waiting for. E.g. the task
	 * might be "Graph CBA", but the action we are waiting for would be "Load CBA
	 * Quotes".
	 *
	...
	private boolean hasControl() {
		return (!primaryProgressDialog.isMaster() || isMaster());

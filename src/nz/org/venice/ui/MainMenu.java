...
public class MainMenu implements ActionListener, IModuleListener {
	 ...
	 */
	public static synchronized MainMenu getInstance(JFrame frame, nz.org.venice.ui.DesktopManager desktopManager) {
		if (instance == null)
	...
	private MainMenu(JFrame frame, nz.org.venice.ui.DesktopManager desktopManager) {
		this.frame = frame;
		...
		fileImportPreferencesMenuItem = MenuHelper.addMenuItem(this, fileImportMenu, Locale.getString("PREFERENCES"));
		// File -> Import -> End of Day Quotes
		fileImportQuotesMenuItem = MenuHelper.addMenuItem(this, fileImportMenu, Locale.getString("END_OF_DAY_QUOTES"));

		// File -> Sync
		fileSyncMenu = MenuHelper.addMenu(fileMenu, Locale.getString("SYNC"));
		// File -> Import -> Sync Intra-day Quotes
		fileSyncIDQuotesMenuItem = MenuHelper.addMenuItem(this, fileSyncMenu, Locale.getString("INTRA_DAY_QUOTES"));
		...
		fileExportPreferencesMenuItem = MenuHelper.addMenuItem(this, fileExportMenu, Locale.getString("PREFERENCES"));
		// File -> Export -> End of Day Quotes
		fileExportQuotesMenuItem = MenuHelper.addMenuItem(this, fileExportMenu, Locale.getString("END_OF_DAY_QUOTES"));
		// File -> Preferences
		filePreferencesMenuItem = MenuHelper.addMenuItem(this, fileMenu, Locale.getString("PREFERENCES"), 'R');
		...
		// Table -> Alerts
		quoteAlertMenu = MenuHelper.addMenu(quoteMenu, Locale.getString("ALERT_TITLE"), 'A');

		// Table -> Alerts -> List All
		quoteAlertListAllMenuItem = MenuHelper.addMenuItem(this, quoteAlertMenu, Locale.getString("LIST_ALL"));

		// Table -> Alerts -> List by Symbol
		quoteAlertListSymbolsMenuItem = MenuHelper.addMenuItem(this, quoteAlertMenu,
				Locale.getString("LIST_BY_SYMBOLS"));

		quoteAlertRefreshMenuItem = MenuHelper.addMenuItem(this, quoteAlertMenu,
		...
		quoteCompanyListAllMenuItem = MenuHelper.addMenuItem(this, quoteMenuCompany, Locale.getString("LIST_ALL"));

		// Table -> Companies + Funds -> List by rule
		quoteCompanyListRuleMenuItem = MenuHelper.addMenuItem(this, quoteMenuCompany, Locale.getString("LIST_BY_RULE"));

		// Table -> Companies + Funds -> List by date
		quoteCompanyListDateMenuItem = MenuHelper.addMenuItem(this, quoteMenuCompany, Locale.getString("LIST_BY_DATE"));

		// Table -> Indices
		JMenu quoteMenuIndices = MenuHelper.addMenu(quoteMenu, Locale.getString("MARKET_INDICES"), 'I');

		// Table -> Indices -> List All
		quoteIndicesListAllMenuItem = MenuHelper.addMenuItem(this, quoteMenuIndices, Locale.getString("LIST_ALL"));

		// Table -> Indices -> List by Rule
		quoteIndicesListRuleMenuItem = MenuHelper.addMenuItem(this, quoteMenuIndices, Locale.getString("LIST_BY_RULE"));
		...
		quoteCommoditiesListAllMenuItem.setAccelerator(KeyStroke.getKeyStroke('L', InputEvent.CTRL_MASK));

		// Table -> All Stocks -> List by Rule
		quoteCommoditiesListRuleMenuItem = MenuHelper.addMenuItem(this, quoteMenuCommodities,
				Locale.getString("LIST_BY_RULE"), 'U');
		quoteCommoditiesListRuleMenuItem.setAccelerator(KeyStroke.getKeyStroke('U', InputEvent.CTRL_MASK));

		// Table -> All Stocks -> List by Date
		quoteCommoditiesListDateMenuItem = MenuHelper.addMenuItem(this, quoteMenuCommodities,
				Locale.getString("LIST_BY_DATE"), 'D');
		quoteCommoditiesListDateMenuItem.setAccelerator(KeyStroke.getKeyStroke('D', InputEvent.CTRL_MASK));

		// Table -> Stocks -> List by Symbols
		JMenu quoteMenuStocks = MenuHelper.addMenu(quoteMenu, Locale.getString("STOCKS"), 'S');

		quoteStocksListSymbolsMenuItem = MenuHelper.addMenuItem(this, quoteMenuStocks,
				Locale.getString("LIST_BY_SYMBOLS"), 'B');
		quoteStocksListSymbolsMenuItem.setAccelerator(KeyStroke.getKeyStroke('B', InputEvent.CTRL_MASK));
		...
		graphCommodityCodeMenuItem = MenuHelper.addMenuItem(this, graphCommodityMenu,
				Locale.getString("GRAPH_BY_SYMBOLS"), 'G');
		graphCommodityCodeMenuItem.setAccelerator(KeyStroke.getKeyStroke('G', InputEvent.CTRL_MASK));
		...
		graphMarketAdvanceDeclineMenuItem = MenuHelper.addMenuItem(this, graphMarketIndicator,
		 ...
		 * Analysis
		 **********************************************/
		JMenu analysisMenu = MenuHelper.addMenu(menuBar, Locale.getString("ANALYSIS"), 'A');

		analysisPaperTradeMenuItem = MenuHelper.addMenuItem(this, analysisMenu, Locale.getString("PAPER_TRADE"));

		analysisGPMenuItem = MenuHelper.addMenuItem(this, analysisMenu, Locale.getString("GP"));

		analysisGAMenuItem = MenuHelper.addMenuItem(this, analysisMenu, Locale.getString("GA"));

		analysisANNMenuItem = MenuHelper.addMenuItem(this, analysisMenu, Locale.getString("ANN"));

		/**********************************************
		 * Window
		 **********************************************/
		windowMenu = MenuHelper.addMenu(menuBar, Locale.getString("WINDOW"), 'W');
		windowTileHorizontalMenuItem = MenuHelper.addMenuItem(this, windowMenu, Locale.getString("TILE_HORIZONTALLY"));
		windowTileHorizontalMenuItem.setEnabled(false);
		windowTileVerticalMenuItem = MenuHelper.addMenuItem(this, windowMenu, Locale.getString("TILE_VERTICALLY"));
		windowTileVerticalMenuItem.setEnabled(false);
		windowCascadeMenuItem = MenuHelper.addMenuItem(this, windowMenu, Locale.getString("CASCADE"));
		windowCascadeMenuItem.setEnabled(false);
		windowGridMenuItem = MenuHelper.addMenuItem(this, windowMenu, Locale.getString("ARRANGE_ALL"));
		...
		helpAboutMenuItem = MenuHelper.addMenuItem(this, helpMenu, Locale.getString("ABOUT"));
		helpContentsMenuItem = MenuHelper.addMenuItem(this, helpMenu, Locale.getString("CONTENTS"));
		helpContentsMenuItem.setAccelerator(KeyStroke.getKeyStroke('H', InputEvent.CTRL_MASK));
		helpViewLicenseMenuItem = MenuHelper.addMenuItem(this, helpMenu, Locale.getString("VIEW_LICENSE"));
		...
		Thread menuAction = new Thread() {
			...
			public void run() {
				...
				if ((mo = menuItemToMacroHash.get(menu)) != null) {
					...
					try {
						...
						JOptionPane.showInternalMessageDialog(desktop,
								Locale.getString("NO_JYTHON_ERROR", e.getMessage()),
								Locale.getString("MACRO_EXECUTION_ERROR"), JOptionPane.ERROR_MESSAGE);
					}
				...
				// Maybe it's a portfolio?
					...
					String portfolioName = (String) portfolioHash.get(menu);
					...
					if (PreferencesManager.getConfirmExitSetting()) {
						int choice = JOptionPane.showConfirmDialog(desktop, Locale.getString("CONFIRM_VENICE_EXIT"),
				...
				// Maybe it's a watch screen
					...
					String watchScreenName = (String) watchScreenHash.get(menu);
				...
				// Maybe its a portfolio?
					...
					String portfolioName = (String) portfolioTableHash.get(menu);
				...
				// Maybe its a portfolio?
					...
					String portfolioName = (String) portfolioGraphHash.get(menu);
	...
	/**
	 * Inform menu that the list of portfolios has changed and that its menus should
	 * be redrawn
	 */
	public void updatePortfolioMenu() {
		...
		// portfolio
		filePortfolioNewMenuItem = MenuHelper.addMenuItem(this, filePortfolioMenu, Locale.getString("NEW_PORTFOLIO"));
	...
	/**
	 * Inform menu that the list of watch screens has changed and that its menus
	 * should be redrawn
	 */
	public void updateWatchScreenMenu() {
		...
		quoteWatchScreenNewMenuItem = MenuHelper.addMenuItem(this, quoteWatchScreenMenu,
	...
	/**
	 * Take the list of menu item and add them to the given menu. Return a hashmap
	 * which maps each menu created with the given menu item.
	 */
	...
	public void buildMacroMenu() {
		...
		macroManageMenuItem = MenuHelper.addMenuItem(this, macroMenu, Locale.getString("MANAGE"), 'M');
		macroManageMenuItem.setAccelerator(KeyStroke.getKeyStroke('M', InputEvent.CTRL_MASK));

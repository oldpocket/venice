...
public class TradingDateDialog {
	...
	/**
	 * Opens a new dialog asking the user to enter a date. If the user enters an
	 * illegal date, a date that falls on a weekend or any date that we don't have
	 * data for, then the date will be rejected and the user will be asked to
	 * re-enter the date. The date will be returned or <code>null</code> if the user
	...
	private static TradingDate parseDate(JDesktopPane parent, String dateText) {
		...
		try {
			...
			JOptionPane.showInternalMessageDialog(parent, Locale.getString("ERROR_PARSING_DATE", dateText),
		...
		try {
			...
			JOptionPane.showInternalMessageDialog(parent, Locale.getString("DATE_ON_WEEKEND", dateText),
					Locale.getString("INVALID_DATE"), JOptionPane.ERROR_MESSAGE);
			return null;
		}
		...
		if (!QuoteSourceManager.getSource().containsDate(date)) {
			JOptionPane.showInternalMessageDialog(parent, Locale.getString("NO_QUOTES_DATE", dateText),
					Locale.getString("INVALID_DATE"), JOptionPane.ERROR_MESSAGE);
			return null;
		}

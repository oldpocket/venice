...
public class TextsEditorDialog {
	...
	private TextsEditorDialog(String title, String[] labels, boolean[] areas, String[] texts, int buttonArray,
			boolean isEditable) {
		this.texts = texts;
		this.areas = areas;
		assert buttonArray == OK_BUTTON || buttonArray == OK_CANCEL_BUTTON;

		buildDialog(title, labels, buttonArray, isEditable);
	}

	private void buildDialog(String title, String[] labels, int buttonArray, boolean isEditable) {
	...
	public static String[] showAddDialog(String title, String[] labels, boolean[] areas) {
		...
		while (!isValid) {
			TextsEditorDialog dialog = new TextsEditorDialog(title, labels, areas, texts, OK_CANCEL_BUTTON, true);

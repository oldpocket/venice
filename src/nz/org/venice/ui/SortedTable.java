...
public class SortedTable extends JTable {
	...
	private class HeaderCellRenderer extends JLabel implements TableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
			...
			if (sortedTable.getColumnSortStatus(column) == DONT_SORT)
				...
				URL upImageURL = ClassLoader.getSystemResource(upImage);
				...
				URL downImageURL = ClassLoader.getSystemResource(downImage);
	...
	public class SortModel extends AbstractTableModel implements TableModelListener {
		...
		private class SortComparator implements Comparator {
			...
			private int applySortDirection(int comparision) {
				...
				if (sortDirection == SORT_UP) {
					...
					/*
					 * if(comparision > 0) comparision = -1; else if(comparision < 0) comparision =
					 * 1;
					 */
				}
		...
		public void sort(int sortColumn, int sortDirection) {
			...
			for (int i = 0; i < getRowCount(); i++) {
				tableElement = new TableElement(userModel.getValueAt(i, sortColumn), i);
		...
		public boolean isCellEditable(int row, int column) {
			return userModel.isCellEditable(getUnsortedRow(row), convertToDisplayedColumn(column));
		}
	...
	public Component getTableCellRendererComponent(TableCellRenderer tableCellRenderer, JTable table, Object value,
			boolean isSelected, boolean hasFocus, int row, int column) {
		Component c;
		try {
			c = tableCellRenderer.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
	...
	private void setCustomHeaderRenderer() {
		...
		JComponent header = (JComponent) this.getTableCellRendererComponent(r, (JTable) this, (Object) "", false, false,
				0, 0);

		HeaderCellRenderer renderer = new HeaderCellRenderer(header.getBorder(), header.getBackground(),
	...
	public int[] getSelectedRows() {
		...
		if (getSelectedRowCount() > 0) {
			for (int i = 0; i < getSelectedRowCount(); i++)
				selectedRows[i] = model.getUnsortedRow(unsortedSelectedRows[i]);
		}

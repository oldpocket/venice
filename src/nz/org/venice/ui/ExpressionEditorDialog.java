...
/**
 * A dialog for adding, editing and viewing stored expressions. A stored
 * expression is an expression with a name that is saved in user preferences.
 * Stored expressions can be quickly recalled (and modified) anywhere Venice
 * accepts expressions.
 *
...
public class ExpressionEditorDialog {
	...
	private ExpressionEditorDialog(String title, boolean displayName, String name, String expression, int buttonArray,
			boolean isEditable) {
		this.name = name;
	...
	private void buildDialog(String title, final boolean displayName, int buttonArray, boolean isEditable) {
		internalFrame = new JInternalFrame(title, true, /* resizable */
				false, /* closable */
				false, /* maximisible */
				false); /* iconifiable */
		...
		final JTextArea expressionEditor = new JTextArea(EXPRESSION_ROWS, EXPRESSION_COLUMNS);
		...
		ModuleFrame.setSizeAndLocation(internalFrame, DesktopManager.getDesktop(), true, true);
	...
	/**
	 * Show a dialog which allows the user to add a new stored expression. Make sure
	 * you run this in its own thread - not in the swing dispatch thread!
	 *
	 * @param storedExpressions current stored expressions.
	 * @param title             title of dialog.
	 * @param expression        initial expression text
	 * @return New stored expression or <code>null</code> if the user cancelled the
	 *         dialog.
	 */
	public static StoredExpression showAddDialog(List storedExpressions, String title, String expression) {
		boolean isValid = false;
		String name = "";
		StoredExpression storedExpression = null;

		while (!isValid) {
			ExpressionEditorDialog dialog = new ExpressionEditorDialog(title, true, name, expression, OK_CANCEL_BUTTON,
					true);
	...
	/**
	 * Show a dialog which allows the user to edit a current stored expression. Make
	 * sure you run this in its own thread - not in the swing dispatch thread!
	 *
	 * @param title      title of dialog.
	...
	public static String showEditDialog(String title, String expression) {
		ExpressionEditorDialog dialog = new ExpressionEditorDialog(title, false, "", expression, OK_CANCEL_BUTTON,
				true);
	...
	/**
	 * Show a dialog which allows the user to view a current stored expression. Make
	 * sure you run this in its own thread - not in the swing dispatch thread!
	 *
	 * @param title      title of dialog.
	...
	public static void showViewDialog(String title, String expression) {
		ExpressionEditorDialog dialog = new ExpressionEditorDialog(title, false, "", expression, OK_BUTTON, false);
		dialog.waitUntilClosed();
	}
	...
	private static boolean validateStoredExpression(List storedExpressions, String oldName, String newName,
		...
		if (newName.length() == 0) {
			JOptionPane.showInternalMessageDialog(DesktopManager.getDesktop(),
					Locale.getString("MISSING_EQUATION_NAME"), Locale.getString("ERROR_STORING_EQUATION"),
		...
		// existing stored expression's name.
			...
			if (isDuplicateName) {
				JOptionPane.showInternalMessageDialog(DesktopManager.getDesktop(),
						Locale.getString("DUPLICATE_EQUATION_NAME", newName),
						Locale.getString("ERROR_STORING_EQUATION"), JOptionPane.ERROR_MESSAGE);
				isValid = false;
			}
		} else if (expressionText.length() > java.util.prefs.Preferences.MAX_VALUE_LENGTH) {
			JOptionPane.showInternalMessageDialog(DesktopManager.getDesktop(),
					Locale.getString("SYMBOL_TOO_LONG", newName), Locale.getString("ERROR_STORING_EQUATION"),
					JOptionPane.ERROR_MESSAGE);
			isValid = false;
		}

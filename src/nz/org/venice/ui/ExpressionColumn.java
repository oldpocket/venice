 ...
 * column in quote tables that displays the results of a user expression applied
 * to the data in the table. The data type of the data displayed in the column
 * will be {@link ExpressionResult}.
 *
...
public class ExpressionColumn extends Column implements Cloneable {
	 ...
	 * @param shortName      The short name of the column which appears in the table
	 *                       header.
	...
	/**
	 * Execute the expression and calculate the result for each quote. This function
	 * takes a list of quotes, rather than extracting them from the quote bundle,
	 * because typically the table (and therefore this column) does not display all
	 * the quotes in the quote bundle. The reason is that to display a single day's
	 * quotes requires the loading of two day's worth of quotes. Two days are needed
		...
		if (expression != null) {
			for (Iterator iterator = quotes.iterator(); iterator.hasNext();) {
				...
				try {
					...
					double result = expression.evaluate(new Variables(), quoteBundle, quote.getSymbol(), offset);
	 ...
	 */
	public Object clone() {
		return new ExpressionColumn(getNumber(), getFullName(), getShortName(), getVisible(), getExpressionText(),
				getExpression());
	}
}

...
public class AdjustPriceDataDialog extends JInternalFrame implements ActionListener {
	...
	/**
	 * Create a new price adjustment dialog, and reading the type, value and start
	 * point from the user.
	 *
	...
	private JPanel getMainPanel() {
		...
		adjustmentTypeComboBox = GridBagHelper.addComboBox(mainPanel, Locale.getString("ADJUST_TYPE"), adjustmentTypes,
				gridbag, c);
		...
		adjustmentDirectionComboBox = GridBagHelper.addComboBox(mainPanel, Locale.getString("ADJUST_DIRECTION"),
				adjustmentDirections, gridbag, c);
		...
		dateField = GridBagHelper.addTextRow(mainPanel, Locale.getString("DATE"), "", gridbag, c, 15);

		dateField.setToolTipText(Locale.getString("DATE_FIELD_TOOLTIP"));

		valueField = GridBagHelper.addTextRow(mainPanel, valueLabel, "", gridbag, c, 15);

		valueField.setToolTipText(Locale.getString("VALUE_TOOLTIP"));

		helpButton = GridBagHelper.addHelpButtonRow(mainPanel, Locale.getString("SPLIT_DIV_ADJUST"), gridbag, c);
	...
	/**
	 * React to button presses and update labels according to the adjustment type
	 * selected.
	 * 
	...
	/*
	 * For splits, the user enters a ratio of how many new shares were created e.g.
	 * Two for one, for which the user would enter '2'. For a reverse split one for
	 * two, the user enters '0.5'. For that reason, zero is not permitted.
	 */
	...
	private boolean validateInput() {
		...
		try {
			...
			if (value.doubleValue() == 0.0) {
				JOptionPane.showInternalMessageDialog(desktop, Locale.getString("DIVIDE_BBY_ZERO_ERROR"),
						Locale.getString("INVALID_NUMBER_TITLE"), JOptionPane.ERROR_MESSAGE);
				inputOk = false;
			}
		} catch (NumberFormatException e) {
			inputOk = false;
			JOptionPane.showInternalMessageDialog(desktop, Locale.getString("ERROR_PARSING_NUMBER", e.getMessage()),
					Locale.getString("INVALID_NUMBER_TITLE"), JOptionPane.ERROR_MESSAGE);
		}

		try {
			TradingDate date = new TradingDate(dateField.getText(), TradingDate.BRITISH);

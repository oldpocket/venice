 ...
 * progress dialog running at anyone time. If, in a thread, a request for
 * another progress dialog is called, it will use the current progress dialog
 * being displayed for that thread. By using a proxy progress dialog class
 * ({@link SecondaryProgressDialog}), we can control how much the second comer
 * can change the current progress, without the caller even knowing.
 * <p>
 ...
 * <b>Master Dialogs</b>
 * <p>
 * Sometimes when you are displaying progress you might be performing several
 * tasks, where some of the tasks display their own progress. In this situation
 * it might be undesirable for the contained tasks to show their progress. You
 * can stop them by making your progress dialog a master dialog.
 * 
 * <pre>
 * progress.setMaster(true);
 * </pre>
 * 
 * This will stop their progress being displayed. The only thing they will have
 * control over is the note field.
 * <p>
 ...
 * is useful for initialisation tasks that can be very slow and should always be
 * shown to the user.
 * <p>
 *
 * @see IProgressDialog
...
public class ProgressDialogManager {
	...
	/**
	 * Create a new {@link PrimaryProgressDialog} if one is not already being
	 * displayed for the current thread. If one is displayed, return a proxy to the
	 * current dialog, i.e. a {@link SecondaryProgressDialog}. The secondary
	 * progress dialog may or may not have full control over the real dialog.
	 *
	...
	public static IProgressDialog getProgressDialog(boolean isCancelButtonToBePainted) {
		...
		if (primaryProgressDialog == null) {
			primaryProgressDialog = new PrimaryProgressDialog(DesktopManager.getDesktop(), isCancelButtonToBePainted);

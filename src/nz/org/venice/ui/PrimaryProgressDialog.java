...
public class PrimaryProgressDialog implements ProgressDialog {
	...
	/**
	 * Create a new Progress Dialog. The dialog will not be displayed until
	 * {@link #show} is called. The Primary Progress Dialog is the actual dialog
	 * being displayed, it is returned by the first call to
	 * {@link ProgressDialogManager#getProgressDialog}.
	 *
	 ...
	 */
	public PrimaryProgressDialog(JDesktopPane parent, boolean isCancelButtonToBePainted) {
	...
	private void newDialog(JDesktopPane parent, boolean isCancelButtonToBePainted) {
		...
		JOptionPane optionPane = new JOptionPane(panel, JOptionPane.INFORMATION_MESSAGE, JOptionPane.OK_CANCEL_OPTION,
				null, options, null);
	 ...
	 *              which is not necessarily the action we are waiting for. e.g. the
	 *              task might be "Graph CBA", but the action we are waiting for
	 *              would be "Load CBA Quotes".
	 */
	...
	/**
	 * Get the note or rather the action we are curently waiting for. E.g. the task
	 * might be "Graph CBA", but the action we are waiting for would be "Load CBA
	 * Quotes".
	 *
	...
	/**
	 * Set the note to be the action we are curently waiting for. E.g. the task
	 * might be "Graph CBA", but the action we are waiting for would be "Load CBA
	 * Quotes".
	 *
	...
	public void setProgress(int progress) {
		...
		if (isIndeterminate()) {
			...
			if (newPercent != percent) {
				percent = newPercent;
				progressLabel.setText(Locale.getString("PROGRESS_PERCENT", Integer.toString(getProgress()),
						Integer.toString(getMaximum()), Integer.toString(newPercent)));
	...
	/**
	 * Return whether the dialog is indeterminate. An indeterminate progress does
	 * not given the user any indication of when the task might be completed.
	 *
	...
	/**
	 * Set whether the dialog is indeterminate. An indeterminate progress does not
	 * given the user any indication of when the task might be completed.
	 *

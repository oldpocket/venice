...
public class DesktopManager extends javax.swing.DefaultDesktopManager implements java.io.Serializable {
	...
	/**
	 * Show a simple warning message to the user. Unlike the show error message
	 * dialog, this will not cancel the current thread. Use this if the operation is
	...
	public static void showWarningMessage(final String message) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				JOptionPane.showInternalMessageDialog(desktop_instance, multiLineMessage,
						Locale.getString("VENICE_PROBLEM_TITLE"), JOptionPane.WARNING_MESSAGE);
	...
	/**
	 * Show a simple error message to the user. This will also cancel the current
	 * thread if the progress dialog is up.
	 *
	...
	public static void showErrorMessage(final String title, final String message) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				incrementMessagesDisplayed();
				JOptionPane.showInternalMessageDialog(desktop_instance, multiLineMessage, title,
	 ...
	 *
	 * @return <code>true</code> if we are displaying a message, <code>false</code>
	 *         otherwise.
	 */
	...
	/**
	 * Display a new frame upon the current desktop. Frame will be displayed at
	 * (0,0) and not centred.
	 *
	...
	/**
	 * Call save() on every open module. This will save all the modules' preferences
	 * data.
	 */
	public void save() {
		// Get all frames open

...
/**
 * The export module allows exporting of quotes from Venice. It provides an
 * interface to allow the user to perform an export of all the quotes in
 * Venice's database. The actual exporting is handled by other classes.
 *
...
public class ExportQuoteModule extends JPanel implements IModule {
	...
	private void buildGUI() {
		...
		// Export to files
		{
			...
			fileNamesTextField = GridBagHelper.addTextRow(titledPanel, Locale.getString("FILE_NAMES"), fileNames,
					gridbag, c, 15);
	...
	private void exportQuotesToFiles() {
		...
		JOptionPane.showInternalMessageDialog(desktop, Locale.getString("EXPORT_COMPLETE"),
				Locale.getString("EXPORT_COMPLETE_TITLE"), JOptionPane.INFORMATION_MESSAGE);
	}

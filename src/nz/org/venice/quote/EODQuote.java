...
/**
 * Representation of an end-of-day stock quote for a given stock on a given
 * date.
 *
...
public class EODQuote implements IQuote {
	...
	/**
	 * Difference in quote prices (fraction of a cent) before they are assumed to be
	 * the same.
	 */
	 ...
	 */
	public EODQuote(Symbol symbol, TradingDate date, long day_volume, double day_low, double day_high, double day_open,
			double day_close) {
		setSymbol(symbol);
		setDate(date);
	...
	/**
	 * Clean up the quote. This fixes any irregularities that might exist in the
	 * quote, such as the day low not being the lowest quote.
	 *
	...
	public boolean equals(EODQuote quote) {
		return (getSymbol().equals(quote.getSymbol()) && getDate().equals(quote.getDate())
				...
				&& Math.abs(getDayClose() - quote.getDayClose()) < EPSILON && getDayVolume() == quote.getDayVolume());
	}
	...
	public String toString() {
		return new String(getSymbol() + ", " + getDate() + ", " + QuoteFormat.quoteToString(getDayOpen()) + ", "
				+ QuoteFormat.quoteToString(getDayHigh()) + ", " + QuoteFormat.quoteToString(getDayLow()) + ", "
				+ QuoteFormat.quoteToString(getDayClose()) + ", " + getDayVolume());

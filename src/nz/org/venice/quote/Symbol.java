...
/**
 * An immutable representation of a stock symbol, e.g. <code>CBA</code> or
 * <code>WBC</code>. Previously the stock symbol was stored as a String but this
 * created the question of whether it should be stored in lower or upper case.
 * Unfortunately, due to development drift, even though the stock symbols were
 * always displayed in upper case, they were stored in lower case. Often a
 * "missing" conversion meant the symbol was displayed in lower case or worse it
 * was stored in upper case which could cause the stock symbol to become
 * "invisible".
 *
 * <p>
 * By creating a single class we reduce the amount of <code>toUpperCase()</code>
 * and <code>toLowerCase()</code> calls required, and have a single place to
 * store symbol parsing code.
 *
 * <p>
 * To reduce memory, symbols are stored canonically. That is there is a single
 * object for each symbol. For example the string symbol "CBA" would be
 * represented by a single class, no matter where it was used. So instead of
 * instantiating a new symbol class, you find the canonical object using the
 * {@link #find} method.
 *
...
public class Symbol implements Cloneable, Comparable {
	...
	private Symbol(String string) throws SymbolFormatException {
		...
		for (int i = 0; i < string.length(); i++) {
			...
			if (!Character.isLetterOrDigit(letter) && letter != '.' && letter != '^' && letter != '-' && letter != '&'
					&& letter != ':')
	...
	/**
	 * Return the character at the given offset. See the java.lang.String class for
	 * more details.
	 *
	...
	/**
	 * Convert a string containing a list of symbols separated by spaces or commas
	 * into a sorted set of symbols with duplicates removed. This function Checks
	 * that all symbols appear in the quote source.
	 *
	 * e.g "CBA WBC TLS" -> [CBA, TLS, WBC].
	 *
	 * @param string      a comma or space separated list of symbols
	 * @param checkExists set this flag to <code>TRUE</code> to make sure that the
	 *                    symbols are in the current quote source
	 * @return a sorted set of symbols
	 * @exception SymbolFormatException if the string doesn't contain a list of
	 *                                  valid quotes
	 */
	...
	/**
	 * Convert a string containing a single symbol into a quote symbol. This differs
	 * from {@link Symbol#find} as it performs better error checking and checks that
	 * the symbol exists.
	 *
	 * @param string a string containing a single symbol
	 * @exception SymbolFormatException if the string doesn't contain a valid quote
	 ...
	 * @return the value <code>0</code> if the symbols are equal; <code>1</code> if
	 *         this symbol is after the specified symbol or <code>-1</code> if this
	 *         symbol is before the specified symbol.
	 */
	...
	/**
	 * Convert the symbol to a string. This is identical to the {@link Symbol#get}
	 * method.
	 *
	 * @return the symbol string
	 */

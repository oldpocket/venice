 ...
 * global end-of-day quote cache - {@link EODQuoteCache}.
 * <p>
 * The purpose of this class is therefore to group together a set of quotes that
 * are needed by a single task. This grouping allows the quotes to be loaded in
 * at one time, which is much faster than loading them in quote by quote.
 * <p>
 * Also by placing a set of quotes in a bundle it simplifies caching. Caching is
 * performed by {@link EODQuoteBundleCache}.
 *
 * Example:
 * 
 * <pre>
...
public class EODQuoteBundle implements QuoteBundle {
	...
	/**
	 * Get a stock quote. If the stock is earlier than the first date in the bundle,
	 * the bundle will be expand to include the new date given. If the stock symbol
	 * is not included in the original symbol list, the quote bundle will be
	...
	public double getQuote(Symbol symbol, int quoteType, int dateOffset) throws MissingQuoteException {
		...
		// If the quote is not in the quote cache, perhaps the quote bundle is not
		// loaded.
	 ...
	 * specialised QuoteBundle implementations such as {@link GPQuoteBundle} can
	 ...
	 * @return the quote
	 * @exception EvaluationException   if the script isn't allow access to the
	 *                                  quote.
	 * @exception MissingQuoteException if the quote was not found
	 */
	 ...
	 * @exception MissingQuoteException if the quote was not found
	 */
	public double getNearestQuote(Symbol symbol, int quoteType, int dateOffset) throws MissingQuoteException {
		...
		if (dateOffset > getFirstOffset()) {
			...
			while (!quoteCache.containsQuote(symbol, dateOffset) && dateOffset >= getFirstOffset()) {
				dateOffset--;
			}
	...
	public boolean containsQuote(Symbol symbol, int dateOffset) {
		...
		if (getQuoteRange().containsSymbol(symbol) && dateOffset >= getFirstOffset() && dateOffset <= getLastOffset())
			return true;
		else
			return false;
	}
	...
	/**
	 * Return an iterator over this quote bundle. The iterator will return, in
	...
	/**
	 * If we know the given quote is not in the quote cache, this function will
	 * reload this quote bundle.
	 *
	 * @return <code>true</code> if the quote bundle was reloaded,
	 *         <code>false</code> otherwise.
	 */
	...
	/**
	 * Try to expand the new quote bundle so that it include a new date and/or a new
	 * symbol. This function is passed the
	 *
	 ...
	 *         <code>false</code> if the quote range already includes the symbol and
	 *         date range.
	 */
	private boolean tryExpand(Symbol symbol, int dateOffset) {
		...
		if (getQuoteRange().getType() == EODQuoteRange.GIVEN_SYMBOLS && !getQuoteRange().containsSymbol(symbol)) {

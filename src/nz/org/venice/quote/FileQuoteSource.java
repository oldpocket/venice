 ...
 * implements the QuoteSource interface to allow users to directly use their
 * stock quote files without creating a database.
 *
 * Example:
 * 
 * <pre>
...
public class FileQuoteSource implements IQuoteSource {
	...
	private List getContainedQuotes(URL fileURL, EODQuoteRange quoteRange) {
	...
	/**
	 * Returns the company name associated with the given symbol. Not implemented
	 * for the file quote source.
	 *
	...
	/**
	 * Returns the symbol associated with the given company. Not implemented for the
	 * file quote source.
	 *
	...
	public boolean symbolExists(Symbol symbol) {
		...
		if (checkFiles()) {
			...
			for (Iterator iterator = dateToURL.keySet().iterator(); iterator.hasNext();) {
				...
				List quotes = getContainedQuotes(getURLForDate(date), new EODQuoteRange(symbol));
				if (quotes.size() > 0)
					return true; // found!
			}
		}
	 ...
	 * @return the exchange rate being the number of destinationCurrency that you
	 *         can buy per sourceCurrency
	 */
	...
	/**
	 * Reset the first and last dates, forcing this quotesource to reload data from
	 * the filesystem.
	 */

...
/**
 * Provides a filter to parse the Insight Trader quote format. This format uses
 * 2 digit years and prices in cents, volume is divided by 100. The first column
 * is the symbol, then the date, open, high, low, close & volume. Exampe:
 * 
 * <pre>
...
public class InsightTraderQuoteFilter implements IFileEODQuoteFilter {
	...
	public String toString(EODQuote quote) {
		return new String(quote.getSymbol() + " " + quote.getDate().toString("mm/dd/yy") + " "
				+ Math.round(quote.getDayOpen() * 100) + " " + Math.round(quote.getDayHigh() * 100) + " "
				+ Math.round(quote.getDayLow() * 100) + " " + Math.round(quote.getDayClose() * 100) + " "

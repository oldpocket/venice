...
import nz.org.venice.chart.Chart;
...
public class CommandManager {
	 ...
	 */
	public void tableTransactions(final Portfolio portfolio, final EODQuoteBundle quoteBundle) {
	...
	public void tableStocks(final int type) {
		Thread thread = new Thread(new Runnable() {
			public void run() {
				String title = new String(Locale.getString("LIST_IT", EODQuoteRange.getDescription(type)));
					...
					for (Iterator iterator = symbolsCopy.iterator(); iterator.hasNext();) {
						...
						if (!QuoteSourceManager.getSource().symbolExists(symbol)) {
							JOptionPane.showInternalMessageDialog(desktop,
									Locale.getString("NO_QUOTES_SYMBOL", symbol.toString()),
	...
	public void tableStocksByDate(final int type) {
		Thread thread = new Thread(new Runnable() {
			public void run() {
				...
				String title = new String(Locale.getString("LIST_IT_BY_DATE", EODQuoteRange.getDescription(type)));
				TradingDate date = TradingDateDialog.getDate(desktop, title, Locale.getString("DATE"));
				if (date != null)
	...
	public void tableStocksByRule(final int type) {
		Thread thread = new Thread(new Runnable() {
			public void run() {
				String title = new String(Locale.getString("LIST_IT_BY_RULE", EODQuoteRange.getDescription(type)));
				String rule = ExpressionQuery.getExpression(desktop, title, Locale.getString("RULE"));
				if (rule != null)
	...
	private void tableStocks(String title, int type, String rule, SortedSet symbols, TradingDate date) {
		...
		if (!thread.isInterrupted()) {
			Object[] quoteBundleSingleDate = getQuoteBundle(type, symbols, date);
	...
	private synchronized Object[] getQuoteBundle(int type, SortedSet symbols, TradingDate date) {
		EODQuoteRange quoteRange = null;
		boolean singleDate;
		...
		if (type == EODQuoteRange.GIVEN_SYMBOLS) {
			quoteRange = new EODQuoteRange(new ArrayList(symbols));
			singleDate = false;
		} else {
	...
	public void tableAlerts() {
		Thread thread = new Thread(new Runnable() {
			public void run() {
				...
				if (ar == null) {
					JOptionPane.showInternalMessageDialog(desktop, Locale.getString("ALERT_DISABLED_ERROR"),
							Locale.getString("ALERT_DISABLED_ERROR"), JOptionPane.WARNING_MESSAGE);
				} else {
					try {
						alertModule = new AlertModule(desktop, ar, aw);
						desktopManager.newFrame(alertModule);
					} catch (AlertException e) {
						JOptionPane.showInternalMessageDialog(desktop, "localiseme", e.getMessage(),
	...
	public void tableAlertsBySymbol(final List symbols) {
		Thread thread = new Thread(new Runnable() {
			public void run() {
				...
				if (symbols == null) {
					TreeSet symbolsSet = (TreeSet) SymbolListDialog.getSymbols(desktop,
							Locale.getString("LIST_BY_SYMBOLS"));
					symbolsCopy = (symbolsSet != null) ? new ArrayList(symbolsSet) : symbols;
				} else {
					symbolsCopy = symbols;
				}
				...
				if (ar == null) {
					...
					try {
						alertModule = new AlertModule(desktop, symbolsCopy, ar, aw);
						desktopManager.newFrame(alertModule);
					} catch (AlertException e) {
						JOptionPane.showInternalMessageDialog(desktop, Locale.getString("ALERT_EXCEPTION"),
	...
	public void triggeredAlerts() {
		...
		boolean alertsTriggered = AlertManager.alertsTriggered(triggeredAlerts, triggerValues);
		...
		Thread thread = new Thread(new Runnable() {
			public void run() {
				...
				while (alertIterator.hasNext()) {
					...
					int optionSelected = AlertTriggeredDialog.show(desktop, alert, triggerValue.toString());
		...
		final Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					if (lastDate != null) {
						if (!thread.isInterrupted()) {
							EODQuoteRange quoteRange = new EODQuoteRange(portfolio.getStocksHeld(),
									lastDate.previous(1), lastDate);
					...
					DesktopManager.showErrorMessage(Locale.getString("ERROR_LOADING_PORTFOLIO_TITLE"), e.getMessage());
				}
	...
	public void openPortfolio(final Portfolio portfolio) {
		final Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				if (lastDate != null) {
					if (!thread.isInterrupted()) {
						EODQuoteRange quoteRange = new EODQuoteRange(portfolio.getStocksHeld(), lastDate.previous(1),
								lastDate);
	 ...
	 * @param quoteBundle fully loaded quote bundle
	 */
	public void openPortfolio(final Portfolio portfolio, final EODQuoteBundle quoteBundle) {
		desktopManager.newFrame(new PortfolioModule(desktop, portfolio, quoteBundle));
	}
	...
	/**
	 * Open up a result table that will display a summary of results from paper
	 * trading.
	 *
	...
	/**
	 * Open up a result table that will display a summary of results from genetic
	 * programming.
	 *
	...
	/**
	 * Open up a result table that will display a summary of results from artificial
	 * neural network.
	 *
	...
	public void newWatchScreen() {
		...
		TextDialog dialog = new TextDialog(desktop, Locale.getString("ENTER_WATCH_SCREEN_NAME"),
	...
	public void openWatchScreen(String watchScreenName) {
		try {
			...
			DesktopManager.showErrorMessage(Locale.getString("ERROR_LOADING_WATCH_SCREEN_TITLE"), e.getMessage());
		}
	}
	...
	public void openWatchScreen(final WatchScreen watchScreen) {
		final Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				if (lastDate != null) {
					if (!thread.isInterrupted())
						quoteBundle = new MixedQuoteBundle(watchScreen.getSymbols(), lastDate.previous(1), lastDate);

					if (!thread.isInterrupted())
						desktopManager.newFrame(new WatchScreenModule(watchScreen, quoteBundle));
				}
	...
	public void openPreferences() {
		...
		synchronized (this) {
			if (!wakeIfPresent(preferencesModuleFrame)) {
				...
				preferencesModuleFrame = desktopManager.newFrame(preferencesModule, true, false, true);
			}
		}
	}
	 ...
	 * @param quoteBundle fully loaded quote bundle
	 */
	public void graphPortfolio(Portfolio portfolio, EODQuoteBundle quoteBundle) {
	 ...
	 */
	public void graphPortfolio(Portfolio portfolio, EODQuoteBundle quoteBundle, TradingDate startDate,
			TradingDate endDate) {
		...
		if (symbolsTraded.size() > 0 && quoteBundle == null) {
			...
			quoteBundle = new EODQuoteBundle(new EODQuoteRange(symbolsTraded, startDate, endDate));
		}
		...
		if (symbolsTraded.size() == 0)
			...
			portfolioGraphSource = new PortfolioGraphSource(portfolio, quoteBundle, PortfolioGraphSource.MARKET_VALUE);
	...
	public void tablePortfolio(Portfolio portfolio, EODQuoteBundle quoteBundle, TradingDate startDate,
			TradingDate endDate) {
	...
	/**
	 * Displays a graph closing prices for stock(s), based on their code. The
	 * stock(s) is/are determined by a user prompt if a set of symbols is not
	 * supplied.
	 *
	...
	public void graphStockBySymbol(final java.util.List symbols) {
		...
		final Thread thread = new Thread(new Runnable() {
			public void run() {
				...
				if (symbols == null)
					symbolsCopy = SymbolListDialog.getSymbols(desktop, Locale.getString("GRAPH_BY_SYMBOLS"));
				else {
	...
	/**
	 * Displays a graph closing prices for stock(s), based on their name. The
	 * stock(s) is/are determined by a user prompt
	 */
	/*
	 * public void graphStockByName() { final Thread thread = new Thread(new
	 * Runnable() { public void run() { SortedSet s =
	 * SymbolListDialog.getSymbolByName(desktop, "Graph by name"); graphStock(s); }
	 * }); thread.start(); }
	 */

	/**
	 * Displays a graph of closing prices for an index, based on a list of symbols.
	 * The stock(s) is/are determined by a user prompt if a set of symbols is not
	 * supplied.
	 *
	 * @param symbols Optional. Set of symbols to graph.
	 */
	...
	private void graphIndex(SortedSet symbols) {
		...
		if (symbols != null) {
			...
			/*
			 * Has data is aggregate of all symbols, the actual symbol used to mark the
			 * chart is irrelevant.
			 */
			Symbol symbol = (Symbol) iterator.next();
			chart.add(graph, symbol, quoteBundle, 0);
	...
	public ChartTracking getTracker(ChartModule chartModule, Chart chart, Symbol symbol) {
		...
		ChartTracking tracker = new ChartTracking(chartModule, chart, table, symbol);
	...
	public void openAboutDialog() {
		if (!isAboutDialogUp) {
			...
			String aboutMessage = (Locale.getString("VENICE_LONG") + ", " + Main.LONG_VERSION + " / "
					+ Main.RELEASE_DATE + "\n" +

					Locale.getString("COPYRIGHT", Main.COPYRIGHT_DATE_RANGE) + ", " + "Andrew Leppard\n\n" +
					...
					+ simplifiedChinese.getDisplayName() + "), " + "Alberto Nacher (" + italian.getDisplayName()
					+ "),\n" + "Jordi Pujol (" + catalan.getDisplayName() + "), " + "Pontus Str\366mdahl ("
					+ swedish.getDisplayName() + ") &\n" + "Christian Brom (" + german.getDisplayName() + ")");

			String aboutVenice = Locale.getString("ABOUT_VENICE", Locale.getString("VENICE_SHORT"));
			JOptionPane.showInternalMessageDialog(desktop, aboutMessage, aboutVenice, JOptionPane.PLAIN_MESSAGE);
	...
	/**
	 * Displays an import file dialog that allows the user to import preferences
	 * into the application from an XML file.
	 */
	...
	/**
	 * Displays an import file dialog that allows the user to export preferences
	 * from the application into an XML file.
	 */
	...
	/**
	 * Displays the import quotes modules that allows the user to import quotes into
	 * the application.
	 */
	...
	/**
	 * Displays the intra-day quote quote sync module that allows the user to
	 * automatically download new intra-day quotes into the application.
	 */
	...
	/**
	 * Displays the export quotes modules that allows the user to export quotes from
	 * the application.
	 */
	...
	/**
	 * Checks to see if the current frame is open. If so it will make sure the frame
	 * is visible and move it to the front of the screen. This function has too
	 * purposes: (1) To re-use previously created frames (2) To prevent multiple
	 * instances of the frames being displayed.
	 *
	 * @param frame the frame to check (may be null)
	 * @returns <code>TRUE</code> if the frame is now displayed; <code>FALSE</code>
	 *          otherwise
	 */
	...
	public IGraph getNewGraph(EODQuoteBundle quoteBundle, boolean index) {
		...
		if (defaultChart.compareTo("BAR_CHART") == 0) {
			graph = new BarChartGraph(dayOpen, dayLow, dayHigh, dayClose);
		} else if (defaultChart.compareTo("CANDLE_STICK") == 0) {
			graph = new CandleStickGraph(dayOpen, dayLow, dayHigh, dayClose);

...
public class GPGondolaSelection extends Page implements IAnalyserPage {
	...
	public GPGondolaSelection(JDesktopPane desktop, double maxHeight) {
		...
		GPGondolaSelectionPanel[0] = new GPGondolaSelectionPanel(defaultValuesFloatInteger.length, desktop,
				defaultValuesFloatInteger, defaultTextFieldValuesFloatInteger);

		// Terminal Integers
		int[] defaultValuesTerminalIntegers = { 5000, /*
														 * Ordinary number has 50% probabilty as default As default we
														 * don't generate DayOfYearExpression() or MonthExpression()
														 * because it would make it easy for the GP to hook onto
														 * specific dates where the market is low. By removing these it
														 * forces the GP to use the stock data to generate buy/sell
														 * decisions.
														 */
		...
		String[] defaultTextFieldValuesTerminalIntegers = { Locale.getString("PERCENTAGE_ORDINARY_NUMBER"),
				...
				Locale.getString("PERCENTAGE_FUNCTIONS", "month()"), Locale.getString("PERCENTAGE_FUNCTIONS", "day()"),
				...
				/*
				 * order and held must be the last ones, because they can be skipped according
				 * to GP parameters. See getRandom in GPGondolaSelectionPanel for further
				 * details. {@link GPGondolaSelectionPanel#getRandom}
				 */
				Locale.getString("PERCENTAGE_FUNCTIONS", "held"), Locale.getString("PERCENTAGE_FUNCTIONS", "order") };
		GPGondolaSelectionPanel[1] = new GPGondolaSelectionPanel(defaultValuesTerminalIntegers.length, desktop,
				defaultValuesTerminalIntegers, defaultTextFieldValuesTerminalIntegers);
		...
		String[] defaultTextFieldValuesTerminalFloats = { Locale.getString("PERCENTAGE_ORDINARY_NUMBER"),
				...
				/*
				 * stockcapital must be the last one, because it can be skipped according to GP
				 * parameters. See getRandom in GPGondolaSelectionPanel for further details.
				 * {@link GPGondolaSelectionPanel#getRandom}
				 */
				Locale.getString("PERCENTAGE_FUNCTIONS", "stockcapital"), };
		GPGondolaSelectionPanel[2] = new GPGondolaSelectionPanel(defaultValuesTerminalFloats.length, desktop,
				defaultValuesTerminalFloats, defaultTextFieldValuesTerminalFloats);
		...
		String[] defaultTextFieldValuesFloatQuote = { Locale.getString("PERCENTAGE_FUNCTIONS", "open"),
				Locale.getString("PERCENTAGE_FUNCTIONS", "low"), Locale.getString("PERCENTAGE_FUNCTIONS", "high"),
				Locale.getString("PERCENTAGE_FUNCTIONS", "close") };
		GPGondolaSelectionPanel[3] = new GPGondolaSelectionPanel(defaultValuesFloatQuote.length, desktop,
				defaultValuesFloatQuote, defaultTextFieldValuesFloatQuote);
		...
		String[] defaultTextFieldValuesBoolean = { Locale.getString("PERCENTAGE_FUNCTIONS", "not"),
				Locale.getString("PERCENTAGE_FUNCTIONS", "="), Locale.getString("PERCENTAGE_FUNCTIONS", ">="),
				Locale.getString("PERCENTAGE_FUNCTIONS", ">"), Locale.getString("PERCENTAGE_FUNCTIONS", "<="),
				Locale.getString("PERCENTAGE_FUNCTIONS", "<"), Locale.getString("PERCENTAGE_FUNCTIONS", "!="),
				Locale.getString("PERCENTAGE_FUNCTIONS", "and"), Locale.getString("PERCENTAGE_FUNCTIONS", "or") };
		GPGondolaSelectionPanel[4] = new GPGondolaSelectionPanel(defaultValuesBoolean.length, desktop,
				defaultValuesBoolean, defaultTextFieldValuesBoolean);

		// Float Expression
		int[] defaultValuesFloatExpression = { 400, 400, 400, 400, 400, 400, 400, 400, 400, 400, 400, 400, 400, 400,
		...
		String[] defaultTextFieldValuesFloatExpression = { Locale.getString("PERCENTAGE_TERMINAL"),
				...
				Locale.getString("PERCENTAGE_FUNCTIONS", "lag()"), Locale.getString("PERCENTAGE_FUNCTIONS", "min()"),
				Locale.getString("PERCENTAGE_FUNCTIONS", "max()"), Locale.getString("PERCENTAGE_FUNCTIONS", "sum()"),
				Locale.getString("PERCENTAGE_FUNCTIONS", "sqrt()"), Locale.getString("PERCENTAGE_FUNCTIONS", "abs()"),
				Locale.getString("PERCENTAGE_FUNCTIONS", "cos()"), Locale.getString("PERCENTAGE_FUNCTIONS", "sin()"),
				Locale.getString("PERCENTAGE_FUNCTIONS", "log()"), Locale.getString("PERCENTAGE_FUNCTIONS", "exp()"),
				Locale.getString("PERCENTAGE_FUNCTIONS", "avg()"), Locale.getString("PERCENTAGE_FUNCTIONS", "ema()"),
				Locale.getString("PERCENTAGE_FUNCTIONS", "macd()"),
				Locale.getString("PERCENTAGE_FUNCTIONS", "momentum()"),
				Locale.getString("PERCENTAGE_FUNCTIONS", "rsi()"), Locale.getString("PERCENTAGE_FUNCTIONS", "sd()"),
				Locale.getString("PERCENTAGE_FUNCTIONS", "bol_lower()"),
				Locale.getString("PERCENTAGE_FUNCTIONS", "bol_upper()") };
		GPGondolaSelectionPanel[5] = new GPGondolaSelectionPanel(defaultValuesFloatExpression.length, desktop,
				defaultValuesFloatExpression, defaultTextFieldValuesFloatExpression);

		// Integer Expression
		int[] defaultValuesIntegerExpression = { 480, 476, 476, 476, 476, 476, 476, 476, 476, 476, 476, 476, 476, 476,
		...
		GPGondolaSelectionPanel[6] = new GPGondolaSelectionPanel(defaultValuesIntegerExpression.length, desktop,
				defaultValuesIntegerExpression, defaultTextFieldValuesIntegerExpression);

		// Positive Short Integer Expression
		int[] defaultValuesPositiveShortIntegerExpression = {
				/*
				 * We put as default to zero the two following experssions: '*' and
				 * '(+1)*float'. (Addendum: (+1)*float no longer casts to int and have been
				 * replaced by floor() and ceil()) That's done, because them both may increase
				 * the integer number too much and we don't want that because we want a small
				 * positive integer. Anyway user interface permits the use of the above
				 * expressions.
				 */
				1250, 1250, 1250, 0, 1250, 1250, 1250, 1250, 1250, 0, 0, 0 };
		...
		GPGondolaSelectionPanel[7] = new GPGondolaSelectionPanel(defaultValuesPositiveShortIntegerExpression.length,
				desktop, defaultValuesPositiveShortIntegerExpression,
		...
		int[] defaultValuesNegativeShortIntegerExpression = {
				/*
				 * We put as default to zero the two following experssions: '*' and
				 * '(-1)*float'. That's done, because them both may increase the integer number
				 * too much and we don't want that because we want a small positive integer.
				 * Anyway user interface permits the use of the above expressions.
				 * 
				 * (-1)* float removed because it no longer casts to int
				 */
				1750, 1650, 1650, 0, 1650, 1650, 1650, 0 };
		...
		GPGondolaSelectionPanel[8] = new GPGondolaSelectionPanel(defaultValuesNegativeShortIntegerExpression.length,
				desktop, defaultValuesNegativeShortIntegerExpression,
	...
	public boolean parse() {
		...
		if (!isAllValuesAcceptable()) {
			...
			if (!isFitAll()) {
				ConfirmDialog dialog = new ConfirmDialog(desktop, Locale.getString("GP_FIT"),
	...
	private void fitAll() {
		// Fit all values one after another (only if previous one was fitted without
		// error)
		for (int ii = 0; ii < maxPanels; ii++) {
	...
	private void setGraphic(Dimension preferredSize) {
		...
		titledBorderSections[7] = new TitledBorder(
				Locale.getString("GP_GONDOLA_SELECTION_TITLE_POSITIVE_SHORT_INTEGER"));
		titledBorderSections[8] = new TitledBorder(
				Locale.getString("GP_GONDOLA_SELECTION_TITLE_NEGATIVE_SHORT_INTEGER"));
		for (int ii = 0; ii < maxPanels; ii++) {

...
/**
 * A trading individual evovled by the GP. The Individual contains a buy rule
 * and a sell rule that will have been evolved by the GP.
 *
...
public class Individual implements Comparable {
	...
	private final static String PORTFOLIO_NAME = Locale.getString("GENETIC_PROGRAMME_PORTFOLIO");
	...
	/**
	 * Create a new individual by "breeding" or combining the two parent
	 * individuals.
	 *
	 ...
	 */
	public Individual(Random random, Mutator buyRuleMutator, Mutator sellRuleMutator, Individual father,
			Individual mother) {
		...
		{
			// Single
			if (breedType == BREED_BY_RECOMBINING || breedType == BREED_BY_DOUBLE_RECOMBINING
	 ...
	 * quote data will also be rejected, e.g. rules that are only based on the day
	 * of the week, order, etc.
	 *
	 * <p>
	 * The minimum size check is to prevent us wasting time on too simplistic rules.
	 * The maximum size check is to prevent rules exponentially increasing and so
	 * reduce CPU time. We reject rules not based on quote data as also being too
	 * simplistic to be worthwhile.
	 *
	...
	/**
	 * Paper trade with the individual's buy and sell rules. Set the
	 * <code>stockValue</code> to <code>null</code> to trade by number of stocks in
	 * the portfolio.
	 *
	 ...
	 */
	public Money paperTrade(GPQuoteBundle quoteBundle, OrderCache orderCache, TradingDate startDate,
			TradingDate endDate, Money initialCapital, Money stockValue, int numberStocks, Money tradeCost,
			String tradeValueBuy, String tradeValueSell) throws EvaluationException {

		// Is there a fixed number of stocks?
		if (stockValue == null)
			portfolio = PaperTrade.paperTrade(PORTFOLIO_NAME, quoteBundle, new Variables(), orderCache, startDate,
					endDate, getBuyRule(), getSellRule(), initialCapital, numberStocks, tradeCost, tradeValueBuy,
					tradeValueSell);
		// Or a fixed value?
		else {
			portfolio = PaperTrade.paperTrade(PORTFOLIO_NAME, quoteBundle, new Variables(), orderCache, startDate,
					endDate, getBuyRule(), getSellRule(), initialCapital, stockValue, tradeCost, tradeValueBuy,
					tradeValueSell);
		}
	 ...
	 * @return <code>-1</code> if this individual is worth less, <code>0</code> if
	 *         the individuals are worth the same or <code>1</code> if this
	 *         individual is worth more.
	 */

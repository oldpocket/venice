...
/**
 * This class is a specialised version of the EODQuoteBundle tailored
 * specifically for the GP. It differs from in that it enforces a window of
 * dates. This window prevents the GP expressions from accessing quotes too far
 * in the past which would slow down calculations (e.g. calculating the average
 * value of a stock for the last 3,000 days), and also prevents GP expressions
...
public class GPQuoteBundle extends EODQuoteBundle {
	 ...
	 * @param window      prevent access to quotes this many days before the first
	 *                    quote in the given quote bundle
	 */
	 ...
	 * @return the quote
	 * @exception EvaluationException if the script isn't allowed access to the
	 *                                quote.
	 */
	public double getQuote(Symbol symbol, int quoteType, int today, int offset)
		...
		// is negative.
		else if (Math.abs(offset) > Math.abs(window) || offset == Integer.MIN_VALUE)

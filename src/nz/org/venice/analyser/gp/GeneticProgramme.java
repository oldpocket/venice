...
public class GeneticProgramme {
	 ...
	 */
	public GeneticProgramme(GPQuoteBundle quoteBundle, GPGondolaSelection GPGondolaSelection, OrderCache orderCache,
	...
	public void nextIndividual(Expression buyRule, Expression sellRule, int mutations) {
		...
		while (!validIndividual) {
			...
			if (individual.isValid(MIN_SIZE, MAX_SIZE)) {
				try {
					Money value = individual.paperTrade(quoteBundle, orderCache, startDate, endDate, initialCapital,
							stockValue, numberStocks, tradeCost, tradeValueBuy, tradeValueSell);
	...
	/**
	 * Create a new individual. If it is the first generation then we will create an
	 * entirely random individual, otherwise we will base it on the combination of
	...
	private void competeForBreeding(Individual individual, Money value) {
		...
		if (individual != null && value != null && value.isGreaterThan(initialCapital)) {
			...
			Individual sameTradeIndividual = (Individual) nextBreedingPopulation.get(value);

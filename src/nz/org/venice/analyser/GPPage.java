...
/**
 * An analysis tool page that lets the user enter basic Genetic Programming
 * configuration parameters. This page is used only by the {@link GPModule}. The
 ...
 * </ul>
 *
 * The generations field describes the number of generations the GP will run
 ...
 * <p>
 * The window size field describes the number of quote days that the buy/sell
 * rules can access. When equations are evaluating buy or sell decisions, they
 * can only access this many quote days into the past.
 *
...
public class GPPage extends Page implements IAnalyserPage {
	...
	public boolean parse() {
		...
		if (displayPopulation > breedingPopulation) {
			showErrorMessage(Locale.getString("DISPLAY_POPULATION_ERROR"), Locale.getString("INVALID_GP_ERROR"));
			return false;
		}

		if (generations <= 0) {
			showErrorMessage(Locale.getString("NO_GENERATION_ERROR"), Locale.getString("INVALID_GP_ERROR"));
			return false;
		}

		if (window < MINIMUM_WINDOW_SIZE) {
			showErrorMessage(Locale.getString("WINDOW_SIZE_ERROR", MINIMUM_WINDOW_SIZE),
					Locale.getString("INVALID_GP_ERROR"));
			return false;
		}

		if (population <= 0) {
			showErrorMessage(Locale.getString("NO_INDIVIDUAL_ERROR"), Locale.getString("INVALID_GP_ERROR"));
			return false;
		}

		if (breedingPopulation <= 0) {
			showErrorMessage(Locale.getString("NO_BREEDING_INDIVIDUAL_ERROR"), Locale.getString("INVALID_GP_ERROR"));
			return false;
		}

		if (displayPopulation <= 0) {
			showErrorMessage(Locale.getString("NO_DISPLAY_INDIVIDUAL_ERROR"), Locale.getString("INVALID_GP_ERROR"));
			return false;
		}
	...
	/**
	 * Return the number of quote days that any indivdual's buy or sell rules can
	 * access at anyone time. The buy or sell rule will only be able to see this
	 * many quote days into the past.
	 *
	...
	private void layoutPage() {
		...
		displayPopulationTextField = GridBagHelper.addTextRow(innerPanel, Locale.getString("DISPLAY_POPULATION"), "",
				gridbag, c, 7);
		windowTextField = GridBagHelper.addTextRow(innerPanel, Locale.getString("WINDOW_SIZE"), "", gridbag, c, 5);

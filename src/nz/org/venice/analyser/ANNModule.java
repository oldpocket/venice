...
public class ANNModule extends Page implements IModule {
	...
	private ANNResult paperTrade(EODQuoteBundle quoteBundle, String quoteRangeDescription, OrderCache orderCache,
		...
		if (mode == PortfolioPage.STOCK_VALUE_MODE) {
			portfolio = ANNPaperTrade.paperTrade(Locale.getString("ANN_OF", quoteRangeDescription), quoteBundle,
					variables, orderCache, startDate, endDate, initialCapital, stockValue, tradeCost, tradeValueBuy,
					tradeValueSell, progress, inputExpressions, artificialNeuralNetwork);
		} else {
			assert portfolioPage.getMode() == PortfolioPage.NUMBER_STOCKS_MODE;
			portfolio = ANNPaperTrade.paperTrade(Locale.getString("ANN_OF", quoteRangeDescription), quoteBundle,
					variables, orderCache, startDate, endDate, initialCapital, numberStocks, tradeCost, tradeValueBuy,
					tradeValueSell, progress, inputExpressions, artificialNeuralNetwork);
		}

		return new ANNResult(portfolio, quoteBundle, initialCapital, tradeCost, startDate, endDate,
	...
	private void paperTraining(IProgressDialog progress, EODQuoteBundle quoteBundle, String quoteRangeDescription,
		...
		if (mode == PortfolioPage.STOCK_VALUE_MODE) {
			ANNPaperTrade.paperTraining(Locale.getString("ANN_OF", quoteRangeDescription), quoteBundle, variables,
					orderCache, startDate, endDate, initialCapital, stockValue, tradeCost, tradeValueBuy,
					tradeValueSell, progress, ANNTrainingPage, inputExpressions, artificialNeuralNetwork);
		} else {
			assert portfolioPage.getMode() == PortfolioPage.NUMBER_STOCKS_MODE;
			ANNPaperTrade.paperTraining(Locale.getString("ANN_OF", quoteRangeDescription), quoteBundle, variables,
					orderCache, startDate, endDate, initialCapital, numberStocks, tradeCost, tradeValueBuy,
					tradeValueSell, progress, ANNTrainingPage, inputExpressions, artificialNeuralNetwork);
		}
	}

	/*
	 * Return a list with a ANNResult object. It is used to know all about paper
	...
	private List getANNResults() {
		...
		// We set the progress bar, it will be incremented one by one for each trading
		// date
		...
		try {
			...
			ANNResults.add(paperTrade(quoteBundle, quoteRangeDescription, orderCache, startDate, endDate,
					initialCapital, mode, stockValue, numberStocks, tradeCost, variables, tradeValueBuy, tradeValueSell,
					progress, inputExpressions, artificialNeuralNetwork));

		} catch (EvaluationException e) {
			ProgressDialogManager.closeProgressDialog(progress);
			progress = null;

			showErrorMessage(e.getReason(), Locale.getString("ERROR_EVALUATING_EQUATION"));
	...
	private void trainANN() {
		...
		try {
			...
			paperTraining(progress, quoteBundle, quoteRangeDescription, orderCache, startDate, endDate, initialCapital,
					mode, stockValue, numberStocks, tradeCost, variables, tradeValueBuy, tradeValueSell,
					ANNTrainingPage, inputExpressions, artificialNeuralNetwork);
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				if (resultsFrame == null || resultsFrame.isClosed()) {
					resultsFrame = CommandManager.getInstance().newANNResultTable();
				} else {
				...
				ANNResultModule resultsModule = (ANNResultModule) resultsFrame.getModule();

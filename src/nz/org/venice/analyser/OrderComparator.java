...
/**
 * This comparator orders the stock quotes on a given date. The GP allows the
 * user or order the evaluation of stock quotes, by several criteria such as
 * unordered, alphabetically by symbol, volume decreasing etc. This comparator
...
public class OrderComparator implements Comparator {
	...
	/**
	 * Create a new order comparator that will order the quotes in the given quote
	 * bundle using the given order.
	 *
	...
	/**
	 * Create a new order comparator that will order the quotes in the given quote
	 * bundle using the given equation.
	 *
	 * @param quoteBundle     quote data used for ordering
	...
	/**
	 * Return the method of ordering. If the quotes will be ordered by equation then
	 * this function will return {@link #EQUATION}.
	 *
	 * @return order key
	 */
	 ...
	 * @return <code>-1</code> if the first symbol comes first, <code>0</code> if
	 *         the symbols are equal OR <code>1</code> if the first symbol comes
	 *         last.
	 */
	...
	private int compareByEquation(Symbol symbol1, Symbol symbol2) {
		...
		try {
			double valueOne = orderByEquation.evaluate(new Variables(), quoteBundle, symbol1, dateOffset);
			double valueTwo = orderByEquation.evaluate(new Variables(), quoteBundle, symbol2, dateOffset);

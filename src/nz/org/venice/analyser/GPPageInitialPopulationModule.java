...
public class GPPageInitialPopulationModule extends AbstractTable implements Module {
	...
	public GPPageInitialPopulationModule(Page page) {
		...
		columns.add(new Column(BUY_RULE_COLUMN, Locale.getString("BUY_RULE"),
				Locale.getString("BUY_RULE_COLUMN_HEADER"), String.class, Column.VISIBLE));
		columns.add(new Column(SELL_RULE_COLUMN, Locale.getString("SELL_RULE"),
				Locale.getString("SELL_RULE_COLUMN_HEADER"), String.class, Column.VISIBLE));
		columns.add(new Column(PERCENT_COLUMN, Locale.getString("PERCENT_MUTATION"),
				Locale.getString("PERCENT_MUTATION_COLUMN_HEADER"), String.class, Column.VISIBLE));
	...
	private void viewBuyRule() {
		...
		if (row != -1) {
			...
			Thread thread = new Thread(new Runnable() {
				public void run() {
					ExpressionEditorDialog.showViewDialog(Locale.getString("VIEW_BUY_RULE"), result[BUY_RULE_COLUMN]);
				}
			});
	...
	private void viewSellRule() {
		...
		if (row != -1) {
			...
			Thread thread = new Thread(new Runnable() {
				public void run() {
					ExpressionEditorDialog.showViewDialog(Locale.getString("VIEW_SELL_RULE"), result[SELL_RULE_COLUMN]);
				}
			});
	...
	private void storeBuyRule() {
		...
		if (row != -1) {
			...
			Thread thread = new Thread(new Runnable() {
				public void run() {
					ExpressionEditorDialog.showAddDialog(Locale.getString("STORE_BUY_RULE"), result[BUY_RULE_COLUMN]);
				}
			});
	...
	private void storeSellRule() {
		...
		if (row != -1) {
			...
			Thread thread = new Thread(new Runnable() {
				public void run() {
					ExpressionEditorDialog.showAddDialog(Locale.getString("STORE_SELL_RULE"), result[SELL_RULE_COLUMN]);
				}
			});
	...
	// Display a dialog allowing the user to edit the selected rules and the
	// percentage value.
		...
		if (row != -1) {
			...
			Thread thread = new Thread(new Runnable() {
				public void run() {
					...
					boolean[] areas = { true, true, false };
					String[] newRules = TextsEditorDialog.showEditDialog(Locale.getString("EDIT"), labels, areas,
							result);
		...
		Thread thread = new Thread(new Runnable() {
			public void run() {
				String[] labels = { Locale.getString("BUY_RULE_COLUMN_HEADER"),
						...
						Locale.getString("PERCENT_MUTATION_COLUMN_HEADER") };
				boolean[] areas = { true, true, false };
				String[] newRules = TextsEditorDialog.showAddDialog(Locale.getString("ADD"), labels, areas);
				if (newRules[0] != null)
	...
	private boolean isAllValuesAcceptable() {
		try {
			...
			this.page.showErrorMessage(Locale.getString("ERROR_PARSING_NUMBER", e.getMessage()),
					Locale.getString("INVALID_GP_ERROR"));
			return false;
		}

		if (!isAllValuesPositive()) {
			this.page.showErrorMessage(Locale.getString("NO_POSITIVE_VALUES_ERROR"),
					Locale.getString("INVALID_GP_ERROR"));
			return false;
		}
	...
	private void setNumericalValues() throws ParseException {
		...
		for (int i = 0; i < perc.length; i++) {
			...
			if ((!result[PERCENT_COLUMN].equals("")) && (!result[PERCENT_COLUMN].equals(nullString))) {
				perc[i] = (int) Math
						.round(PERCENT_DOUBLE * (decimalFormat.parse(result[PERCENT_COLUMN]).doubleValue()));
			} else {
				perc[i] = 0;
			}
		}
	...
	private boolean isTotalOK() {
		...
		if (total == 0) {
			 ...
			 * Locale.getString("INVALID_GP_ERROR"), JOptionPane.ERROR_MESSAGE); return
			 * false;
			 */
		}
		return true;
	...
	public void save(HashMap settings, String idStr) {
		...
		for (int i = 0; i < results.size(); i++) {
			...
			if (!((values[BUY_RULE_COLUMN].compareTo(nullString) == 0)
				...
				String value = values[BUY_RULE_COLUMN] + separatorString + values[SELL_RULE_COLUMN] + separatorString
						+ values[PERCENT_COLUMN];
	...
	public void load(String value) {
		...
		if (values.length == NUMBER_COLUMN) {
			...
			if (!((values[BUY_RULE_COLUMN].compareTo("") == 0) && (values[SELL_RULE_COLUMN].compareTo("") == 0))) {
	...
	public int getRandom() {
		if (isAllValuesAcceptable()) {
			...
			this.page.showErrorMessage(Locale.getString("ERROR_GENERATING_RANDOM_NUMBER"),
					Locale.getString("INVALID_GP_ERROR"));
		}
		return 0;
	}
	...
	public boolean parse() {
		if (!isAllValuesAcceptable()) {
			...
			if (!isFitAll()) {
				ConfirmDialog dialog = new ConfirmDialog(this.page.desktop, Locale.getString("GP_FIT_PAGE"),

...
public class ANNNetworkTypePage extends Page implements AnalyserPage {
	...
	/**
	 * Construct a new network type parameters page. It manages: the default/custom
	 * artificial neural network choice. the import/export of artificial neural
	 * network from/to file system and Joone,
	 *
	...
	public void save(String key) {
		...
		if (!artificialNeuralNetwork.isSaved()) {
			int answer = JOptionPane.showConfirmDialog(desktop, Locale.getString("ANN_NOT_SAVED_MESSAGE"),
					Locale.getString("ANN_NOT_SAVED_TITLE"), JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
	...
	/**
	 * Parse the GUI. We don't need parse() method, we use parse(int inputRows)
	 * instead, because we need to know how many input we have to manage with ANN.
	 */
		...
		if (this.isDefaultSelected()) {
			if (this.artificialNeuralNetwork.isANNNull()) {
				...
				if (!this.artificialNeuralNetwork.isInputOK(inputRows)) {
					answer = JOptionPane.showConfirmDialog(desktop, Locale.getString("ANN_INPUT_MESSAGE"),
							Locale.getString("ANN_INPUT_TITLE"), JOptionPane.YES_NO_CANCEL_OPTION,
			...
			if (this.artificialNeuralNetwork.isANNNull()) {
				answer = JOptionPane.showConfirmDialog(desktop, Locale.getString("ANN_NO_MESSAGE"),
						Locale.getString("ANN_NO_TITLE"), JOptionPane.YES_NO_CANCEL_OPTION,
						JOptionPane.QUESTION_MESSAGE);
				if (answer == JOptionPane.YES_OPTION) {
				...
				answer = JOptionPane.showConfirmDialog(desktop, Locale.getString("ANN_OUTPUT_MESSAGE"),
						Locale.getString("ANN_OUTPUT_TITLE"), JOptionPane.YES_NO_CANCEL_OPTION,
						JOptionPane.QUESTION_MESSAGE);
				if (answer == JOptionPane.YES_OPTION) {
	...
	private void setGraphic() {
		...
		TitledBorder networkTypeTitled = new TitledBorder(Locale.getString("ANN_PAGE_NETWORK_TYPE_LONG"));
		...
		defaultANNButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				...
				if (!artificialNeuralNetwork.isSaved()) {
					int answer = JOptionPane.showConfirmDialog(desktop, Locale.getString("ANN_NOT_SAVED_MESSAGE"),
							Locale.getString("ANN_NOT_SAVED_TITLE"), JOptionPane.YES_NO_CANCEL_OPTION,
							JOptionPane.QUESTION_MESSAGE);
					if (answer == JOptionPane.YES_OPTION) {
						// save the network
						saveButton.doClick();
		...
		customANNButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				...
				pathTextField.setEnabled(true);
				int answer = JOptionPane.showConfirmDialog(desktop, Locale.getString("ANN_CHANGE_MESSAGE"),
						Locale.getString("ANN_CHANGE_TITLE"), JOptionPane.YES_NO_CANCEL_OPTION,
						JOptionPane.QUESTION_MESSAGE);
				if (answer == JOptionPane.YES_OPTION) {
					// load the network
					loadButton.doClick();
		...
		// They manage the interchange of artificial neural network between Venice and
		// Joone
		...
		loadButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				...
				if (action == JFileChooser.APPROVE_OPTION) {
					...
					try {
						// Load
						artificialNeuralNetwork.loadNeuralNet(pathTextField.getText());
						JOptionPane.showInternalMessageDialog(desktop, Locale.getString("OK_READING_FROM_FILE"),
								Locale.getString("ANN_IMPORT_EXPORT"), JOptionPane.INFORMATION_MESSAGE);
						customANNButton.setSelected(true);
					} catch (FileExtensionException ex) {
						// Not a valid Joone extensions error
						showErrorMessage(Locale.getString("ERROR_EXTENSION_READING_FROM_FILE", file.toString()),
								Locale.getString("INVALID_ANN_ERROR"));
					} catch (Exception ex) {
						// General file loading error
						showErrorMessage(Locale.getString("ERROR_READING_FROM_FILE", file.toString()),
								Locale.getString("INVALID_ANN_ERROR"));
					}
			...
			public void actionPerformed(final ActionEvent e) {
				...
				if (action == JFileChooser.APPROVE_OPTION) {
					...
					try {
						...
						JOptionPane.showInternalMessageDialog(desktop, Locale.getString("OK_WRITING_TO_FILE"),
								Locale.getString("ANN_IMPORT_EXPORT"), JOptionPane.INFORMATION_MESSAGE);
						customANNButton.setSelected(true);
					} catch (FileExtensionException ex) {
						// Not a valid Joone extensions error
						showErrorMessage(Locale.getString("ERROR_EXTENSION_WRITING_TO_FILE", file.toString()),
								Locale.getString("INVALID_ANN_ERROR"));
					} catch (Exception ex) {
						// General file saving error
						showErrorMessage(Locale.getString("ERROR_READING_TO_FILE", file.toString()),
								Locale.getString("INVALID_ANN_ERROR"));
					}
		...
		pathTextField = GridBagHelper.addTextRow(innerPanel, Locale.getString("PATH_ANN"), "", gridbag, c, 17);

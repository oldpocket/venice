...
public class ANNPage extends Page implements IAnalyserPage {
	...
	/**
	 * Construct a new input/output artificial neural network parameters page. It
	 * manages: the input of artificial neural network choice with a ordered table
	 * of expressions. the output thresholds of artificial neural network, there are
	 * 2 output thresholds, one for buy signal and one for sell signal.
	 *
	...
	public boolean parse() {
		...
		try {
			if (!buyThresholdTextRow.getText().equals("")) {
				buyThreshold = Double.parseDouble(buyThresholdTextRow.getText());
			}
			if (!sellThresholdTextRow.getText().equals("")) {
				sellThreshold = Double.parseDouble(sellThresholdTextRow.getText());
			}
		...
		if (((buyThreshold < 0.0D) || (buyThreshold > 1.0D)) || ((sellThreshold < 0.0D) || (sellThreshold > 1.0D))) {
			showErrorMessage(Locale.getString("ANN_THRESHOLDS_ERROR"), Locale.getString("INVALID_ANN_ERROR"));
			returnValue = false;
		}
	...
	private void setGraphic(Dimension preferredSize) {
		...
		TitledBorder titledBorder = new TitledBorder(Locale.getString("ANN_PAGE_PARAMETERS_LONG"));
		...
		TitledBorder titledBorderSectionOne = new TitledBorder(Locale.getString("INPUT_EXPRESSION_COLUMN_HEADER"));
		TitledBorder titledBorderSectionTwo = new TitledBorder(Locale.getString("OUTPUT_EXPRESSION"));
		...
		buyThresholdTextRow = GridBagHelper.addTextRow(innerPanelOne, Locale.getString("OUTPUT_BUY_THRESHOLD"), "",
				gridbag, c, 12);
		sellThresholdTextRow = GridBagHelper.addTextRow(innerPanelOne, Locale.getString("OUTPUT_SELL_THRESHOLD"), "",
				gridbag, c, 12);

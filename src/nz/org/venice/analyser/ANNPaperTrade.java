 ...
 * network. A specific class has been developed, extended from PaperTrade
 * {@link PaperTrade}, because ANNs have a complete different behaviour compared
 * to other analysis based on Gondola language. ANNs need to be trained, and the
 * training session needs to know how the things would be happened, if different
 * choices have been taken day by day. For further information about the
 * techniques used, you should find out the Cross Target technique (@author
 * Prof. Pietro Terna). That is the technique used here to get the buy/sell
 * signals. See http://web.econ.unito.it/terna/ct-era/ct-era.html.
 *
 * <p>
 * The final portfolio will contain a single cash and a single share account.
 *
 * Cross Target method to get buy and sell signal through an ANN. The cross
 * target method works in the following way: we make some guesses about buy and
 * sell signals (actions) and about capital (effect of actions), the guesses are
 * done by artificial neural network (ANN); then we train the ANN comparing what
 * the ANN has guessed with the following values: the buy and sell signals are
 * compared with the buy and sell signals which would be to get a capital equal
 * to the capital guessed plus the percental increment wished; the capital
 * signal is compared with the capital got trading with the guessed buy and sell
 * signals.
 *
 * For the sake of simplicity in Merchant of Venice we've used a simplified
 * version of CT technique. We do not use the capital as output of ANN, but we
 * use only two outputs (the buy and sell signals). We pilot the buy and sell
 * signals according to what happens in the future: we calculate if we gain
 * enough in one of the next days (one from the next day trading until the
 * window forecast day trading). We gain enough if and only if the earning
 * percentage is higher than the user defined one, in one of the window forecast
 * days.
 *
 * The core of the CT method has done in the setANNTrainingParameters method in
 * this class.
 *
...
public class ANNPaperTrade extends PaperTrade {
	...
	/**
	 * Iterate through our stock holdings on the given date and decide whether to
	 * sell any stock.
	 *
		...
		for (Iterator iterator = stockHoldings.iterator(); iterator.hasNext();) {
			...
			try {
				...
				for (int ii = 0; ii < inputDoubles.length; ii++) {
					inputDoubles[ii] = inputExpressions[ii].evaluate(variables, quoteBundle, symbol, dateOffset);
				}
	...
	/**
	 * Iterate through all the stocks on the market on the given date and decide
	 * whether to buy any stock.
	 *
	 * @param environment             the paper trade environment
	 * @param quoteBundle             the historical quote data
	 * @param variables               any Gondola variables set
			...
			for (Iterator iterator = symbols.iterator(); iterator.hasNext();) {
				...
				if (!environment.shareAccount.isHolding(symbol)) {
					...
					try {
						...
						double[] inputDoubles = new double[inputExpressions.length];
						for (int ii = 0; ii < inputDoubles.length; ii++) {
							inputDoubles[ii] = inputExpressions[ii].evaluate(variables, quoteBundle, symbol,
									dateOffset);
						}
						...
						if (buy[artificialNeuralNetwork.OUTPUT_BUY]) {
							...
							// Did we have enough money to buy at least one share?
							// Will the stock reach the price wanted (tradeValueWanted)?
							if (buy(environment, variables, symbol, stockValue, tradeCost, tradeValueWanted,
									dateOffset + 1)) {
	...
	public static Portfolio paperTrade(String portfolioName, EODQuoteBundle quoteBundle, Variables variables,
		...
		Environment environment = paperTrade.new Environment(quoteBundle, portfolioName, startDate, endDate, capital,
				tradeValueBuy, tradeValueSell);
		...
		while (dateOffset < environment.endDateOffset) {
			...
			variables.setValue("capital", getCapital(environment.portfolio, environment.quoteBundle, dateOffset));
			...
			sellTrades(environment, quoteBundle, variables, dateOffset, tradeCost, symbols, orderCache,
					inputExpressions, artificialNeuralNetwork);
			buyTrades(environment, quoteBundle, variables, dateOffset, tradeCost, symbols, orderCache, stockValue,
					inputExpressions, artificialNeuralNetwork);
		...
		setTip(environment, quoteBundle, variables, dateOffset, tradeCost, orderCache.getTodaySymbols(dateOffset),
				orderCache, inputExpressions, artificialNeuralNetwork);
	...
	public static void paperTraining(String portfolioName, EODQuoteBundle quoteBundle, Variables variables,
		...
		while (dateOffset < environment.endDateOffset) {
			...
			for (Iterator iterator = symbols.iterator(); iterator.hasNext();) {
				Symbol symbol = (Symbol) iterator.next();
				/*
				 * Set ANNInputArray and ANNOutputDesiredArray arrays row by row. Each call to
				 * setANNTrainingParameters method makes an assignment to a single row.
				 */
				setANNTrainingParameters(ANNInputArray[ANNArrayPointer], ANNOutputDesiredArray[ANNArrayPointer],
						environment, quoteBundle, variables, dateOffset, symbol, inputExpressions,
						artificialNeuralNetwork, ANNTrainingPage.getMinEarningPercentage(),
		...
		// Train the ANN
		artificialNeuralNetwork.runTraining(ANNInputArray, ANNOutputDesiredArray, ANNTrainingPage.getLearningRate(),
				ANNTrainingPage.getMomentum(), ANNTrainingPage.getPreLearning(), ANNTrainingPage.getTotCycles(),
				ANNArrayPointer);
	}
	 ...
	 * @param endDate                 last date of trading
	 * @param capital                 initial capital in the portfolio
	 * @param numberStocks            try to keep this number of stocks in the
	 *                                portfolio
	 * @param tradeCost               the cost of a trade
	 * @param tradeValueBuy           the value at which we want to buy
	 * @param tradeValueSell          the value at which we want to sell
	...
	public static void paperTraining(String portfolioName, EODQuoteBundle quoteBundle, Variables variables,
		...
		while (dateOffset < environment.endDateOffset) {
			...
			try {
				...
				stockValue = portfolioValue.divide(numberStocks).subtract(tradeCost.multiply(2));
			} catch (MissingQuoteException e) {
				// Ignore and move on
			}
	...
	private static void setTip(Environment environment, EODQuoteBundle quoteBundle, Variables variables, int dateOffset,
		...
		setSellTip(environment, quoteBundle, variables, dateOffset, tradeCost, symbols, orderCache, inputExpressions,
				artificialNeuralNetwork);

		setBuyTip(environment, quoteBundle, variables, dateOffset, tradeCost, symbols, orderCache, inputExpressions,
				artificialNeuralNetwork);
	...
	/*
	 * This is the core method which manages the Cross Target technique. All the CT
	 * method is described at the beginning of this class.
	 */
	private static void setANNTrainingParameters(double[] ANNInputArrayRow, double[] ANNOutputDesiredArrayRow,
		...
		try {
			for (int ii = 0; ii < inputExpressions.length; ii++) {
				ANNInputArrayRow[ii] = inputExpressions[ii].evaluate(variables, quoteBundle, symbol, dateOffset);
			}
		...
		/*
		 * open and close prices of the next trading days (the next trading days are
		 * equal to window forecast parameter). buySignal is activated if there is at
		 * least one day among the window forecast days, when we gain a percentage equal
		 * to or higher than earning percentage.
		 */
		double openPrice = 0;
		...
		if (buySignal) {
			// BUY AND NOT SELL
			ANNOutputDesiredArrayRow[artificialNeuralNetwork.OUTPUT_BUY] = artificialNeuralNetwork.HIGH_BOOL;
			ANNOutputDesiredArrayRow[artificialNeuralNetwork.OUTPUT_SELL] = artificialNeuralNetwork.LOW_BOOL;
		} else {
			// NOT BUY AND SELL
			ANNOutputDesiredArrayRow[artificialNeuralNetwork.OUTPUT_BUY] = artificialNeuralNetwork.LOW_BOOL;
			ANNOutputDesiredArrayRow[artificialNeuralNetwork.OUTPUT_SELL] = artificialNeuralNetwork.HIGH_BOOL;
		}
	}
}

 ...
 * work with (a quote range). This page is used by both the
 ...
 * </ul>
 *
 * The date range denotes the date range of the quotes to work with. The
 * optional period parameter allows this page to let the user specify multiple
 * date ranges, the date ranges aren't specified explicitly here but are created
 * from the given period option. The symbols denotes the symbols to work with.
 * The order denotes the preferred order in working with the symbols.
 *
...
public class QuoteRangePage extends Page implements AnalyserPage {
	...
	public void save(String key) {
		...
		if (allowMultipleDateRanges) {
			settings.put("is_multiple_date_ranges", multipleDateRangesCheckBox.isSelected() ? "1" : "0");
	...
	public boolean parse() {
		...
		try {
			...
			showErrorMessage(Locale.getString("ERROR_PARSING_DATE", e.getDate()),
		...
		if (startDate.after(endDate)) {
			showErrorMessage(Locale.getString("DATE_RANGE_ERROR"), Locale.getString("INVALID_QUOTE_RANGE_ERROR"));
			return false;
		}

		if (!QuoteSourceManager.getSource().containsDate(startDate)) {
			showErrorMessage(Locale.getString("NO_QUOTES_DATE", startDateTextField.getText()),
					Locale.getString("INVALID_QUOTE_RANGE_ERROR"));
			return false;
		}

		if (!QuoteSourceManager.getSource().containsDate(endDate)) {
			showErrorMessage(Locale.getString("NO_QUOTES_DATE", endDateTextField.getText()),
					Locale.getString("INVALID_QUOTE_RANGE_ERROR"));
			return false;
		}
		...
		try {
			...
			showErrorMessage(Locale.getString("DATE_ON_WEEKEND", startDateTextField.getText()),
					Locale.getString("INVALID_QUOTE_RANGE_ERROR"));
			return false;
		}
		...
		try {
			...
			showErrorMessage(Locale.getString("DATE_ON_WEEKEND", endDateTextField.getText()),
					Locale.getString("INVALID_QUOTE_RANGE_ERROR"));
			return false;
		}
		...
		try {
			...
			showErrorMessage(e.getMessage(), Locale.getString("INVALID_QUOTE_RANGE_ERROR"));
			return false;
		}
		...
		if (orderByExpressionButton.isSelected()) {
			try {
				...
				showErrorMessage(e.getReason(), Locale.getString("ERROR_PARSING_EQUATION"));
	...
	/**
	 * Return the period. Only call this function if you selected multiple date
	 * ranges.
	 *
	 * @return the period.
	 */
	...
	private void layoutPage() {
		...
		// Date Range Panel
		{
			...
			startDateTextField = GridBagHelper.addTextRow(innerPanel, Locale.getString("START_DATE"), "", gridbag, c,
					15);
			endDateTextField = GridBagHelper.addTextRow(innerPanel, Locale.getString("END_DATE"), "", gridbag, c, 15);
		...
		if (allowMultipleDateRanges) {
			...
			multipleDateRangesCheckBox = GridBagHelper.addCheckBoxRow(innerPanel,
					Locale.getString("ENABLE_MULTIPLE_DATE_RANGES"), false, gridbag, c);
		...
		// Symbols Panel
		{
			...
			symbolListComboBox = GridBagHelper.addSymbolListComboBox(innerPanel, Locale.getString("SYMBOLS"), "",
					gridbag, c);

...
public class ANNPageModule extends AbstractTable implements IModule {
	...
	/*
	 * Put a fixed length for the order column, so that ordering the strings is the
	 * same as ordering the numbers. Put blank spaces before the string
	 * representation of the number, so that they are comparable as strings.
	 */
	...
	private class Model extends AbstractTableModel {
		...
		/*
		 * This add the results without ordering them. The method is called only when
		 * expressions are loaded from preferences, we do not calulateOrderColumn
		 * because we do not want to change the order column.
		 */
		...
		/*
		 * This add the results ordering them. The method is called only when add one
		 * element to the expressions' table.
		 */
		...
		// Recalculate the order column, so that all elements are always correctly
		// ordered
		private void calculateOrderColumn() {
			...
			while (ii < this.getRowCount()) {
				...
				for (int jj = 0; jj < this.getRowCount(); jj++) {
					int current = -1;
					String strCurr = new String(((String) this.getValueAt(jj, ORDER_COLUMN)).trim());
					try {
	...
	/**
	 * Construct a new input artificial neural network parameters grid. It manages:
	...
	public ANNPageModule(Page page) {
		...
		// Add the order column, it's the column containing the numbers that specify the
		// order
		// of the expression in the artificial neural network input
		columns.add(new Column(ORDER_COLUMN, Locale.getString("ORDER_EXPRESSION"),
				Locale.getString("ORDER_EXPRESSION_COLUMN_HEADER"), String.class, Column.VISIBLE));
		// Add the expression column, it's the column containing the expressions
		columns.add(new Column(EXPRESSION_COLUMN, Locale.getString("INPUT_EXPRESSION"),
				Locale.getString("INPUT_EXPRESSION_COLUMN_HEADER"), String.class, Column.VISIBLE));
	...
	private void handleMouseClicked(MouseEvent event) {
		...
		if (event.getButton() == MouseEvent.BUTTON3) {
			...
			JMenuItem popupStoreMenuItem = new JMenuItem(Locale.getString("ADD_EXPRESSION"));
			...
			JMenuItem popupEditMenuItem = new JMenuItem(Locale.getString("EDIT_EXPRESSION"));
			...
			// Add a menu item to remove the selected expressions of the table of
			// expressions
	...
	/*
	 * Sort the table considering the order column. The method is called when the
	 * ANNPageModule is created, so that the values saved in the preferences be
	 * ordered correctly.
	 */
	private void sortResults() {
		// Call a method of SortedTable class so that the values are sorted by
		// ORDER_COLUMN
	...
	public void addExpression() {
		...
		Thread thread = new Thread(new Runnable() {
			public void run() {
				String newExpression = ExpressionEditorDialog.showEditDialog(Locale.getString("ADD_EXPRESSION"), "");
				String[] newExpressionArray = new String[NUMBER_COLUMN];
				// we don't care about order,
				// calculateOrderColumn method will do instead.
				newExpressionArray[ORDER_COLUMN] = (new Integer(model.getRowCount())).toString();
	...
	public void editExpression() {
		...
		if (row != -1) {
			...
			Thread thread = new Thread(new Runnable() {
				public void run() {
					String newExpression = ExpressionEditorDialog.showEditDialog(Locale.getString("EDIT_EXPRESSION"),
	 ...
	 *
	 * @param expression the expression that's going to be added in the table of
	 * expressions
	 */
	...
	public boolean parse() {
		...
		if (inputExpressions.length <= 0) {
			page.showErrorMessage(Locale.getString("NO_GENERATION_ERROR"), Locale.getString("INVALID_ANN_ERROR"));
		...
		for (int ii = 0; ii < inputExpressions.length; ii++) {
			...
			inputExpressions[index] = ExpressionFactory.newExpression(iExp);
			if (inputExpressions[index] == null) {
				page.showErrorMessage(Locale.getString("ERROR_PARSING_EXPRESSION", iExp),
	...
	/*
	 * Get input expressions of the artificial neural network The input are not
	 * ordered, we'll order them later with the parse() method
	 *

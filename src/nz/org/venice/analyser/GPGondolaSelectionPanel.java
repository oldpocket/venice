...
public class GPGondolaSelectionPanel extends JPanel {
	...
	// So we have to check that other percentages in the same panel are different
	// from zero.
	...
	public GPGondolaSelectionPanel(int elements, JDesktopPane desktop, int[] defaultValues,
	...
	/**
	 * Generate a random number between 0 and the number of fields in the panel,
	 * optionally restricting the last, and the secondlast fields.
	 * 
	...
	public int getRandom(boolean isOkLastButOne, boolean isOkLast) {
		...
		for (int ii = 0; ii < totalLength; ii++) {
			...
			if (((!isOkLastButOne) && (ii == (totalLength - 2))) || ((!isOkLast) && (ii == (totalLength - 1)))) {
				continue;
			}
	...
	public boolean isAllValuesAcceptable() {
		...
		try {
			...
			JOptionPane.showInternalMessageDialog(desktop, Locale.getString("ERROR_PARSING_NUMBER", e.getMessage()),
					Locale.getString("INVALID_GP_ERROR"), JOptionPane.ERROR_MESSAGE);
			retValue = false;
		}

		if (!isAllValuesPositive()) {
			JOptionPane.showInternalMessageDialog(desktop, Locale.getString("NO_POSITIVE_VALUES_ERROR"),
					Locale.getString("INVALID_GP_ERROR"), JOptionPane.ERROR_MESSAGE);
			retValue = false;
		}
	...
	private void setNumericalValues() throws ParseException {
		...
		for (int ii = 0; ii < perc.length; ii++) {
			if (!percTextField[ii].getText().equals("")) {
				perc[ii] = (int) Math
						.round(PERCENT_DOUBLE * (decimalFormat.parse(percTextField[ii].getText()).doubleValue()));
			}
		}
	}
	...
	private boolean isTotalOK() {
		...
		// Check total == 0
		if (total == 0) {
			JOptionPane.showInternalMessageDialog(desktop, Locale.getString("NO_TOTAL_GREATER_THAN_ZERO_ERROR"),
					Locale.getString("INVALID_GP_ERROR"), JOptionPane.ERROR_MESSAGE);
			retValue = false;
		}
		return retValue;
	...
	private void setGraphic() {
		...
		for (int ii = 0; ii < percTextField.length; ii++) {
			percTextField[ii] = GridBagHelper.addTextRow(innerPanel, defTextFieldValues[ii], "", gridbag, c,

...
public class GPPageInitialPopulation extends Page implements IAnalyserPage {
	...
	// mutations number (how many times the mutations would be applied by GP
	// algorithm)
	...
	public boolean parse() {
		...
		if (!isAllValuesAcceptable()) {
			...
			if (!isFitAll()) {
				ConfirmDialog dialog = new ConfirmDialog(desktop, Locale.getString("GP_FIT_PAGE"),
	...
	private boolean isAllValuesAcceptable() {
		...
		if (!isAllValuesPositive()) {
			showErrorMessage(Locale.getString("NO_POSITIVE_VALUES_ERROR"), Locale.getString("INVALID_GP_ERROR"));
			retValue = false;
		}
	...
	private void setNumericalValues() throws ParseException {
		...
		if (!generateRandomPopTextRow.getText().equals("")) {
			perc[PERCENT_RANDOM] = (int) Math
					.round(PERCENT_DOUBLE * (decimalFormat.parse(generateRandomPopTextRow.getText()).doubleValue()));
		} else {
			perc[PERCENT_RANDOM] = 0;
		}
		if (!generateInitPopTextRow.getText().equals("")) {
			perc[PERCENT_INIT_POP] = (int) Math
					.round(PERCENT_DOUBLE * (decimalFormat.parse(generateInitPopTextRow.getText()).doubleValue()));
		} else {
	...
	private boolean isTotalOK() {
		...
		// Check total == 0
		if (total == 0) {
			showErrorMessage(Locale.getString("NO_TOTAL_GREATER_THAN_ZERO_PAGE_ERROR"),
					Locale.getString("INVALID_GP_ERROR"));
			retValue = false;
		}
		return retValue;
	...
	private void setGraphic(Dimension preferredSize) {
		...
		generateRandomPopTextRow = GridBagHelper.addTextRow(innerPanel,
				Locale.getString("GP_PAGE_GENERATE_RANDOM_PERC_TEXT_ROW"), "", gridbag, c, 12);
		generateInitPopTextRow = GridBagHelper.addTextRow(innerPanel,
				Locale.getString("GP_PAGE_GENERATE_PERC_TEXT_ROW"), "", gridbag, c, 12);
		numberMutationTextRow = GridBagHelper.addTextRow(innerPanel,
				Locale.getString("GP_PAGE_GENERATE_NUMBER_MUTATION_TEXT_ROW"), "", gridbag, c, 6);
		...
		// Put the default values so that random initial population is the default
		// behaviour

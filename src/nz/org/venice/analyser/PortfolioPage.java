 ...
 * This page is used by both the {@link PaperTradeModule} and {@link GPModule}.
 * The page contains the following user fields:
 *
 * <ul>
 ...
 * </ul>
 *
 * The initial capital field describes the initial amount of money that is
 * available for trading in the portfolio. The trade cost field determines the
 * cost for accumulating or reducing a stock holding. The last two fields
 * describe whether the portfolio has a fixed maximum number of stocks, or has a
 * variable number of stocks depending on the current value of the portfolio.
 *
...
public class PortfolioPage extends Page implements IAnalyserPage {
	...
	/**
	 * Portfolio has a variable number of stocks depending on the value of the
	 * portfolio.
	 */
	...
	public void save(String key) {
		...
		settings.put("mode", (numberStocksButton.isSelected() ? "number_stocks" : "stock_value"));
	...
	public boolean parse() {
		...
		try {
			if (!stockValueTextField.getText().equals(""))
				stockValue = new Money(Float.parseFloat(stockValueTextField.getText()));

			if (!numberStocksTextField.getText().equals(""))
				numberStocks = Integer.parseInt(numberStocksTextField.getText());

			if (!initialCapitalTextField.getText().equals(""))
				initialCapital = new Money(Float.parseFloat(initialCapitalTextField.getText()));

			if (!tradeCostTextField.getText().equals(""))
				tradeCost = new Money(Float.parseFloat(tradeCostTextField.getText()));
		...
		if (initialCapital.isLessThanEqual(Money.ZERO)) {
			showErrorMessage(Locale.getString("NO_CAPITAL_ERROR"), Locale.getString("INVALID_PORTFOLIO_ERROR"));
			return false;
		}

		if (mode == NUMBER_STOCKS_MODE && numberStocks <= 0) {
			showErrorMessage(Locale.getString("EMPTY_PORTFOLIO_ERROR"), Locale.getString("INVALID_PORTFOLIO_ERROR"));
			return false;
		}

		if (mode == STOCK_VALUE_MODE && stockValue.isLessThanEqual(Money.ZERO)) {
			showErrorMessage(Locale.getString("STOCK_VALUE_ERROR"), Locale.getString("INVALID_PORTFOLIO_ERROR"));
			return false;
		}

		if (tradeCost.isLessThan(Money.ZERO)) {
			showErrorMessage(Locale.getString("INVALID_TRADE_COST_ERROR"), Locale.getString("INVALID_PORTFOLIO_ERROR"));
			return false;
		}
	...
	/**
	 * Return the fixed value of each stock in the portfolio. Only call this
	 * function if the portfolio is in {@link #STOCK_VALUE_MODE}.
	 *
	...
	private void layoutPage() {
		...
		// Portfolio panel
		{
			...
			initialCapitalTextField = GridBagHelper.addTextRow(innerPanel, Locale.getString("INITIAL_CAPITAL"), "",
					gridbag, c, 10);
			tradeCostTextField = GridBagHelper.addTextRow(innerPanel, Locale.getString("TRADE_COST"), "", gridbag, c,
					5);

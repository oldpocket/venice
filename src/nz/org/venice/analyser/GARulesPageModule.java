...
public class GARulesPageModule extends AbstractTable implements IModule {
	...
	public GARulesPageModule(JDesktopPane desktop) {
		...
		columns.add(new Column(PARAMETER_COLUMN, Locale.getString("PARAMETER"),
				Locale.getString("PARAMETER_COLUMN_HEADER"), String.class, Column.VISIBLE));
		columns.add(new Column(MIN_PARAMETER_COLUMN, Locale.getString("MIN_PARAMETER"),
				Locale.getString("MIN_PARAMETER_COLUMN_HEADER"), String.class, Column.VISIBLE));
		columns.add(new Column(MAX_PARAMETER_COLUMN, Locale.getString("MAX_PARAMETER"),
				Locale.getString("MAX_PARAMETER_COLUMN_HEADER"), String.class, Column.VISIBLE));
		...
		if (row != -1) {
			...
			Thread thread = new Thread(new Runnable() {
				public void run() {
					...
					boolean[] areas = { false, false, false };
	...
	public void save(HashMap settings, String idStr) {
		...
		for (int i = 0; i < results.size(); i++) {
			...
			if (!((values[PARAMETER_COLUMN].compareTo(nullString) == 0)
				...
				String value = values[PARAMETER_COLUMN] + separatorString + values[MIN_PARAMETER_COLUMN]
	...
	public void load(String value) {
		...
		if (values.length == NUMBER_COLUMN) {
			...
			if (!((values[PARAMETER_COLUMN].compareTo("") == 0) && (values[MIN_PARAMETER_COLUMN].compareTo("") == 0))) {

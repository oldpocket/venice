...
public class ANNTrainingPage extends Page implements IAnalyserPage {
	...
	/**
	 * Construct a new ANN training parameters page. It manages: the training
	 * parameters for the artificial neural network monitor, the cross target
	 * parameters.
	 *
	...
	public void save(String key) {
		...
		PreferencesManager.putAnalyserPageSettings(key + getClass().getName(), settingsInitPop);
	}
	...
	public boolean parse() {
		...
		try {
			if (!learningRateTextRow.getText().equals("")) {
				learningRate = Double.parseDouble(learningRateTextRow.getText());
			}
			if (!momentumTextRow.getText().equals("")) {
				momentum = Double.parseDouble(momentumTextRow.getText());
			}
			if (!preLearningTextRow.getText().equals("")) {
				preLearning = Integer.parseInt(preLearningTextRow.getText());
			}
			if (!totCyclesTextRow.getText().equals("")) {
				totCycles = Integer.parseInt(totCyclesTextRow.getText());
			}

			if (!minEarningPercentageTextRow.getText().equals("")) {
				minEarningPercentage = Double.parseDouble(minEarningPercentageTextRow.getText());
			}
			if (!windowForecastTextRow.getText().equals("")) {
				windowForecast = Integer.parseInt(windowForecastTextRow.getText());
			}
		...
		if ((learningRate < 0.0D) || (learningRate > 1.0D)) {
			showErrorMessage(Locale.getString("ANN_LEARNING_RATE_RANGE_ERROR"), Locale.getString("INVALID_ANN_ERROR"));
			returnValue = false;
		}
		// Check the range for momentum
		// It must be a number between 0.0 and 1.0
		if ((momentum < 0.0D) || (momentum > 1.0D)) {
			showErrorMessage(Locale.getString("ANN_MOMENTUM_RANGE_ERROR"), Locale.getString("INVALID_ANN_ERROR"));
			returnValue = false;
		}
		...
		if (preLearning < 0) {
			showErrorMessage(Locale.getString("ANN_PRE_LEARNING_RANGE_ERROR"), Locale.getString("INVALID_ANN_ERROR"));
			returnValue = false;
		}
		// Check the range for totCycles
		// It must be a positive number and not equal to zero
		if (totCycles <= 0) {
			showErrorMessage(Locale.getString("ANN_TOT_CYCLES_RANGE_ERROR"), Locale.getString("INVALID_ANN_ERROR"));
			returnValue = false;
		}
		...
		if (windowForecast < 1) {
			showErrorMessage(Locale.getString("ANN_WINDOW_FORECAST_RANGE_ERROR"),
					Locale.getString("INVALID_ANN_ERROR"));
			returnValue = false;
		}
	...
	/**
	 * Get pre learning of ANN. pre learning is the initial ignored input patterns
	...
	/**
	 * Get tot cycles of ANN. tot cycles is how many times the net must be trained
	...
	/**
	 * Get the earning percentage of ANN. Earning percentage is the percentage we
	 * want to gain, That's what we want to gain in a number of days equal or less
	...
	private void setGraphic() {
		...
		TitledBorder titledBorder = new TitledBorder(Locale.getString("ANN_TRAINING_PARAMETERS_LONG"));
		...
		TitledBorder titledBorderSectionOne = new TitledBorder(Locale.getString("ANN_TRAINING_PARAMETERS_SHORT"));
		TitledBorder titledBorderSectionTwo = new TitledBorder(Locale.getString("CROSS_TARGET"));
		...
		learningRateTextRow = GridBagHelper.addTextRow(innerPanelOne,
				Locale.getString("TRAINING_PARAMETER_LEARNING_RATE"), "", gridbag, c, 12);
		momentumTextRow = GridBagHelper.addTextRow(innerPanelOne, Locale.getString("TRAINING_PARAMETER_MOMENTUM"), "",
				gridbag, c, 12);
		preLearningTextRow = GridBagHelper.addTextRow(innerPanelOne,
				Locale.getString("TRAINING_PARAMETER_PRE_LEARNING"), "", gridbag, c, 12);
		totCyclesTextRow = GridBagHelper.addTextRow(innerPanelOne, Locale.getString("TRAINING_PARAMETER_TOT_CYCLES"),
				"", gridbag, c, 12);
		...
		minEarningPercentageTextRow = GridBagHelper.addTextRow(innerPanelTwo, Locale.getString("EARNING_PERCENTAGE"),
				"", gridbag, c, 12);
		windowForecastTextRow = GridBagHelper.addTextRow(innerPanelTwo, Locale.getString("WINDOW_FORECAST"), "",
				gridbag, c, 12);

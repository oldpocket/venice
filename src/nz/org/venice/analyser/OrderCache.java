...
/**
 * Caches the order of stock quotes in a quote bundle. The GP allows the user to
 * order the evaluation of stock quotes, by several criteria such as unordered,
 * alphabetically by symbol, volume decreasing etc. This gives priority to
 * certain stocks. For example the user might prefer the GP to concentrate on
 * high volume stocks, so ordering the stocks by volume will give high volume
 * stocks a priority. Since sorting is a slow operation, and the GP traverses
 * over the same day many times, it makes sense to cache the order. Profiling
 * results showed this provides a significant speed increase.
 *

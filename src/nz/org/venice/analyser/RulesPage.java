...
/**
 * An analysis tool page that lets the user enter a buy and sell rule, or a
 * family of related buy and sell rules. This page is used by the
 ...
 * respectively. The rule family option allows the user to embedd variables in
 * the rules. This enables them to specify a family of simillar rules. For
 * example, a buy rule might be <code>avg(close, 15) > avg(close, 30)</code>. If
 * the rule family is enabled, the user could enter
 * <code>avg(close, a) > avg(close, b)</code>. Then the paper trade would try
 * each rule combination of [a, b].
 *
...
public class RulesPage extends Page implements AnalyserPage {
	 ...
	 *
	 * @return <code>true</code> if rule families are enabled, <code>false</code>
	 *         otherwise
	 */
	...
	private void layoutPage() {
		...
		// Rule Family panel
		{
			...
			ruleFamilyEnabledCheckBox = GridBagHelper.addCheckBoxRow(innerPanel,
					Locale.getString("ENABLE_MULTIPLE_RULES"), false, gridbag, c);
			...
			aRangeTextField = GridBagHelper.addTextRow(innerPanel, Locale.getString("RANGE_A_1_TO"), "", gridbag, c, 3);
			bRangeTextField = GridBagHelper.addTextRow(innerPanel, Locale.getString("RANGE_B_1_TO"), "", gridbag, c, 3);
			cRangeTextField = GridBagHelper.addTextRow(innerPanel, Locale.getString("RANGE_C_1_TO"), "", gridbag, c, 3);
